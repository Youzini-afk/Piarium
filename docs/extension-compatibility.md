...
| `pi-mcp-adapter` | upstream `2.27.0`; maintained fork `f71064c` | Merged upstream into the maintained fork without dropping `configCatalog/v1`; the catalog reflects the plugin's exclusive one-source mode, and committed public build artifacts keep the maintained Git source installable without development-only Pi SDK packages. |
