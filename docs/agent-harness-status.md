...
下一步按观察到的"找不到入口"决定词法索引/桥接/embedding。plan 3.11 第 1–5 步已做（D-091/D-093–D-132）。验证纪律注意 D-103：`packages/web` 完整套件的退出码目前会因 `lib/run/supervisor.test.ts` 崩溃隔离用例的未捕获 `EPIPE` 间歇为 1，而断言全过——读结果要看断言数，不能只看退出码，该项与两处挂钟阈值已立项未修。
以下区分已接线行为与仍缺证据。
