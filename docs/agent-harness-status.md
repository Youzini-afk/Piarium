...
| **1.3** `bash` / shell 监督器（PTY、持久 login shell、自动转后台） | host / pi-host | ✓ | ✓ | `host/shell-supervisor.test.ts`；`shell-discovery.test.ts`；`harness-shell-settings.test.ts`；`shell-assembly.test.ts`（生产 `discoverShells()` + Host 默认装配 + 公开 `shell.exec` 真执行）；`harness-e2e.test.ts` #1–3；Windows 真实 smoke（D-200） | ✓ | Pi 内置 bash | 后台 shell 不是终端 tab——D-013 的前置条件"接进 terminal runtime"未兑现；macOS / Linux 真机 smoke 未做；bundled Pi 默认优先级另排 |
...
| **3.4 / 3.5** 原生线程运行时与 7 个工具 | protocol / broker / host / pi-host | ✓ | ✓ | `thread-runtime-session.e2e.test.ts`；`thread-worktree.test.ts`（Git/non-Git、fixed/live、占用路径不删、回收重建）；`thread-runtime.test.ts`（归档保留 report、占用恢复、keep_worktree）；`thread-registry.test.ts`；`thread-space.test.ts`；`thread-routes.test.ts`（archive/restore/reclaim/space）；`phase3-e2e.test.ts`；`worktree-reclaim-guard.test.ts` | ✓（Web/Application Host） | Host 未声明 harnessThreads 时不注册 | 内部目录仍在 Run 启动时物化，父 blocks 也在 Run 启动时读取；完整结果验证记录与虚拟工具仍待接；scope 非 OS 沙箱。归档/空间见 D-202；surface 写回见 3.5a / D-201 |
...
| **3.10** session state rail / overlay / discussion threads | ui / host | ✓ | ✓ | `thread-routes.test.ts`（session 权威、鉴权、integration、archive/restore/reclaim/space）；`thread-runtime.test.ts`（只读存活、blocks 选择、同 session 转换、归档恢复）；`thread-runtime-session.e2e.test.ts`（真实 Pi：开、聊、转实现、继续回答）；`HarnessThreadsPanel.test.ts`（事件合并、archived 投影、space 解析）；`PiTimelineEntries.renderMode.test.tsx`；`HarnessSessionStateTrigger.test.tsx`；`piariumEvents.test.ts` | ✓（工作区持久消息与 session state 有内容时） | Host 无线程运行时时不显示入口；原线程工具仍可操作 | 浏览器内未跑归档点击链。rail、overlay 与时间线标记共用一个 session-scoped feed（D-062–D-064）。合并预览见 D-201；归档/占用见 D-202 |
...
| **3.5a** 固定修订 Integration、草稿写回与绑定预览（D-201） | host / protocol / ui | ✓ | ✓ | `working-state/three-way-merge.test.ts`；`working-state/integration-coordinator.test.ts`（固定结果、类型/mode、草稿路径零写入、surface 正文应用、dirty 分类、父继续编辑失效、故障补偿、真实 Documents 保存排队、共享存储隔离、冲突重试）；`working-state/integration-parents.test.ts`；`documents/registry.test.ts`（operationId 分组撤销、未保存）；`thread-routes.test.ts`（preview/merge/ack）；`HarnessThreadsPanel.test.ts`（dirty 投影） | ✓（磁盘 + Document Registry surface；线程面板与 `thread.merge` 共用 Host 计划） | 恢复失败保留具体路径与 needs-attention；缓冲不可用标 unavailable，不写磁盘；预览未知/未完成不能标 merge-ready | 浏览器内未跑完整用户点击链；验证结果仍不绑定“测试通过”；文本检查不保证行为兼容 |
...
| 物化生命周期 | Git/非 Git 结果已独立发布；copy snapshot 按修订保留，不清空旧版本。copyIgnored 根随 branch 持久化，窄发布只枚举其子树并把变化纳入 native result。用户归档保留结果与转录；恢复按原路径重建，占用路径不删。先保存结果、关闭 session，再持有 Documents 写者屏障回收；活跃写者、编辑器、后台命令、keep_worktree、未完 Integration 或未收集内容保留目录并记录原因。占用区分物化目录与共享对象，预算只沿用户配置 | `thread-worktree.test.ts`、`thread-runtime.test.ts`、`thread-space.test.ts`、`thread-routes.test.ts`、`worktree-reclaim-guard.test.ts`、`thread-worktree-settings.test.ts`、`working-state/working-state-store.test.ts`；未显式分类的 setup 产物仍会保留目录，Git filters/LFS 与 CoW 仍待实施。真实 ENOSPC / 浏览器归档点击链未测 |
