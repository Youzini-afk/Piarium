...
**D-252 已采纳 Rust 系统内核架构；D-273 纠正了 D-267–D-272 的过度 wired/proven 表述；D-275 已关闭 R1，D-276 已关闭 R2，D-277 已关闭 R3。R0 仍按自身证据记为 Partial，阶段 R 整体尚未完成。**
目标与完整范围见 [rust-kernel-design.md](rust-kernel-design.md)，执行顺序为 plan R0–R6。当前仍运行 TS/Node Host 与 Pi worker；
R4–R6 和 R0 的剩余跨平台/发行进程证据仍未交付，不把 R1–R3 完成误写成整个 Rust 阶段完成。
...
D-277 已完成 R3：生产 baseline 的 filesystem inventory/body capture、immutable root materialization、staging/backup/restart reconcile、execution-directory remove 与空间测量已进入 kernel；Git 只保留真实 inventory/index/filter 与 linked-worktree metadata 适配，不再用 TS copy 回写 workspace body。native result root 已取代生产 mandatory snapshot；旧 TS materializer/switch/snapshot 仅作测试 seam。R4–R6 仍未交付。R0 的跨平台/发行进程证据仍按其自身里程碑保持 Partial，不反向阻塞 R1–R3。
...
| R2 文件与恢复 | **Complete**（production file authority / shared resource gate / restart reconcile） | Rust `fileResources` 拥有 Documents-authorized canonical root identity、exact/subtree lease、稳定 capture、条件 apply、mkdir/remove/rename、真实对象安装与 started-operation restart reconciliation。Documents write/move/delete、workspace-scoped Files CRUD、Recovery/Integration disk apply/compensation 共用该后端；生产 `fs.lock` 也是 kernel lease。Piarium 模式的 Pi `write`/`edit`/`apply_patch` 只走 `document.branchWrite` / `document.surfaceWrite`，Host mutation backend 不可用时明确失败，不再退回 pi-host 直写。Registry 仍是 unsaved-buffer authority；mixed surface receipt/grouped undo 由 Host 定向协调，Rust 不存第二份编辑器正文。WorkspaceAPI/Git/bulk/external adapter 尚由各领域执行其副作用，但已登记同一 kernel exact/subtree writer，不再维护第二套可写 gate。D-277 后 baseline/materialization/CoW/执行目录生命周期已由 R3 单独收口 |
| R3 基线与物化 | **Complete**（fixed baseline / durable materialization / managed-directory lifecycle） | Rust `file.scan`/`file.capture` 负责生产 filesystem baseline inventory/body capture；捕获后按同一路径集合再次核对 file state，Git dirty inventory 另保留内容身份窗口，`captureScopes` 冻结范围参与 baseline 与 materialized result。`file.materialize` 从 immutable root 构建 operation-specific staging，校验对象后尝试真实 clone backend（Linux FICLONE / macOS `clonefile`）或正式 copy fallback，再以 backup/promote + restart reconcile 切换 execution generation；Windows 本机只证明 copy 并明确 `reflink:0`。Git execution context 只迁移 linked-worktree admin metadata，`read-tree` + `add` + internal baseline commit 使用真实 filter/EOL/index 语义，required clean filter 失败会中止且不改 Rust materialized bytes。materialized settle 由 Git changed paths + prior/current frozen captureScopes 或非 Git Rust inventory 发布 immutable result root；生产 archive/history 不再依赖旁路 `.snapshot`。reclaim/discard/delete 通过 kernel subtree remove，linked-worktree metadata prune；`file.measure` 在 Unix 报实际 blocks，Windows physical allocation 未证明时 `allocatedBytes:null`。Thread/Run 回收策略、活动 writer/command guard 和引用保护仍由 Host/Registry 决定。真实 Windows release kernel 的 R3 scan/measure/materialize/restart 反例与 Host Thread/WorkingState focused suites通过；未测文件系统不宣称 CoW 已证明 |
