...
下一步按 plan 3.11（D-091）：第 1–3 步已做（结构接口 + LSP outline + explore 切片、冷启动对照、web-tree-sitter TS/TSX 与命中分类；D-098–D-102 修验收缺陷）。验证纪律注意 D-103：`packages/web` 完整套件的退出码目前会因 `lib/run/supervisor.test.ts` 崩溃隔离用例的未捕获 `EPIPE` 间歇为 1，而 196 文件 / 1653 项断言全过——读结果要看断言数，不能只看退出码，该项与两处挂钟阈值已立项未修。
