...
| **3.17** 按命令整理 bash / get_output（D-197） | host / pi-host / protocol | ✓ | ✓ | `output-organize/organize.test.ts`（vitest/tsc/eslint/git 成功、失败、无法识别；中段失败块；ANSI/CRLF/分片；超预算省略；`shell.exec` 原文+display；增量 read 整理且显式分页仍是原文）；`bash-tool.test.ts` / `output-tools.test.ts`（公开工具用 display）；`tool-result-truncation.test.ts`（bash/get_output 不再头尾切）；`observation-services.test.ts`（原始游标） | ✓ | 未识别命令走通用展示；显式 offset/length 与 `out_` 读原文 | 未接包管理器通配、TOML 规则、jest 专名或附加模型总结。不是完整 shell 解释器 |
