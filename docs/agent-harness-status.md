...
| R1 状态与存储 | Partial（root/path 纵切、产品记录领域 wire、preview 与 directory operation port 已 wired；完整 consumer cutover 未完成） | 当前唯一格式为 v8。Rust 已拥有 blob、AVL/Merkle root、branch/revision/current pin、typed recovery tables、domain record/reference 与 GC；root/path/range 读写、固定 query pin、materializer 输入和精确 retrieval record ref 已走 kernel。result/draft/child/parent verification/review 已有独立生成 DTO 和领域方法，result 持久 envelope 不再保存完整状态平表；Integration preview 与 directory apply 已消费选定 root/路径和 Rust operation port，但 ThreadRuntime/IntegrationCoordinator 仍有 callback 级投影、branch undo/reconcile 及其他同步消费者，combined Recovery/agent-mutation 仍需切换，故不能标 R1 proven/default-on |
...
当前生产接线仍是 Application Host 启动并管理 kernel；`Thread/Run` catalog 仍由 TS、未保存缓冲仍由 Document Registry、Pi session/model/credential 仍由 Pi、知识/向量仍由 TriviumDB/adapter。WorkingState root/path 与 retrieval/turn/checkpoint/mutation 走 kernel；combined Recovery/operation-file、Integration durable journal 和 agent-mutation 仍写 TS recovery SQLite。该 SQLite 是尚未迁移职责的当前 writer，不是兼容 fallback；R1 下一切片必须以 typed kernel API 替换并删除它。R2–R6 保持未完成。
