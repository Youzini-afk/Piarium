...
下一步按 plan 3.11（D-091）：第 1–3 步已做（结构接口 + LSP outline + explore 切片、冷启动对照、web-tree-sitter TS/TSX 与命中分类）。
