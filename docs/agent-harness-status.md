...
3.12 已接（符号图读者，D-133–D-138），验收另把「目录建得起来」这个前置条件从不可用修到可用（枚举 74 s → 199 ms，建目录 18.4 → 4.8 分钟，D-140）。**确认连接修正**（D-143）：literal-call 查询原来匹配参数列表里任意位置的字符串、且不匹配 awaited 泛型调用；修后 `explore-tool.ts` 的 `request("explore.search")` 首次进图，全库重扫边数 14040 → 13459。**3.13 已接**（D-144–D-149）：对象优先于问句；任务匹配分层，不再把路径字母序当 RRF 名次、不再把 `GROUP_WEIGHT` 与 RRF 混加；问题对象先查图；已在 rg 池但未读的图命中仍物化；理由绑定窗口；`connects`/`associates` 分等；`limit` 是上限；因直接线索已验证而跳过的泛词记 `direct-verified`。这是观察，不是评测，不声称检索质量或速度提升。**3.14 已接**（D-153–D-156）：查询内词组权重可见；同 tier 比较加权覆盖；同一张表进入局部选择与打包；定位题有答案后不再展开/填 `offTopic`。验收补三项（D-157）：`roleFit` 降为第三键后测试夹具一度压过生产注册（问题 1 首条变夹具），改为关系相当时用比较表达生产优先；`details.windows` 改按需开启（问题 2 实测 482 窗口 / 185 KB，`details` 总计 295 KB，可见预算仅 24 KiB）；量具补第三态「单元已选中但所需证据在打包正文之外」。不声称检索质量或速度。**下一步已排期而非"等观察"**（D-158–D-160）：3.15 把 explore 收敛到"答案要求 → 取证 → 证据单元 → 一次联合选择 → 正文与呈现记录"，拆十一项打分公式；3.16 语义来源（嵌入第三路候选、重排、查询扩展），`models.embedding` / `models.rerank` 槽位本地默认远程可选，索引指纹绑定，不设信任门与花费守卫；3.17 bash 输出压缩按命令分派规则解析替掉字节头尾切。**3.16 第一片索引侧 已接**（D-166–D-169）：本地空间/配方身份、范围键代际存储、结构切块、Host 后台扫描；缺模型包 `unavailable`。检索进 explore 仍待同一片后半。**3.15①②④接口 已接**（D-163–D-165）：图到达理由；共享查询期文件角色；`focusRanges` 与 `arrival` / `assessment` / `purpose` 已替换命中行入口和 `EvidenceGrade`，`hasBothConnectsEnds` 要求同一连接值。其余 3.15 项与 3.16 仍是已决定、未实施；3.15/3.16 联合设计已采纳（D-161：证据单元 `arrival / assessment / purpose` 三分、`focusRanges` 替 `hitLines`、文件级 RRF 限读取调度、重排替掉 `windowScore`、三类身份、代码向量独立代际存储、捕获时 overlay），执行顺序 3.15②④接口 → 3.16 四片串行。检索范围定为一等参数（D-162：不索引整个电脑，分层、语义跟注意力走；3.16 第一片守范围键与文档身份两条接缝约束，用户级 / 工作集 / 集合范围等 `knowledge-work-in-files` 消费者）。AFT（`cortexkit/aft`）作参照，但其切块（符号名 + 签名 + 正文前 300 字符）是反例；`knowledge/embedding.ts` 已有七家远程适配器是 3.16 的起点。plan 3.11 第 1–5 步、3.12、3.13 与 3.14 已做。D-103 第 2 项已由 D-133 修；第 1 项（`thread-runtime` 20ms stalled）和第 3 项（pi-host `harness-e2e` #3）仍待。
以下区分已接线行为与仍缺证据。
