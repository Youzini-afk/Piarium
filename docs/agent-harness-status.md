...
服务视图隔离与正文修订绑定，D-088/D-089 已完成写入失效与写入守卫；D-090 第一组已实施，缺陷 1 的候选广度按 D-092 待补。
下一步按 plan 3.11（D-091）：先定结构来源接口并以 agent 视图 `documentSymbol` 实现、让结构切片消费带修订的符号范围；量一次冷启动
对照基线；再引入 web-tree-sitter + TS/TSX 包作第二 provider 并接命中分类。
以下区分已接线行为与仍缺证据。
...
| 结构来源 / 3.11 | 未开始。已决定（D-091）：结构来源为带修订绑定的可插拔 provider，首个包装 agent 视图 `documentSymbol`，第二个是 web-tree-sitter（Host 内、wasm 版、语法包 = 语法 wasm + Piarium 查询、按内容哈希缓存）；消费者依次为 explore 切片、命中分类、连接边、冷仓库符号目录与 `imports` 边。分发：TS/TSX/JS/JSON 随应用捆绑，其余按需下载，目标覆盖大部分常用语言；语言 ≥ 3 时才做设置页 | 接口与 LSP 实现无新依赖；引入 tree-sitter 前先在本仓库量一次 agent 视图冷启动到 `documentSymbol` 可用的时间记此处作对照。已覆盖语言与解析耗时实测后记此处，不预先声称 |
