...
| **3.10** session state rail / overlay | ui / host | ✓ | ✓ | `thread-routes.test.ts`；`HarnessThreadsPanel.test.ts`；`HarnessSessionStateTrigger.test.tsx`（窄屏入口与数量）；`HarnessKnowledgeReviewSection.test.tsx`；`piariumEvents.test.ts`（SSE 重连/增量/失效） | ✓（有 thread/block/suggestion 时；桌面 rail、窄屏 overlay） | 线程工具仍可完整操作 | 时间线线程卡片与用户创建/转换讨论线未做；overlay 与 rail 共用数据和操作，不是第二套状态（D-062） |
