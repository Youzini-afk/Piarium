...
**D-278 已重新审查 R0–R3：R1 核心状态/存储接管保持 Complete；R2/R3 的生产原语和主要消费者保留，但整体完成度重新打开为 Partial。详见 [审查与返工记录](rust-kernel-audit.md)。R0 仍为 Partial，阶段 R 整体未完成。**
目标与完整范围见 [rust-kernel-design.md](rust-kernel-design.md)，执行顺序为 plan R0–R6。当前仍运行 TS/Node Host 与 Pi worker；
R0、R2/R3 的具体闭环缺口和 R4–R6 仍需分别验收；不以局部子进程测试通过代替整阶段完成。
...
D-277 接入了 R3 filesystem capture、immutable-root materialization、目录回收和测量原语。D-278 修复跨 root 租约、重试、GC、路径和实际 owning/execution 装配，并以真实 Documents/gate 补证据；但低层未决文件操作的 Host 处置，以及 kernel promotion → Git baseline → Thread Registry/view 的持久交接仍未闭环，因此 R2/R3 为 Partial。旧 TS seam 的单测不代表 native 生命周期已验证。
...
| R2 文件与恢复 | **Partial**（production authority wired / recovery acceptance reopened） | Documents/Files/Recovery/Integration 与 fs.lock 已接 Rust file authority；D-278 修复跨 root/alias 互斥、定向 lease coverage、owner 身份、root 替换和不可逆操作重试，真实 Documents save/move/delete 及 Combined Recovery restart/undo 使用实际 kernel gate 通过。Registry 仍是 buffer 权威。剩余：低层 pending file operation 不能只在 root.register 返回数量后被 Host 丢弃；未决目录 rename 等必须形成可见、可解释、可重入的处置闭环，而不是猜成功 |
| R3 基线与物化 | **Partial**（native primitives / major consumers wired） | Rust scan/capture/materialize/measure/remove、Git execution metadata 与主要 writeback 已接入；D-278 修复真实 owning/execution 与 retained managed-root admission、分页漂移、未收集内容保护和 readonly 安装。剩余：native kernel operationId/source root/writeRevision、Git executionBaseline、Thread Registry 和 execution view 必须有持久 switch intent/receipt 跨域对账；setup timeout/abort 不得以 kill 请求代替真实退出；旧 seam 不能冒充 native 纵切。Windows 只证明实际 copy；未测平台不虚报 CoW，不要求本地其他系统 |
