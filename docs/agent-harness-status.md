...
3.12 已接（符号图读者，D-133–D-138），验收另把「目录建得起来」这个前置条件从不可用修到可用（枚举 74 s → 199 ms，建目录 18.4 → 4.8 分钟，D-140）。**确认连接修正**（D-143）：literal-call 查询原来匹配参数列表里任意位置的字符串、且不匹配 awaited 泛型调用；修后 `explore-tool.ts` 的 `request("explore.search")` 首次进图，全库重扫边数 14040 → 13459。**3.13 已接**（D-144–D-149）：对象优先于问句；任务匹配分层，不再把路径字母序当 RRF 名次、不再把 `GROUP_WEIGHT` 与 RRF 混加；问题对象先查图；已在 rg 池但未读的图命中仍物化；理由绑定窗口；`connects`/`associates` 分等；`limit` 是上限；因直接线索已验证而跳过的泛词记 `direct-verified`。这是观察，不是评测，不声称检索质量或速度提升。**3.14 检查点一已接**（D-153）：查询内词组权重与截断三态写入 `details.distinctiveness`，观察脚本能分开「窗口从未生成 / 生成未选中」。比较器未改，不声称检索质量或速度。**下一步**是 3.14 检查点二（加权覆盖进主比较），然后才是按观察决定词法索引/桥接/embedding。plan 3.11 第 1–5 步、3.12、3.13 与 3.14 检查点一已做。D-103 第 2 项已由 D-133 修；第 1 项（`thread-runtime` 20ms stalled）和第 3 项（pi-host `harness-e2e` #3）仍待。
以下区分已接线行为与仍缺证据。
