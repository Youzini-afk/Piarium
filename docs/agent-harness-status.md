...
服务视图隔离与正文修订绑定；下一步是让 `explore` 的结构展开消费带修订的符号范围。
以下区分已接线行为与仍缺证据。
