...
**D-215 嵌套线程接线仍未完全正确。** 角色目录与 `parent.kind: "thread"` 数据模型保留。D-216 已拆开 owning/execution 身份；D-217 已让改父虚拟分支走同一写 gate；D-218 已补 dispatch 基线诚实。物化父集成对象库根、冻结权限和父结束后级联终止仍待本轮后续阶段。
...
**D-218 已补正 dispatch 基线诚实、mode 全字段比较与失败清理。** Git 错误不再吞成空清单或普通目录；捕获窗口变化和活跃 Documents writer 以可重试 `baseline-changed` 失败；gitlink/unsupported 不得静默当空目录或跳过物化；新虚拟文件在形成结果前写入真实默认 mode；apply/补偿按 `sameState`。3.4 / 3.4a 在嵌套权限冻结与级联终止完成前保持 Partial。
