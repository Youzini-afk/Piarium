...
| **3.2** `explore` 管线 | pi-host Coordinator / host Engine | ✓ | ✓ | `host/explore.test.ts`（19，含 workspaceScope 传递、signal 中断、unavailable 映射、outputStore 分页落盘与 handle 返回）；`pi-host/src/harness/explore-tool.ts`；`harness-services.ts` | ✓ | 不注册 | —（可用链路已验证并接通生产 bridge） |
| **3.3** `related` | host | ✓ | ✗ | 单测 | — | — | pi-host 无工具定义 |
| **3.4 / 3.5** 原生线程运行时与 7 个工具 | protocol / broker / host / pi-host | ✓ | ✓ | `thread-runtime-session.e2e.test.ts`（真实持久 Pi child、父 blocks、冻结模型/工具/scope、报告）；`thread-worktree.test.ts`（真实 Git、dirty baseline、zero-commit/non-Git 复制后端、reclaim/rematerialize、result branch/commit、冲突）；`thread-runtime.test.ts`；`thread-registry.test.ts`；`path-authority.test.ts` / `search-service.test.ts`；`phase3-e2e.test.ts`；`zone2-threads.test.ts`（含 overlapWarning 计算与折叠渲染）；`thread-runtime-capability.test.ts`；`runtime-dispatcher-session-launch.test.ts`；`session-delete-coordinator.test.ts` | ✓（Web/Application Host） | Host 未声明 harnessThreads 时不注册 | zero-commit 生命周期与非 Git 复制后端已贯通；条件补偿与用户编辑保护已实现并落盘；动态 worktree 设置生效；scope 非 OS 沙箱 |
...
| **3.4a** 内容寻址工作分支与按需物化 | host / protocol / pi-host / ui | ✓ | ✓ | `working-state/working-state-store.test.ts`（SHA-256 内容寻址、分支、提交、目录快照与磁盘持久化及重载）；`working-state/materializer.test.ts`（物化、权限、软链与移除文件清理）；`working-state/draft-baseline.test.ts`（草稿基线整合） | ✓ | 当前 Git worktree/resultCommit | 完整空间策略与引用保留按需执行 |
| **3.5a** 版本化 Integration 与合并预览 | host / protocol / ui | ✓ | ✓ | `working-state/three-way-merge.test.ts`（13 用例：二进制检测、三方文本合并、冲突标记注入、结构冲突）；`working-state/integration-coordinator.test.ts`（4 用例：干净应用、文本冲突标记、I/O失败条件补偿回退、用户后续编辑安全保护不被覆盖并报告 needs-attention）；`recovery/journal-catalog.test.ts`（持久化事务日志） | ✓ | 当前合并流程 | 验证绑定与双修订预览进一步丰富 |
...
| 窗口读取 / 3.2 | 已实现并接线：explore 管线完整贯通，传递 workspaceScope 与 signal 级联取消，unavailable 错误映射，outputStore 溢出分页与 handle 返回；pi-host 默认注册 explore 工具；草稿基线支持从编辑器 buffer 快照读取 | `host/explore.test.ts`（19）覆盖范围与取消；`explore-tool.ts`、`select-tools.ts` 接通真实调用链；headless 读磁盘 |
...
| 结果与集成 | 已实现并接线：固定结果消费，逐路径三方合并，条件补偿安全检查（仅当磁盘内容仍等于本次写入时才回退，用户后续编辑安全保留并报告 needs-attention），持久化事务日志落盘到 SQLite 与 JSON；thread-services.ts 真实返回 appliedPaths、status 与 diffStats | `integration-coordinator.test.ts`（4）覆盖合并、冲突标记、条件补偿、用户编辑保护；`three-way-merge.test.ts`（13）；`journal-catalog.test.ts`（17）；`working-state-store.test.ts`（4）覆盖持久化与重载 |
| 物化生命周期 | 已实现并接线：zero-commit 与非 Git 复制后端已贯通，reclaim/materialize 目录重建与文件保留，合并正确消费 untracked/changedFiles；动态 worktree settings 支持从工作区配置读取并取消硬编码 600s 超时；Zone 2 支持 active 线程 overlapWarning 预警 | `thread-worktree.test.ts` 覆盖零提交/非 Git 线程的完整生命周期（prepare/reclaim/materialize/merge）；`zone2-threads.test.ts` 覆盖重叠路径警告与折叠 |
| `check` | 现有角色含 bash 且 shared，具备命令执行能力 | 不称只读 agent，不阻止测试/构建正常生成文件；不新增统一副本要求 |

**D-078 本地实施证据**：
- 全量单元测试：Host harness 测试套件 `377 passed, 1 skipped (Windows symlink), 0 failed`（35 个文件）；pi-host 测试套件 `138 passed, 0 failed`（23 个文件）；Node/Vitest recovery 与 working-state `38 passed, 0 failed`（4 个文件）。
