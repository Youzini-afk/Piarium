...
**D-230 已纠正 D-227 的事实权威与生命周期。** retrieval 仍是 `thread.dispatch(role: "retrieval")` 的可等待 Thread。Host 只能标 `source-checked` / `source-valid`，不能称 claim `verified`，也不能因来源齐全且 unknowns 为空就写 `complete`。pending evidence 绑定 `runId`；封印报告保存耐久 artifact，不保存 ephemeral OutputRef。URL 必须带 Host receipt。嵌套 retrieval 读父冻结有效状态，不因 `worktree: none` 回根 live。定向反例已关；真实付费 retrieval、完整桌面 Host 重启与授权 web 抓取仅未实测。不得把该纵切写成 Proven/完整实施。证据见 3.6 retrieval。
