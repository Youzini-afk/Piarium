...
（任务、仍有效要求、成果、未决项、历史锚点）已实现并有定向测试，尚待阶段F由线程spawn消费。D-285的3.18A已落地：
dispatch改为任务中心（`task`必填、`preset`可选），普通派发继承发起者当前模型与活动工具，`shared`只作显式选择，
Run启动时冻结模型/工具/权限/scope/worktree/prompt片段/inputOrigin（`task`或`inherit`），Thread持久化与UI投影改用
`preset`（旧`role`记录仅在历史导入边界读取），嵌套preset-less派发的工具声明不得超出父Run冻结allowlist，自动review
默认改为关闭。仍待实施：task/inherit之外的`continue`/`fresh`输入、定向消息语义、同根共享执行调度、分段成果与依赖更新；
send仍只收直接子active/running，countActive仍按parent分别统计，waiting仍不释放名额。
...
这是当前运行事实；默认持续维护与 coverage 接管的设计已被 D-284 取代，生产替换尚待实施。
...
| **1b.3** 搜索 provider / `websearch` | host / pi-host / ui | ✓ | ✓（配置后） | `web-search.test.ts`（Brave/Exa/Tavily/Jina/SearXNG HTTP 契约、持久/工具域名策略交集、provider unavailable）；`web-search-routes.test.ts`（search-only credential ref/鉴权/不回传 key）；`session-e2e.test.ts`（真实 Pi websearch → Host provider）；`session-registration.test.ts`（provider identity 按 worker generation 冻结、无关坏设置不污染 web binding）；`websearch-tool.test.ts`（credential 撤销明确 unavailable）；`harness-settings.test.ts`（workspace 不可改 provider/credential/render，只能收紧 domain）；Settings 生产入口 | 用户配置后的新会话，无需重启 Host | 无 provider 的会话不注册 `websearch`；凭据被撤销后旧会话请求明确 unavailable | provider identity / render / domain policy 对已创建 worker generation 冻结；credential 每次请求从 Pi auth 解析，不缓存旧 key，不静默换 provider；不做多 provider 隐藏回退（D-283） |
...
| **上下文完整取舍/fresh（D-286）** | pi-host / host / ui | 部分 | 部分 | `history-fresh.test.ts`（`buildFreshInput`：任务/有效要求/成果/未决/锚点、限量截断、无携带价值时诚实空输入、session goal 兜底） | — | — | fresh 构造接缝已实现，阶段 F 的线程 spawn 消费与 task/inherit 尚未接线；不依赖新评分/清洗模型 |
| **3.18A–E（D-285）** 可续做任务线程与定向协作 | protocol / pi-host / host / ui | — | Partial（3.18A） | `protocol/test/harness-threads.test.ts`（preset/frozen契约）；`host/presets.test.ts`（可选预设解析、无便宜/强标签）；`host/thread-services.test.ts`（preset-less继承、显式shared、未知preset拒绝、嵌套工具越权denied）；`host/thread-registry.test.ts`（frozen写入、legacy role归一化）；`host/review-sensor.test.ts`（review默认关）；`pi-host/test/harness/phase3b-e2e.test.ts`（preset askBefore） | — | 任务中心派发、可选预设、Run冻结、preset持久化/UI、嵌套工具约束与review默认关已上线；continue/fresh、定向消息、共享准入、分段成果仍走旧语义 | 3.18B–E待实施；角色旧路径已删除，无双写 |
...
D-285现状对照（3.18A后）：`harness-presets.ts`绑定模型/工具/worktree，`thread-tools.ts`的dispatch只须`task`、`preset`可选；
Run已写入frozen配置；`harness-settings.ts`默认自动review关闭。`thread-runtime.ts`启动仍主要用
brief/parent blocks，parentSession只表关系，不等于继承当前输入；`thread-services.ts:createThreadSendService`仍拒绝settled，
