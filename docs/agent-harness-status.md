...
**D-206 已接通后台 shell 与 terminal runtime，并收掉默认路径的两个缺口。** 公开 `bash` 后台命令与用户终端 tab 共用同一会话身份和进程；关闭查看不杀进程；bundled Pi 在无显式选择时优先于 PATH/system；todo/plan 的 confidence 不再默认弹确认。证据与未测平台见 1.3 / 2.5。

**D-207 已接通固定结果的验证记录与默认自动 review。** 子线程发布结果时绑定同 Run 的真实命令记录；合并可应用性、子检查、父检查分开呈现；review 从结算入口 `createThread` + `startRun` + `spawn`，结论回到源线程与 Zone 2。证据与未测范围见 3.4 / 3.5 / 3.7。

**D-208 已接通 Settings 知识目录与 suggestions 槽位的用户消息提议。** Settings 对 workspace/user `.tdb` 做列表/查看/编辑/停用与取代链；公开 recall 与 Zone 2 只看当前有效 accepted。配置 `models.suggestions` 后 pi-host 草拟、Host `knowledge.suggest` 落库；未配置不借用主模型。证据与未测范围见 2.7 / 2.10。
...
| **1.3** `bash` / shell 监督器（D-200 / D-205 / D-206） | host / pi-host / ui | ✓ | ✓ | `terminal/runtime.test.ts`（程序化 create/attach、HTTP 忽略 spawn/owner、DELETE retain、harness 不可 restart、idle 跳过、上限不计 harness）；`terminal-harness-bridge.test.ts`（同一 handle 上用户附着与 agent 读写）；`shell-assembly.test.ts`（本机真实 Git Bash 后台附着与输入回显、PowerShell 连续调用与非零退出）；`shell-supervisor.test.ts`（转后台后继续采集、无显式 cwd 仍保护会话目录、真实退出/迟到写者）；`openHarnessTerminal.test.ts` / `useTerminalStore.test.ts`（detach tab、不重开）；`runtime-broker/test/runtime-manager.test.ts`（无选择时用 bundled，显式 system 保持优先） | ✓ | Host 不提供能力时保留 Pi 内置；已配置解释器/设置失败明确 unavailable | macOS / Linux 真机 smoke 未做；浏览器完整点击链未跑。bundled Pi 是 runtime 选择，不是 `harness.shell` |
...
| **1.9** `HarnessSettings` + 设置页 | protocol / pi-host / ui | ✓ | ✓ | `protocol/test/harness-settings.test.ts`（含 `review` 用户所有、默认 enabled/非 gate）；`pi-host/test/harness/session-e2e.test.ts`（Smart 与动态 memory 设置进入真会话）；`harnessMemoryPresentation.test.ts` | ✓ | — | 模型槽位、memory mode、automatic review 与 user memory auto-accept 为用户所有；memory / review 的 workspace 值不覆盖，全局默认与 session override 实时解析（D-031/D-081/D-207） |
...
| **2.5** `todo` / `plan` 块 | host / pi-host / ui | ✓ | ✓ | `host/todo-tool.test.ts`（低 confidence 默认直写；仅 `requireConfirmation` 等待）；`pi-host/test/harness/todo-tool.test.ts`（Host 未要求时不弹 UI，明确审批策略才问一次，取消不重试 Host）；`phase2-e2e.test.ts`；session state 侧栏可见可编辑（D-046） | ✓ | 不注册 | confidence 只作信息。未配置审批策略时不弹确认（D-206） |
...
| **2.7** 知识建议 / 审阅托盘 / 取代链 | host / ui / pi-host | ✓ | ✓（user-mark + memory decisions + Settings catalog + suggestions 槽位用户消息） | `knowledge-suggestions.test.ts`（同 scope 取代、user-message 含 dismissed 去重）；`context-routes.test.ts`（审阅托盘 auth/409/取代）；`knowledge-catalog-routes.test.ts`（Documents workspace 身份、编辑/停用/取代链、跨 scope 不串）；`store.test.ts`（accepted 编辑、单 id 停用、reopen 后 recall 不含旧正文、user store 拒非 user 写）；`knowledge-services.test.ts`（`knowledge.suggest` 落库与驳回去重）；`decision-suggestions.test.ts`；pi-host `knowledge-suggestion-extension.test.ts`（未配置不调用、JSON/null 解析、Host 落库）；`KnowledgeSettingsPage.test.ts` / `knowledgeCatalogRequest.test.ts`；既有审阅托盘与标记测试 | ✓（用户显式动作；keeper decisions；配置 suggestions 后的用户消息） | 未配置 suggestions 槽位时不调模型、不借用主模型；suggested/dismissed/superseded 不参与 recall | 真实付费 suggestions 模型质量未观察；Settings 完整浏览器点击链未跑。删除是 `invalidAt` 停用，不是物理删节点 |
...
| **3.4 / 3.5** 原生线程运行时与 7 个工具 | protocol / broker / host / pi-host | ✓ | ✓ | Pi `thread-runtime-session.e2e.test.ts`（真实会话、选定旧结果合并与父回合撤销）；`thread-lifecycle.acceptance.test.ts`（真实 Git/native：删除目录、原路径重建、继续修改、旧新结果独立可读）；`thread-worktree.test.ts`（Git/non-Git、准备取消归属）；`thread-runtime.test.ts`（关闭后失败重试、结构化阶段、guard 内复核、并发预算、恢复与自动回收互斥；D-207：真实命令绑定到 published revision、隐藏 review `startRun`+spawn、完成结论回写源线程）；`thread-registry.test.ts`；`thread-space.test.ts`；`thread-routes.test.ts`；`phase3-e2e.test.ts`；`worktree-reclaim-guard.test.ts`；`working-state/verification-records.test.ts`；`working-state/working-state-store.test.ts`（验证记录无 schema bump 重开）；`verification-coordinator.test.ts`（同 Run 绑定、目录外 unbound、新修订不继承旧 review、迟到旧完成不成当前、草稿合并不可验证未保存缓冲） | ✓（Web/Application Host） | Host 未声明 harnessThreads 时不注册 | 内部目录仍在 Run 启动时物化，父 blocks 也在 Run 启动时读取；虚拟工具仍待接；scope 非 OS 沙箱。验证记录已接（D-207），不把 `allExitedZero` 写成“结果已通过”。真实 ENOSPC、macOS/Linux、非 Git/无 HEAD 的完整桌面归档恢复未实测。surface 见 3.5a / D-203 |
| **3.6** 角色目录 / 团队提示 | protocol / pi-host | ✓ | ✓（随 dispatch） | `host/roles.test.ts`（14） | ✓（随 dispatch） | 未配置槽位的角色不出现 | — |
| **3.7** review 传感器 | host / protocol / ui | ✓ | ✓ | `host/review-sensor.test.ts`（空 diff / 关闭 / 无角色跳过；hidden + `startRun`；同修订去重并取消旧 running；父 `onAgentSettled` 失败关闭）；`thread-runtime.test.ts`（settle 后 create+start+spawn，prompt 含任务与 published diff、不含父对话，完成结论与 findings 回写源线程）；`zone2.test.ts`（三事实分行、`<review>`、仅 review 也组装）；`protocol/test/harness-settings.test.ts`（`review` 用户所有、默认 enabled/非 gate） | ✓（默认审阅；gate 默认关） | 未配置 review 槽且无主模型时跳过，不是失败；`enabled: false` 不派发 | 触发是已发布子结果，不是父 journaled 变化。隐藏线程不进默认 `<threads>` / UI 列表。faux provider 只证明调用链，不是真实模型审阅质量。Settings / 线程面板完整浏览器点击链未跑 |
...
**D-207 本地实施证据**：protocol `harness-settings.test.ts`（review 默认与用户所有）；Host
`verification-records.test.ts`、`verification-coordinator.test.ts`、`working-state-store.test.ts`（无 schema bump 重开）、
`review-sensor.test.ts`、`zone2.test.ts`、`thread-runtime.test.ts`（绑定 + startRun/spawn + 完成回写）定向组通过。
不把 faux provider 写成真实审阅质量。未跑完整浏览器 Settings/线程面板点击链，也未发起付费模型实验。

**D-208 本地实施证据**：protocol `harness.test.ts`（`knowledge.suggest` 进入方法表）；Host `store.test.ts`、
`knowledge-suggestions.test.ts`、`knowledge-catalog-routes.test.ts`、`knowledge-services.test.ts`、
`context-routes.test.ts`；pi-host `knowledge-suggestion-extension.test.ts`；UI `i18nParity.test.ts`、
`metadata.test.ts`、`KnowledgeSettingsPage.test.ts`、`knowledgeCatalogRequest.test.ts`、
`piariumEvents.test.ts` 定向组通过。未跑完整浏览器 Settings 点击链，也未发起付费 suggestions 模型实验。
...
| 来源 | 未完成 |
| --- | --- |
| D-023 | Zone 2 尚缺 user terminal；memory 事件加速触发未接。suggestions 槽位用户消息提议与 Settings 知识目录已由 D-208 接通 |
| D-024 / D-026 | merge/归档后的 worktree 与分支回收已由 D-077/D-202 接线；归档/恢复 UI 见 D-202 |
| D-013 | 已由 D-206 接通 terminal runtime；见矩阵 1.3 |
