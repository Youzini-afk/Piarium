...
**D-215 已接通嵌套线程生产接线。** 具备角色目录嵌套工具的真实 Pi 子线程可 `dispatch` 孙线程；Host 解析 `parent.kind: "thread"`，scope/权限只能继承或收窄，未授权工具被拒绝。嵌套基线复制父分支有效视图；孙结果先集成到父分支，再由父结果进入根工作区。根 Zone 2 / `threads` 只列直接子线程。证据见 3.4 / 3.4a / 3.6。
...
| **3.4 / 3.5** 原生线程运行时与 7 个工具 | protocol / broker / host / pi-host | ✓ | ✓ | Pi `thread-runtime-session.e2e.test.ts`（真实会话、选定旧结果合并与父回合撤销）；`thread-lifecycle.acceptance.test.ts`（真实 Git/native：删除目录、原路径重建、继续修改、旧新结果独立可读）；`thread-worktree.test.ts`（Git/non-Git、准备取消归属、父 Git 身份范围）；`thread-runtime.test.ts`（关闭后失败重试、恢复/回收互斥；命令身份绑定、父 Integration 窗口重载、review startRun/spawn 与精确 Run 回写；虚拟 scratch 回收与 bash 预算预占；dispatch 基线后父漂移隔离）；`thread-services.test.ts`（queued 也准备基线、捕获失败删除 Thread；嵌套 parent/排队、扩 scope 与未授权工具拒绝）；`nested-threads.test.ts`（父分支视图基线、父漂移隔离、孙结果入父分支再入根工作区）；`thread-nesting.test.ts`；`zone2-threads.test.ts`（根列表不含孙线程）；`working-state/workspace-baseline.test.ts`（Git clean/staged/unstaged/untracked/删除/ignored 默认不进/captureScopes 进入、unborn/非 Git、BOM/CRLF/二进制、捕获取消）；`thread-registry.test.ts`；`thread-space.test.ts`；`thread-routes.test.ts`；`phase3-e2e.test.ts`；`worktree-reclaim-guard.test.ts`；`working-state/verification-records.test.ts` / `working-state-store.test.ts`（变化范围身份、一次性观察、旧记录降为 uncertain）；`working-state/working-branch-writes.test.ts`（Host router 虚拟写/读、兄弟隔离、CAS、scope 拒绝、物化后目录发布）；`verification-coordinator.test.ts`（actor/worker/Run/binding generation、start/end/publish 身份、精确父 session/operation、非完整集成、草稿与 Host 重启） | ✓（Web/Application Host） | Host 未声明 harnessThreads 时不注册 | 父 blocks 仍在 Run 启动时读取；scope 非 OS 沙箱。Git 验证绑定覆盖 HEAD/base 与可枚举变化范围；普通 ignored 输入与非 Git 全目录不冒充已验证。`allExitedZero` 不是“结果已通过”。真实付费嵌套 Pi 会话与完整桌面 Host 重启未实测。真实 ENOSPC、macOS/Linux、非 Git/无 HEAD 的完整桌面归档恢复未实测。surface 见 3.5a / D-203；验证见 D-210；虚拟写入见 D-213；dispatch 基线见 D-214；嵌套见 D-215 |
| **3.6** 角色目录 / 团队提示 | protocol / pi-host / host | ✓ | ✓（随 dispatch；嵌套工具随角色 allowlist） | `host/roles.test.ts`（hard-implement/frontend 含 nest 工具，review/retrieval 不含）；`thread-services.test.ts`（未授权 dispatch 拒绝）；pi-host `select-tools` 按 `tools.dispatch` 等与 Host `control.thread` 装配 | ✓（随 dispatch） | 未配置槽位的角色不出现；无 nest 工具的角色不能嵌套 | 嵌套深度无固定上限；兄弟经父 send/结果协调 |
...
| **3.4a** 内容寻址工作分支、草稿基线与结果物化 | host / protocol / pi-host | ✓ | ✓（隔离只读视图 + 虚拟文本写入 + 按需物化 + dispatch 基线 + 嵌套分支） | `working-state/working-state-store.test.ts`（schema 1/2→3、draft objects/ref、固定多修订、窄路径与 captureScopes、effectiveState/origin、writeRevision CAS）；`working-state/draft-baseline.test.ts`；`working-state/materializer.test.ts`；`working-state/branch-view.test.ts`；`working-state/working-branch-view.test.ts`（Host router 上的 read/grep/find/ls，父/worktree drift 隔离与 scope 拒绝）；`working-state/working-branch-writes.test.ts`（虚拟 write 不碰父盘、兄弟隔离、迟到修订冲突、物化后 publishDirectoryResult）；`working-state/workspace-baseline.test.ts`（Git 变化集与 ignored/captureScopes、unborn/非 Git、字节诚实、取消不建分支）；`thread-runtime.test.ts`（surface 释放后的 queued spawn、revision 0、copyIgnored scope、虚拟 spawn 绑定、scratch 回收、bash 预算预占、prepare 后父漂移隔离）；`thread-services.test.ts`（dispatch 必准备、失败删除）；`nested-threads.test.ts`（父虚拟分支作孙基线、嵌套 merge 不写根盘、父结果再入工作区）；`working-state/integration-coordinator.test.ts`（虚拟新文件省略 mode 仍可应用到工作区）；`thread-worktree.test.ts`（fixed/live、virtual scratch）；`working-state/path-requirement.test.ts`；pi-host `read-tool.test.ts` / `find-ls-tool.test.ts`（working-branch provenance）；`workspace-mutation-journal.test.ts` / `apply-patch-tool.test.ts`（`document.branchWrite` 优先，disk 才落盘） | ✓（隔离线程） | 旧 Git base/resultCommit 是导入来源；带草稿的 Thread 缺原生结果时不走旧合并旁路；shared/none 仍读 live 父目录 | Merkle 优化、跨平台 CoW 与历史引用释放 UI 待交付；嵌套见 D-215。物化预算与占用治理见 D-204；显式 copyIgnored 已随 branch 冻结并捕获后续新增/修改/删除；基线读工作目录字节，不把 Git blob 冒充转换后正文 |
...
**D-215 本地实施证据**：protocol `harness-roles.ts`（hard-implement/frontend 含 nest 工具）；Host `thread-services.test.ts`、
`thread-nesting.test.ts`、`nested-threads.test.ts`、`roles.test.ts`、`zone2-threads.test.ts`、`service-host.test.ts`、
`integration-coordinator.test.ts` 定向组覆盖 owning Thread parent、排队、扩 scope/未授权工具拒绝、父分支视图基线、
孙结果入父分支再入根工作区、根投影不含孙线程、省略 mode 的虚拟新文件可应用。未跑真实付费嵌套 Pi 会话或完整桌面 Host 重启。
