...
**D-252 已采纳 Rust 系统内核架构；D-273 纠正了 D-267–D-272 的过度 wired/proven 表述；D-275 已关闭 R1，D-276 已关闭 R2。R0 仍按自身证据记为 Partial，阶段 R 整体尚未完成。**
目标与完整范围见 [rust-kernel-design.md](rust-kernel-design.md)，执行顺序为 plan R0–R6。当前仍运行 TS/Node Host 与 Pi worker；
R3–R6 和 R0 的剩余跨平台/发行进程证据仍未交付，不把 R1/R2 完成误写成整个 Rust 阶段完成。
...
R0–R2 的 kernel vertical 已进入真实 Host→Rust 子进程调用链。R1 的生产 WorkingState consumer 已统一使用 immutable root/path 接口，combined Recovery/Integration/agent-mutation 元数据也统一由 Rust typed recovery writer 持久化；D-275 已收口内置 Recovery 的 storage-location 与耐久验收边界。D-276 进一步把 canonical file root、重叠路径 lease、真实文件 capture/apply/rename/remove/mkdir 与 restart reconciliation 接到 Rust file-resource authority；Documents/Files/Recovery/Integration 和 Piarium 原生 mutation 走同一后端，Registry 继续拥有未保存 buffer。
R3–R6 仍未交付。Git/bulk Workspace adapter 尚由其领域实现执行实际副作用，但在 R2 已登记同一 kernel exact/subtree writer，不再拥有第二套“可写/可回收” gate；baseline/materialization/CoW/执行目录生命周期仍明确属于 R3。R0 的跨平台/发行进程证据仍按其自身里程碑保持 Partial，不反向阻塞 R1/R2。
