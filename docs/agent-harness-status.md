...
（任务、仍有效要求、成果、未决项、历史锚点）已实现并有定向测试，尚待阶段F由线程spawn消费。D-285按plan3.18A–E交付
普通派发/可选预设、task/inherit/continue/fresh、定向消息、同根共享执行调度和分段成果，尚未开始。当前代码仍必填role，
send只收直接子active/running，settled实现线程不能由该入口续做；countActive按parent分别统计，waiting不释放名额；
review仍默认enabled:true/gate:false。
