...
科研入口加入现有 Agent/IDE 切换器并复用切换事务与动画；项目/会话导航保持用户所选 Shell。
...
受管远程实验、跨机器调度/恢复尚未交付，转入 7I；原生集群不在当前范围。登记机器和注入测试 backend 不等于真实后端已经存在。
...
未开展的真机、真实模型和对比实测保留为证据范围说明，不因测试清单未补齐阻断交付。7G/7H/7I 是单独接续的功能阶段。
...
本次仅有文档变更；当前 per-request 接缝、游标或工具已经存在也不证明完整快照、历史前缀、容量与角色链已按 D-301 验收。

**D-302 / 后续 7H（2026-09-20）：工具并行与后台长任务设计已接受，尚未实施/验收。**
本次只核对源码并更新文档，不新增运行时或性能验证结论。内置 Pi 0.85.1 有工具批次并行，任一 sequential 工具使整批串行；
`apply_patch` 当前仍标 sequential。`bash` 已能转后台并经 `get_output`/输入/终止工具继续操作，但通用完成通知尚未接入 Zone 2。
用户终端观察、命令 verification 记录与后台完成通知不是同一条交付链；现有文档的按路径并行目标不能当作实际调度证据。

代码发现的待修接缝：`createShellExecService` 默认等 60 s 转后台，`HostServicesBridge` 默认 30 s，公开 bash 未协调等待期限；
本轮未运行超时复现。工具上的 `runMs` 也未由该 shell 服务传入执行端，7H 要删除空参数或按真实已确定合同接通，不能保留假能力。
目标是资源/依赖调度、短等待或显式后台启动、可取消事件等待，以及完成事实经 7G 在每次请求前增量交付；
日志按需读取，输出增长不唤醒模型，空闲后续做须有明确等待/订阅意图。普通 shell 不冒充跨重启耐久实验。
详见 [Harness 5.9](agent-harness.md#59-并发) 和 [plan 7.11 的 7H](agent-harness-plan.md#711-分阶段交付)。
原交付验收与 7G/7H 后续实施分别记录。

**D-304 / 后续 7I（2026-09-20）：受管远程、多机器执行与按需运维线程，设计已接受、尚未实施。**
顺序为 7G → 7H → 7I。本轮只更新设计与计划，不新增远程或批量功能的交付声明。
...
实施责任见 [plan 7.11 的 7I](agent-harness-plan.md#711-分阶段交付) 与 [科研集群设计第 6 节](research-cluster-design.md#6-实验执行与资源协作)。
