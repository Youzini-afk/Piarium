...
**D-228 已纠正 D-225 的正文身份、整组 undo 与耐久补偿。** 根会话仍经 `document.surfaceWrite` 写回同一 Registry 缓冲，不隐式保存。`bufferHash` 只表示规范化编辑器身份；snapshot 保留文件行尾。apply/undo 共用真实 `operationId`，一次 batch 整组撤销。混合写入在第一笔写前记入 `agent-mutation` 操作；取消/I/O throw 后补偿或 needs-attention。`apply_patch` 仅在 `readSource.source === "disk"` 时读磁盘。定向反例已关；完整桌面 Registry 与 Host 进程重启仅未实测。不得把该纵切写成 Proven/完整实施。证据见 3.2 与本轮 D-228 记录。

**D-229 已纠正 D-226 的命令事实。** 用户 tab 的 bash / PowerShell / zsh 注入带本次 integration 代际标识的 OSC 633；未带本代际标识的 OSC 不生成命令。PowerShell 在 prompt 任何赋值前保存 `$?` 与 `$LASTEXITCODE`。`/restart` 与 start 共用同一 generation/reset。`/bin/sh` 不注入 `--init-file`。默认注入保留用户 PROMPT_COMMAND / DEBUG trap、zsh profile 与 PowerShell Enter/prompt。命令/cwd 进入 Zone 2 与 keeper 前编码，避免关闭标签。`putEvent` 对 `workspaceId + commandId` 幂等，重复投递不二次入库或 nudge；产品链无 PTY 重播，不声称 Host 重启去重。定向反例已关；zsh/macOS/Linux 真机与完整桌面重启仅未实测。不得把该纵切写成 Proven/完整实施。证据见 2.2 / 2.3 / 2.4 与本轮 D-229 记录。
...
| **2.2** Zone 2 组装 | host / pi-host | ✓ | ✓ | `host/zone2.test.ts`（含 `</user-terminal>` 不能拆段）；`knowledge/context-runtime.test.ts`（用户命令进 Zone 2、harness 过滤、commandId 去重、游标不倒退、重建 runtime 后重复 commandId 仍一条）；`pi-host/test/harness/session-e2e.test.ts`（Documents 用户写入与用户终端命令在下一真实 Pi turn 出现、event cursor 不重复、system 不变） | ✓ | 无材料时不追加消息 | Git 与 prompt-relevant accepted knowledge 已接。无 shell integration 时不造 `<user-terminal>` 伪命令。D-229 纠正结构编码与持久 commandId 幂等；不得标 Proven |
| **2.3** host 观察者 | host knowledge | ✓ | ✓（Documents + user-change LSP + Git status + user-terminal 带代际 OSC） | `documents/authority.test.ts`（提交后通知且观察失败不反噬写入）；`knowledge/context-runtime.test.ts`（多会话 fan-out、agent 过滤、诊断因果、raw Git status → workspace → event → Zone 2 与去重、用户终端 commandId 去重、重建 runtime 后重复 commandId 一条且 projector 最多 nudge 一次）；`terminal-projection.test.ts`（user/harness 分流、nudge 失败不反噬、重复 commandId 不 nudge）；`store.test.ts`（`putEvent` 按 commandId 幂等）；`git-status.test.ts` / `git-status-runtime.test.ts`；`git/routes.test.ts` / `workspace-routes.test.ts`；`session-e2e.test.ts`（Documents 与 user-terminal 纵切）；`terminal/shell-integration.test.ts`（只接受本代际标识帧）；`shell-integration-scripts.test.ts`（`sh` 不注入、Bash/zsh/PowerShell 保留用户 hook、Windows PowerShell 公式得 1/7/0）；`runtime.test.ts`（只注入 user、untagged OSC 不生成命令、`/bin/sh` 无 `--init-file`、restart 后新 zsh D 不结算旧命令且下一 commandId 属新 generation） | ✓ | 观察失败只降级本轮上下文并记录 Host 错误；不新增 Git 轮询；无 integration 为 not-observed | Git 外部变化在现有 status 下一次刷新时可见，不声称后台实时；Harness shell / agent 自身事件不重复进 Zone 2（D-054）。D-229 纠正命令事实后不得标 Proven。zsh / macOS / Linux 用户终端与完整桌面 Host 重启仅未实测；不声称 Host 重启去重 |
| **2.4** 记忆 keeper（off / assist / takeover） | protocol / pi-host / host / ui | ✓ | ✓ | `memory-agent-extension.test.ts`（动态 off、拒绝/成功、user-command nudge 合并、off/无回合零调用、重复 commandId 不进 keeper material、`</user-terminal>` 编码）；`phase2-e2e.test.ts`（Host 顺序 apply、off 不注入）；`session-e2e.test.ts`（当前模型后台调用、默认/旧值/实时全局/session override、失败投影，memory_edit 不进主对话）；`context-routes.test.ts` / `harnessBlockPresentation.test.ts` / `harnessMemoryPresentation.test.ts` | ✓（缺省 takeover） | off：不维护、不注入、Pi 压缩；assist：维护/注入、Pi 压缩 | 用户命令完成已接 `memory.nudge`；steering/计划编辑/子返回加速未接。辅助费用/Token 明细按 D-080 不提供。不把 faux keeper 写成真实模型质量。D-229：重复投递最多 nudge 一次；不声称 Host 重启去重 |
...
| 来源 | 未完成 |
| --- | --- |
| D-023 | user terminal 与 memory 事件加速已由 D-226 接通，命令事实由 D-229 纠正。suggestions 槽位用户消息提议与 Settings 知识目录已由 D-208/D-211 接通并补正并发/身份。steering/计划编辑/子返回加速仍未接；zsh/macOS/Linux 用户终端仅协议接入未实测 |
