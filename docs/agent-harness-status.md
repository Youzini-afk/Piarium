...
**D-215 嵌套线程接线仍未完全正确。** 角色目录与 `parent.kind: "thread"` 数据模型保留，但验收发现 Documents 给执行目录另一个 workspaceId 后，thread services / Zone 2 / lost resume 会在错误 catalog 里找父 Thread。D-216 已拆开 owning/execution 身份并用 Host session binding 查找；虚拟写入、基线诚实、集成和权限冻结仍待本轮后续阶段。
...
| **3.4 / 3.5** 原生线程运行时与 7 个工具 | protocol / broker / host / pi-host | ✓ | Partial | `workspace-identity.test.ts`（Documents 给原目录与 scratch 不同 workspaceId；session.create + Harness router + 公开 dispatch 建孙线程；孙写入 owning catalog 且 `parent.kind` 为父 Thread；threads/wait/Zone 2/lost resume 看直接子层）；`thread-registry.test.ts`（session binding 重启后仍指向 owning workspace）；`thread-services.test.ts` / `zone2-threads.test.ts`（execution workspaceId 不能冒充 owning catalog）；`thread-worktree.test.ts`（父 worktree 内的 live 路径走独立 `git init`；外部路径走 `--detach` 并写 `.git/worktrees`；两种情况下 `show-toplevel` 都不是父仓库，子 commit 不改父 HEAD/index/分支列表） | — | Host 未声明 harnessThreads 时不注册 | D-216 已接线 owning/execution 与 Git 物化边界。虚拟写入/修订/崩溃恢复、dispatch 基线诚实、嵌套集成/冻结权限/级联终止仍未完全接线。`nested-threads.test.ts` 把 `resolveRuntimeWorkspaceId` 钉成同一个 `"ws"`，不能作为身份证据 |
