...
`details.windows` 达 482 窗口 / 185 KB，可见预算是 24 KiB）。下面"管线与真实依赖"到"pack"各条记录的是**已交付**的形状，
3.15 逐步把它们收敛到上面五段；哪一段已交付见 status。
...
- *fan-out*：rg（磁盘）经 search-service 一条路径，传入 actor 与本轮 inputContext，脏路径由后端在计数前排除、草稿命中由同一
  ...
  **语义召回是第三路路径候选**（3.16，D-158）：对仓库按结构单元切块、嵌入、建索引，查询时嵌入问题取最近邻，产出候选路径与
  ...
  （第 8.5 节）；索引与后端指纹绑定（后端 + 模型 + 维度 + 切块版本），指纹变了整个索引作废、期间只有词法加图，这是数学事实
  不是防御；远程查询失败或超时回退词法加图，工具不因可选来源挂掉而挂掉。索引以 Documents 修订为准增量更新，与符号目录同一套
  机制。**不设信任门、不设花费守卫**：用户配置远程提供商这个动作本身就是同意，成本是用户自己的账，harness 只做合理优化
  （增量重嵌、查询嵌入缓存、批量）不拿它限制设计（D-158）。远程放开的能力——多粒度嵌入（符号 / 文件摘要 / 目录摘要）、
  更大的重排范围、更强或代码专训的模型、查询扩展——都是本地跑不动而远程跑得起的东西，是这个槽位存在的理由。
...
- *模型增强*：按"当前缺的那一步"选一种，不默认 intent → judge → repair 全跑（策略，不是硬轮数不变量）：没有可匹配的仓库词汇 →
  ...
  几百个候选片段，用交叉编码器逐对打"与问题多相关"的分，输出是可比较的分数而不是路由决定，错了代价小；它替掉的是
  `windowScore` 里那部分手调相关性，不替掉答案要求与证据检查（那些不能补偿）。后端按 `models.rerank` 槽位选：远程可重排
  一两百个候选，本地 CPU 交叉编码器要按延迟预算收窄范围。judge 在候选物化后只返回候选 ID 与理由，未知 ID 拒绝；补查
...
事实标准（Cohere / Jina / Voyage 各异，本地 TEI 仿 Cohere），需要一层适配器、按需求逐家接。本地模型文件走 3.11 第 5 步为语法包
建好的内容寻址存储、sha256 清单与按需下载。
