...
单一 `dim`（`store.ts`），所以**代码语义索引不进这份库**（3.16，D-161）：代码向量是独立的、可重建的**代际存储**，按向量空间
身份与代际命名（`knowledge/{hostId}/{workspaceId}/semantic/{spaceId}/{generation}`），仍用 TriviumDB，仍由 Host 唯一写。
用户换嵌入模型或维度时作废并重建的是这个派生索引，不牵动 blocks、knowledge 与结构图；它不是第二份可写知识权威，
因为它随时能从当前正文重算出来。
