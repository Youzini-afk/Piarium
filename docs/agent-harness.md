...
| 压缩 | 默认由 Pi 压缩。候选接管需要分支匹配、块修订与实际 context entry 覆盖，并使用 Pi 的安全切点；仅有 keeper 块不够。`KeeperCoverageStore` 已接生产检查（D-076），Host 重启后保守丢水位并回退。缺覆盖或必要来源不可用时不接管；真实语义效果由测试者验证（第 8.4/8.6 节） |
...
2. **替换（候选接管，默认关闭）。** 材料是 memory blocks、主 agent 计划与 Host 事实。接管前必须验证 MemoryCheckpoint：分支祖先路径
   ...
   拒绝接管并安全回退到 Pi 默认压缩。生产 takeover 仍保持关闭（`takeoverEnabled` 默认 false），需回放对比后才讨论开启。
...
线程标为等待用户继续，不结算 Thread、不关闭 Pi session。想清楚后一键"转为实现线程"：先挂独立 worktree，再结束讨论 Run、
