...
| 多 agent | **原生**子会话 worker（不依赖 `pi-subagents`）；角色 = **模型 × 任务性质**（检索 / 快速实现 / 难度实现 / 前端 / 审查 / 检查），每个角色独立模型槽位，全部可并发；`dispatch(role, task)` 异步 + `wait`；隔离 / 权限 / 深度由 harness 决定；未配置槽位的角色不注册；并行写者各进 worktree；兄弟不通信 |
| 防过度委派 | 不设配额、不做准入规则、**不估成本**（估不准且会把注意力引向算账）；只靠系统提示说明角色与"自己更快更省就自己做"的判断原则，加并发上限默认 12（超出排队）；"派发前询问"是默认不生效的用户设置 |
| 长时间委派 | `wait` 超时由 harness 按 provider 缓存 TTL 计算，唤醒 = 刷新缓存 + 看进度 + harness 标出卡死的子；整理与压缩永不按时间触发 |
...
| harness 的 UI 投影 | 后台 shell 成为可附着的终端 tab；输出句柄在工具卡片内可展开全文；Zone 2 默认折叠、可查看；压缩边界在时间线可见 |
...
| `get_output` / `write_to_process` / `kill_shell` | 新增 | 读并行，写与杀独占 | 后台 shell 与输出句柄 |
...
| `dispatch` / `wait` / `merge` | 新增 | `dispatch` 并行、`wait` 独占当前步、`merge` 串行 | 把任务交给团队中的一个角色（异步）、等待、把子 worktree 三方合并回来（第 5.7、9.2 节） |
...
`dispatch(role, task, { scope?: paths })` 把一个任务交给第 9.2.2 节角色目录中的一个成员，**异步**生成子会话 worker，
立即返回子 id；父继续工作。系统提示把角色呈现为团队成员而非工具（第 9.2.4 节）。每个角色有自己的结果 schema：
`review` 返回带严重度的发现列表，`check` 返回通过 / 失败与证据，实现类角色返回改动文件、结论、未解决项、置信度。
中间过程留在子 agent 的上下文里，不进入主上下文。

`wait(ids?, timeout_ms?)` 等待其中任一完成或超时，返回已完成者的结果与其余的极简状态；`timeout_ms` 的默认值由 harness
按 provider 缓存 TTL 计算（第 9.2.6 节），模型通常不传。`get_output(child_id)` 读子的完整轨迹，`kill(child_id)` 终止。
`merge(child_id)` 把写者子 agent 的 worktree 变更三方合并到父工作树，冲突时返回冲突文件列表、标记留在文件中，父用
...
| 原生权限门控 | `tool_call` 门控交还给会话中任何强制型 Pi 扩展；关闭时 UI 显式警告"无 Piarium 权限门控" |
...
- **A. 尾部追加（几乎免费）**：Zone 2 消息、模型调用 `recall` / `related` 得到的 tool_result、上一轮工具结果。
  90% 的"无感"发生在这里。
...
不变（第 5.10 节）。子 agent 在 Fleet 中是卡片，在父的时间线上是一张折叠的子卡片带摘要与轨迹句柄。
...
共享工作区）；**权限**（继承父策略；只读角色由原生门控强制而非提示词；后台子 agent 只能用预批准工具）；**深度**（默认
1，子不再生子）；**并发**（默认 12，可配置；到上限的派发排队）；每子 agent 的步数与 token 上限可配置，超出返回部分
结果并注明。父会话结束时子全部终止。
...
- 子 agent 启动时拿到：任务说明、父的 `plan` 与相关块的**快照**（标注"这是快照，父可能已前进"）、host 事实。不拿父的
  完整对话——对应"agent 误以为与子共享状态"。
- 子的完整轨迹对父**可读**（`get_output(child_id)` 走句柄），按需拉，不默认灌进父上下文——Walden 的"共享完整轨迹"
  原则的按需版本。
- 子完成时父收到结构化结果 + 轨迹句柄，不收散文。
...
恢复日志把合并与后续的冲突处理记为父回合的普通变更集（逐路径 before / after），回滚父的这一轮能撤掉。合并成功后
worktree 删除；合并失败或用户设置"保留供检查"时保留。Git 面板显示每个子的 worktree 与差异；用户可选"合并前审阅"，
默认关——自主流程不停顿。
...
唤醒同时完成三件事：刷新缓存、让父看到进度、**由 harness 标出无活动的子**（默认 5 分钟无工具活动）供父终止重派。
算账：30 分钟委派、5 分钟 TTL，唤醒 7 次 ≈ 0.7× 上下文，低于一次冷 miss 的 1.0×，且多了监督；1 小时 TTL 下压倒性
划算。提示词只需一句："若无完成且无异常，再次 `wait`"。
...
本文档第 5–9 节即其规格。工作区形态：仓库；验证器：编辑后诊断、可选测试门、review 传感器；权限默认由原生权限
管理（第 9.1.2 节）给出：只读工具放行，写与 shell 按用户策略。
