...
- *词项分组*：同一对象的变体（完整字面量与其驼峰拆分）用于扩大匹配，但拆分不单独投票。不同对象之间的共同命中才增加覆盖。
  ...
  这张表写进 `details.distinctiveness`，状态对下一轮观察可见。它还不是比较器里的主排序键——加权覆盖进主比较是下一检查点。
...
- *物化*：先用便宜信息排候选，按需读取当前来源正文。与问题主要对象直接对应的精确定义或连接线索，在既有
  ...
  `limit` 是上限不是必填数：定位题找到可用注册片段可早停；「所有注册端」不能找到一个就宣称搜全；因直接线索已验证而未启动的泛词记
...
- *fuse*：任务匹配决定优先级，词法与文件角色只在同级候选间比较（D-145）。层次是：当前正文中的直接关系证据
  （`register("explore.search", …)`）强于完整对象的一般提及，强于名字含该对象的其他定义，强于同时含若干散文词的命中。
  不再把原始组分与 RRF 混加成一个无语义边界的大分数；词组内不再按路径字母序生成名次，路径序只作同分稳定 tie-break。
  文件角色按问题决定（实现/注册优先源码，设计/取舍优先文档，依赖/许可/变更历史才轮到 lockfile），不用结构 provider 状态或
  「图里没有符号」当相关性惩罚（D-148）。D-090「测试路径不默认降权」落实为条件式：问题明确查生产实现/注册时，对象与关系匹配
  相当的候选中优先非测试；查测试时反过来；无法判断保持中性。命中分类只打在已经物化的窗口上（D-095）。
