...
Status: design accepted; code profile v1 and Rust system-kernel Stage R delivered — per-capability state is in agent-harness-status.md, not here
...
steering、用户计划编辑和子线程返回沿现有 keeper 入口加速（D-238）。steering 在 Pi 接受后投递正文；计划只观察用户 UI 已成功
持久化的版本，不观察 agent/keeper 自己写块。子返回由 Registry 在本次 Run 报告持久化后发出，材料绑定 Run 身份与真实结局，
不把上一轮残留报告当作新结果。嵌套返回的接收者是父 Thread 当前绑定的 Pi session，知识仍属 owning workspace；父会话关闭后不
为记忆维护重开。事件正文按观察材料编码，进入同一个在飞/冷却队列；重复投递不额外调用模型，off 为零调用。
