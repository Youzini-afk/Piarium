...
来源、版本、草稿与输出句柄沿既有实际 authority；现行代码和新设计的差距只记在 status，不能把模型槽位或计划当作已经接线。
...
| `explore` 与模型选择 | 工具本身有独立开关；models.explore 服务局部语义决策，清空时无该模型调用；当前消费者待 3.15D 接线。嵌入/重排绑定分别见第 8.5 节 |
...
**LLM 做局部语义决策（D-174/D-175，当前待接）。** 使用已有 `models.explore` 槽位，由 pi-host 的 ModelRuntime 发起短调用，
...
结果。取消/超时应到达本轮来源与读取任务，迟到结果不能复活请求；取消查询不取消其他读者共享的索引建设。具体工作预算按实际
后端能力配置，不在设计中猜新的秒数、文件数、轮数或并发硬上限。
...
| 槽位 | 服务的能力 | 默认 | 未配置时 |
| --- | --- | --- | --- |
| `models.explore` | 查询理解/搜索表达与候选相关性/选段（3.15D，消费者待接） | 未配置 | 使用算法与可用向量，明确模型未参与，不回退主模型 |
...
缓存与索引；模型能力和实际工作负载分别验证。LLM 查询理解/搜索表达与候选判断是 3.15D 的当前交付项，使用已有会话
