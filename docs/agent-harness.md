...
Status: design accepted; D-284 background compaction is pending implementation; delivery facts are in agent-harness-status.md
...
| 模型槽位 | **每个用模型的能力一个独立槽位**（explore / retrievalAgent / quickImplement / hardImplement / frontend / review / check / reader / suggestions / permissionJudge），用户填、预设只填表；仅 hardImplement 与 review 默认主模型，其余未配不调用主模型。续接摘要沿活动会话的 ModelRuntime 与请求配置派生，是内置例外；不新增摘要槽位、凭据栈或辅助费用看板（D-284） |
...
只有适用任务或条目修订改变、或其原呈现已退出活跃窗口时才再次出现。UI 可持续展示最新状态，模型不需要每轮接收整块面板。
...
后台摘要准备（第 8.4 节）与阅读模型（第 5.8 节）由 harness 调用；摘要是单次模型请求，不创建子线程。`review` 角色
...
- 子 agent 启动时拿到任务说明、按创建选择携带的父 plan/用户笔记快照，以及相关 Host 事实；快照注明来源与时点，空和不可用分开。
  不复制父完整对话，不为派发临时生成 memory blocks；检索角色继续遵守 `carryBlocks: false`。
- 子记录对父**可读但分层**（`read_thread(id, what)`，第 9.3.6 节）：先状态与已有报告，需要时读计划/笔记与转录切片。
  未形成报告就明示，不能把最后一次工具调用或片段改名为完整结论。按需读取，不默认灌进父上下文。
...
**等待不触发整理。** 父等待期间已有摘要准备可以完成；空闲和缓存 TTL 不启动新任务。子返回作为新材料进入尾部，下一请求
按真实容量检查。持续新增内容不使固定前缀候选失效，线程生命周期也不依赖摘要或缓存保活。
...
自动开跑 + 父订阅完成事件"。开线带走任务简报与按选择携带的父计划/用户笔记快照，注明来源时点；不复制父完整历史，
不为开线运行 keeper。人开的线程可以选择不携带，相关设置文案不再暗示后台 progress/decisions 一定存在。
...
打开。实现线两边都能开，讨论线以人开为主。是否携带父计划/笔记是冻结在 `ThreadLaunchManifest.carryBlocks` 的创建选择，重启恢复
不得把“不携带”改回默认值。
...
报告**。相对简报的偏离来自已有完成报告的明确字段；未提供则明示未报告，不从取消 keeper 后消失的 decisions 块推断，也不
额外调用模型补齐。子线程
**没有读父会话的工具**，简报就是契约。
