...
当前待处理请求和活动任务引用重新投递一次。用户 UI 的观察仍独立，Zone 2 不再每轮复制所有线程快照或 keeper 块。
...
Host 从现有事件维护投影，模型在每次实际请求前获得首次短表或变化行，覆盖同回合工具续接与等待返回。
不能只依赖初始化/`before_agent_start`，也不逐 token 更新输入；历史前缀保持不动，变化追加在 Zone 2。
观察确认只覆盖实际送入模型的内容，UI 游标独立；压缩/fresh 后按原文收据补必要当前状态，不重播全部历史。
...
7D 的目标包括本机和受管远程实验、真实进程/job 控制、多机器资源与原生集群适配。
Agent Run 与计算 attempt 独立；SSH 连接状态不冒充作业状态，资源请求不冒充 allocation。
Host 编排、Rust 执行核心与原生 scheduler 分别承担自己的权威；可选资源协调 Thread 讨论取舍，但不能绕过资源确认。
这些仍待实施，详见 [实验执行设计](research-cluster-design.md#6-实验执行与资源协作) 和 plan 7C–7F。
