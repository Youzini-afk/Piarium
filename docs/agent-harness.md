...
其中允许自然语言报告、将 `submit_facts` 作为按需结构化交付已进入生产；`research_search` 提供受 Host 授权的论文发现/详情，关系和结构阅读仍在后续 L 阶段。
...
| `retrieval` 角色 | 需要较长阅读与语义判断的开放事实问题、跨文件机制追踪 | Host 校验后的事实报告：问题与范围、source-checked 事实、路径修订/行范围或带 receipt 的 URL、耐久证据、unknowns；判断和改变条件按需由主线综合，不是每轮交付字段。Host 不证明 claim 为真 |
