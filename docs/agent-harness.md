...
- *seed*：主 agent 显式给出的 `anchors`（已知符号、方法名、错误文本、路径片段）是最强种子，优先取得候选、有独立于泛词的预算、
  按字面匹配，但不是硬过滤——猜错的同义词只是候选，不能排除正确实现（D-090）。其次是问题里的标识符（保留语言允许的 Unicode 字母、
  组合字符、单字符及 `_`/`$` 等形式）、引号字面量、路径片段、错误信息；连续中文问题用平台 `Intl.Segmenter` 增加词项，同时保留
  可能是合法代码标识符的原串——**这是分词，不是跨语言桥接**："这个配置在哪里解析"不会凭空产生 `resolveHarnessSettings`，词汇桥接
  只能来自 anchors、仓库已有中英文文档/注释、可用语义索引或已配置的意图转换；栈帧 `file:line` 解析；上下文种子由 Coordinator 提供
  （主上下文文件、会话触碰、上一次结果）与 UI 提示（若有）。问题类型分类同时支持中英文线索（在哪 / 哪里 / where；怎么 / 如何 /
  how；改了 / 影响 / what if；为什么 / 报错 / why / 有栈）。
- *词项分组*：同一概念的变体（原始标识符与其驼峰拆分、字面量、路径线索、中文分词）用于扩大匹配；不同概念之间的共同命中才增加
  相关性。摊平后每个变体独立投票会让拆得更碎的名字获得更多权重，让高频碎词压过精确实现。精确符号与引号字面量保留高辨识度；
  单字符标识符优先在已知局部作用域或符号信息里查，不与长方法名用同一全仓策略（D-090）。
...
- *fuse*：用 **RRF** 按各路名次融合，不生硬相加不同量纲的分数；先保留来源身份与确定性的并列顺序，相关来源不冒充独立证据。
  ...
  与函数正文不按同一方式计分——命中落在哪类节点由结构来源 provider 给出（D-091 的命中分类），rg 自己只知道"这一行有这个词"。
  RRF 分差不是答案置信度。没有目标、来源不可用、版本冲突等可观察缺口可提示补查；“缺少解释所需配置”
  等语义判断仍是待验证推断。
...
- *slice*：切片单位是**与当前问题相关、尽可能自包含的原文单元**，不固定为完整符号：小函数全文；大函数返回签名、命中所在语法块、
  必要外围条件，明确标记省略区间，并保留完整符号的读取入口。不声称自动选出的片段语义完备——比固定 ±3 行更懂结构，比整个大函数
  ...
  缺结构来源退回行窗口并说明来源状态。tree-sitter 是已决定的第二个 provider（D-091）：web-tree-sitter 在 Host 内、语法包 = 语法
  wasm + Piarium 查询、语言身份复用 `languageIdForPath`、解析结果按内容哈希缓存；除切片外它还承担命中分类、连接边形状识别、冷仓库
  符号目录与 `imports` 边。引入顺序见 plan 3.11——接口与 LSP 实现先行，TS/TSX 首刀，引入前量一次冷启动作为对照基线。**不**照搬 OCE 的
...
- *pack*：内部尽量找全可能性，外部只给能支撑当前判断的材料。目标与问题所需支撑组成 bundle，支撑允许为空；片段之间要互补——先比较
  ...
  N 块"的普遍硬限制。歧义时返回区分依据与各自支撑原文，不急着请 judge 十选一。打包保留关系，省略支撑要注明；不预置
  co-change/test/reference 的普遍删除顺序，不让平铺覆盖选择器拆散必要组合。`explore` **在进入通用工具结果之前**按实际可用字节预算
  完成打包，装不下时说明哪个支撑未展开并保留可读引用，不让不理解关系的最后截断器决定什么证据消失；细粒度 provenance 放 `details`，
  模型正文以代码与可行动缺口为主。Host 计 UTF-8 字节，Coordinator 对最终请求另做 token 计量或带标签的估算（D-090）。
