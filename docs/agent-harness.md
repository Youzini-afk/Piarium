...
| 检索 | 只需精确匹配用 grep；需要定位后顺便理解上下文用 explore（接受主 agent 的已知锚点，确定性召回、词项分组、按需物化、结构切片与互补打包，替主 agent 做掉机械跳转）；跨文件推理交 retrieval 角色。不是逐级失败才升级的阶梯。explore 接通即注册，可用来源逐步扩展；intent/judge/查询修复按已配置 explore 槽位和"当前缺的那一步"选一种运行，无槽位走纯算法；不等完整图、向量或独立评测（D-078/D-090） |
...
**输出压缩：按命令分派的规则解析，不是字节切（3.17，D-160）。** 上面的"首部 + 尾部"是已交付形状，它对内容一无所知：
50 KB 的测试输出，失败块在中间就得再拉一次。目标形状是 AFT 已经验证过的五层：**命令专用解析器**（`vitest` / `jest`、`tsc`、
`eslint` / `biome`、`git`、`cargo`、`pytest` 等，按命令 token 匹配，知道各自输出长什么样，精确抽出统计行与每个失败块）→
...
全文照旧进 OutputStore，句柄不变。**不用小模型做总结替代**：结构化输出本来就有结构，解析器几微秒精确抽出、不会漏；模型总结
漏掉一个失败是静默的，agent 以为测试全过——头尾切至少诚实，agent 知道中间被切了。模型总结**可以作为附加**放在非结构化输出上
（"这是头、这是尾、这是模型认为重要的几行、全文在句柄"），减少拉取的同时不让 agent 误以为看到了全部。先做 Piarium 自己
天天跑的四个：vitest、tsc、eslint、git。
...
**`explore(question, anchors?, paths?, limit?)` 是正式默认检索能力**，规格与依赖统一见第 6.1 节。它基于主 agent 当前的问题与
已知线索，用本地检索和结构追踪取得一小组可直接判断的代码原文，尽量消除机械性的搜索—阅读往返，保留尚未解决的语义问题（D-090）。
`question` 说想知道什么，`anchors` 说已经知道什么（符号、方法名、错误文本、路径片段）；锚点优先取得候选但不是硬过滤。返回按当前
来源重读、带版本和关系的代码单元；不承诺固定时延，也不声称结构查询能替代所有跨文件推理。纯算法路径先行，意图扩展、候选裁决和
查询修复按"当前缺的那一步"接入；没有 models.explore 就没有模型调用，永不回退主模型。当前磁盘与固定草稿正文、真实 OutputStore
句柄和生产调用已接线并默认注册；候选获取与物化修正、`anchors`、结构切片与可选模型增强按 plan 3.2 顺序推进。
...
对代码，词法、路径和符号是可靠的基础入口：代码检索的查询多为精确的符号名、字面量、错误信息，Claude Code 与 Cognition 的实践
都以此为主路径。语义召回用于弥补词汇不一致，不承担所有检索任务，也不是基础可用性的前提；它在 Piarium 要回答的问题是"有没有找回
词法与结构路径没找到的有用入口"，不是外部排行榜上的总分（D-090）。因此三层各有归属，知识库不参与第一层。

**检索范围是一等参数，工作区只是范围的一种**（D-162）。第 5–9 节按 `code` profile 写，工作区 = 仓库，检索、知识库、Documents
鉴权、符号图全按 `{hostId}/{workspaceId}` 组织——对代码这是对的。但办公与知识工作的东西本来就不在一个目录里：合同在 Documents、
报表在桌面、笔记在另一处、参考资料在 Downloads，还有一半不是文件。对此的回答**不是索引整个电脑**：一台工作机用户目录里
二十万个文件绝大部分是缓存、依赖、系统残留，给垃圾建语义索引没有价值；范围本身就是相关性的第一道过滤，"找那份合同"在
工作区里清晰、在全盘上命中每份模板与每个 README；用户也不要"它什么都知道"的系统（Recall 被撤回重做）。回答是**分层**，
每层范围与成本不同，与 Spotlight / Windows Search / Everything 二十年的结构一致：

| 层 | 范围 | 内容 | 成本 |
| --- | --- | --- | --- |
| 第零层 | 整个用户目录 | 文件名与元数据 | 近乎免费（Everything 直接读文件表，全盘瞬间建好、常驻几十 MB、实时更新） |
| 第一层 | 用户指定的根（Documents / Desktop / Projects / Notes，或排除缓存与系统目录的默认） | 全文词法 | 后台建，靠 OS 文件监听（USN / FSEvents / inotify）增量 |
| 第二层 | **跟着注意力走，不跟着磁盘走** | 语义 | 只对：当前工作区；最近碰过的文件（工作集，办公场景时间局部性极强）；用户钉住的跨目录集合；被第一层查询命中过的文件，命中时才嵌入 |

第二层大小跟用户实际在做的事成正比，不跟硬盘成正比。一次"快速查询"是逐层落：文件名命中就给；没有则全文搜指定根；还没有
（记不清叫什么只记得"讲收入那个"）才在工作集里语义找；找到后 agent 读它。没有一步需要全盘索引。

落到实现：`explore` 与索引的范围键从 `workspaceId` 泛化为 `scope`——`workspace`（`code` 默认）/ `roots: [...]` /
`working-set` / `collection: X` / 将来的 `user`——用户级索引与工作区索引并列存放、同一套存储与身份模型，只是范围键不同
（第 7.1 节）。这对代码也有用："我在另一个仓库里怎么实现的 X"现在答不了。非文件来源（邮件、日历、聊天）仍在范围之外
（第 10.4 节），但结构上与 Spotlight importer / Windows IFilter 同形：连接器把外部东西变成带身份、内容、修订的文档进同一个索引，
`scope` 多一种——所以**文档身份不绑文件路径**，这是 3.16 从第一片起就要守的接缝（第 10.5 节"接缝先于领域"）。
第零、一层与用户级第二层等 `knowledge-work-in-files` 有真实消费者时再做；3.16 只须不把门堵死。

"不是前提"是**不依赖**，不是**不做**。四轮观察（D-142–D-157）已经把词法加结构加图这条路能修的都修到了：目标文件进候选池、
被读到、被切成窗口、进入可见正文，每一级都有量具。剩下的失败类是词法永远修不了的——问题的词和代码的词对不上（"stale draft"
对 `superseded`，答案行 `links.push({ kind: classified })` 不含问题里任何一个词）。语义来源（嵌入召回、重排、可选的查询扩展）
因此**排期为 plan 3.16**，与 3.15 的管线重构并行，不再写"等观察决定"（D-158）。AFT（`cortexkit/aft`）是同一问题域的成熟
实现，已把本地 MiniLM 嵌入、RRF 混合、按命令分派的输出压缩做成常规模块；它是参照，不是依赖。

### 6.1 第一层：文件里有什么

三级各有用途，主 agent 按当前缺什么选择，不是必须逐级失败才升级的阶梯：只需精确匹配用 `grep`；需要定位后顺便理解相关上下文用
`explore`，即使已经知道确切符号；需要跨文件推理才交 `retrieval` 角色（D-090）。

**第一级：`grep` 工具。** rg、固定草稿叠加与分组排序（第 5.3 节）。知道确切符号或字面量时一次调用就该给出第一屏正确的
文件。

**第二级：`explore` 工具——替主 agent 做掉机械跳转（正式实施，D-078；策略 D-090）。**
目标：基于主 agent 当前的问题与已知线索，用本地检索和结构追踪取得一小组可直接判断的代码原文；尽量消除机械性的搜索—阅读往返，
保留尚未解决的语义问题。优化对象是主 agent 获得足够证据、能推进下一步的时间，不是返回一批相关文件的时间。边界一条：**机械依赖
由 Host 接着走**（找到名字才能查定义、找到定义再找注册点、读到参数再读实现），**语义假设留给主 agent**（"问题可能不是权限而是
缓存键"）。内部存在串行依赖不等于需要串行模型调用。

一次调用先识别主 agent 此刻缺的是哪一种，再选能力与返回形状，不把四种都塞进同一条管线：

| 主 agent 此刻缺什么 | 需要的能力 | 合适的返回 |
| --- | --- | --- |
| 知道名字，不知道位置 | 符号、路径、字面量定位 | 定义或关键出现点 |
| 知道概念，不知道实现用什么词 | 概念到仓库词汇的桥接 | 少量可信入口，必要时保留候选分歧 |
| 找到了文件，不知道关键部分在哪 | 文件内定位与结构化阅读 | 相关语法块、签名、必要条件 |
| 找到了入口，不知道它如何连到另一处 | 有方向的关系追踪 | 连接两端的原文及关系依据 |

三条执行原则：**有地址就不先全仓搜索**——路径、栈帧、已知符号是可直接进入的入口；**有机械下一跳就由 Host 接着走**——不让模型
再花一次调用表达"请读这个定义"；**已取得可用材料就不因一个可选来源没返回而压住全部结果**——是否继续等由当前缺口与工作预算决定，
不由 `Promise.all` 决定。

确定性召回、结构切片、版本化正文和互补打包作为默认实现，接通后投入使用。工程测试保证来源、版本、路径与输出正确；实际任务用于
优化召回和延迟，不作为批准这个方向的前提。依赖、已观察缺陷与实施顺序见 plan 3.2。
当前纵切已接通实际 rg、多路径与 actor scope、Documents 版本化连续正文、发起窗口的固定 dirty snapshot 和会话 OutputStore。
`explore`、`grep` 与同名 `read`/`find`/`ls` 已读取该固定来源；后续编辑不改变本轮结果，来源过期不回退磁盘。固定草稿是**一轮输入**而不是
永久权威：Piarium 观察到某路径被写入后，写入之前捕获的快照就不再用草稿回答该路径，读、搜索、枚举、导航与子线程基线一起回到磁盘，
因此 agent 总能读回自己刚写的内容（D-088）。**写入侧同源**：读走草稿而写落磁盘时，原生 `write`/`edit`/`apply_patch` 在写之前被拦住
并给出可执行原因（请用户先保存该文件；放弃不解除拒绝，因为本轮仍按捕获的草稿回答），而不是把用户未保存的改动静默落盘——
这与第 9.2.5b 节对子结果的零写入处理是同一条原则（D-089）。结构展开按第 6.4 节的
...
**目标形态（3.15 起，D-159）。** 3.13/3.14 之后的验收暴露出一个模式：系统在让打分器代替自己表达"什么才算回答了问题"。
同一个旋钮（文件角色）三轮调整，`windowScore` 累积到十一项手调系数，量具每轮被抓到一次。设计复查把七条张力归成三个缺口，
它们互不替代：**答案要求没有贯穿决策**（什么是直接答案、支撑、探索线索，被混进分数和来源等级）；**选择与呈现没有共同决策**
（选中了一个单元，却没有保证模型能看到选择它的理由——大单元正文在打包之前就按命中行 ±3 不可逆地裁掉了）；**引擎没有统一表达
处理结果**（文件、窗口、可见片段被当成同一种候选，量具只能从几个不一致的数组反推）。目标形态因此是五段，每段一个职责：

```text
答案要求（AnswerRequest）  对象、所求关系、范围、阅读形式；区分用户明说 / 系统推断 / 未知；无对象问句也有部分规格
        ↓
取证                       对象检索、词法候选、带到达理由的图展开、语义召回；在当前来源中读取和核验
        ↓
证据单元                   直接证据 / 支撑 / 开放候选；当前源码范围 + 可选阅读深度（定位片段 / 机制片段 / 完整单元）及各自字节成本
        ↓
一次联合选择               先构造满足要求的主证据，再按相关性填支撑；同时遵守条数与实际字节预算；不能补偿的用规则，确实不确定的才用评分
        ↓
正文与呈现记录             模型实际看到了什么、哪些要求仍缺证据、哪些材料为什么未展示；量具读这份记录，不读裁剪前的数组
```

三条不许倒退的规则：**不能补偿的事不进评分**——明确要非测试实现却给夹具、问注册点却只给请求端、证据已省略却宣称已展示，
这些由要求与证据检查处理，多几个关键词抵消不了；**选择的单位是带呈现方案的证据单元**，不是已裁好的窗口，先决定要交付什么证据、
再决定展开多少；**内部知道全部状态不等于把全部状态发给模型**——逐候选追踪只在观察模式输出（D-157 量过：一次内容词问句的
`details.windows` 达 482 窗口 / 185 KB，可见预算是 24 KiB）。

证据单元上三个字段**分开**，不再揉进一个等级枚举（D-161）：`arrival`——怎么找到的（rg 命中 / 图边及其到达理由 / 语义块及其
查询变体与排名）；`assessment`——对答案要求核验到了什么（当前正文存在 register 调用且参数等于目标值 / 只是含名字 / 未核验）；
`purpose`——在这份输出里当主证据、支撑还是备选。三者独立成立，所以一条语义命中可以 `arrival = semantic`、
`assessment = 已确认`、`purpose = primary`，**不必先经过词法命中**；相似度 0.83 是特定模型下的相关性数据，不是 83% 正确概率，
不进 `assessment`。

取证阶段的产物是**召回线索**（来源、查询变体、排名、被索引正文的身份、父单元身份、定位材料），经当前来源读取与重定位才成为
证据单元。定位材料不只是命中行：rg 给原始命中行，图给符号或连接值随后在当前正文核验，语义块给索引时的父单元身份与块哈希/范围，
文件/目录摘要给导航目标。所以证据单元的入口是 `focusRanges`（当前正文里的重点范围），词法命中行只是它的一种来源；`sliceStructureWindows` 没有 `focusRanges` 才返回空（D-165）。
`sliceSymbol` 已拆成提出方案（范围 + 字节成本）和按选定方案渲染；本刀只交付一种方案（签名 + 焦点块 + 省略标记）。
证据单元已带 `arrival` / `assessment` / `purpose`，不再使用 `EvidenceGrade`。`hasBothConnectsEnds` 要求同一连接值的注册端与请求端。
`windowScore` 仍在，但已去掉 `GRADE_RANK * 8`；评估只作打包首槽分区，不回到分数。索引正文与当前正文一致则按块哈希采用对应范围；
文件其他部分变了则按父符号身份与块内容重定位；对应块已实质变化则重切当前单元、重算相关性，不沿用旧范围，也不因找不到查询词
而丢掉它。同一函数被 rg、图、向量都找到，合并的是到达理由，不是复制三份证据；大函数的不同子块各保留重点范围，不按路径去重到
只剩一处。

**读取调度与最终选择是两个粒度、两套规则。** 读取前比较文件：各来源先生成去重的文件排名（词法一路用 D-153/D-154 的加权覆盖，
语义一路取该文件最有希望的块、多个查询变体取 best-rank 归并），再用 RRF 融合决定先读哪些当前正文——
\(R(f)=\sum_{s}\frac{1}{k+r_s(f)}\)，某来源没有该文件就没有那一项，\(k\) 取稳定初值不按十问调。**这不是回到 D-145 拆掉的
东西**：那次拆的是原始组权重与 RRF 分数混加、路径字母序当名次；这里 RRF 只吃排名、只在文件级、只管读取调度，最终选择它不参与。
答案要求指定的直接取证（已知对象的精确定义、所求注册关系）单独保留优先读取机会，不算成"强第三票"。读取后比较单元与呈现方案：
文件的词法得分不广播给它的每个语义子块，某个块的高相似度也不让整个文件的所有窗口继承。

下面"管线与真实依赖"到"pack"各条记录的是**已交付**的形状，3.15 逐步把它们收敛到上面五段；哪一段已交付见 status。

**管线与真实依赖（已交付形状）。** 各阶段不是全并行，join 点如下（→ 表示依赖）：
...
- *seed*：先提取并保护对象，剩余才是问句（D-144）。完整技术字面量（`explore.search`、`tree-sitter`、`document.writeGuard`）、
  显式 `anchors`、引号/反引号内容是对象；拆分形式只作同组回退变体，不再单独投票。普通问句词默认是自然语言内容词，可做弱词法线索，
  不驱动图查询、不拿 identifier 权重。`on(...)` / `emitter.on` /「名为 on」里的 `on` 是对象；单独输入 `on` 也不能被停用词吞成空查询。
  关系词是受控小表，只覆盖图里实际有的边：注册/连接、import、定义；其余一律未知，不推断，不许长成 NLU。中文同样分对象/关系/内容词，
  整句分词的每个片段不自动具有代码标识符强度。主 agent 显式 `anchors` 仍是最强种子，按字面匹配，不是硬过滤（D-090）。
  连续中文问题用平台 `Intl.Segmenter` 增加内容词——**这是分词，不是跨语言桥接**："这个配置在哪里解析"不会凭空产生
  `resolveHarnessSettings`。问题本身提供的对象先查图与精确字面量；读后新发现的对象再按 D-137 展开（D-146 修订）。
- *词项分组*：同一对象的变体（完整字面量与其驼峰拆分）用于扩大匹配，但拆分不单独投票。不同对象之间的共同命中才增加覆盖。
  精确符号与引号字面量保留高辨识度；内容词只在直接线索未验证时启动（D-144 / D-147）。
  每次调用在候选池内算一张**查询内区分度**表（D-153）：\(N\) 与 \(df(g)\) 都是本池去重文件数，不是全仓库 IDF，也不是
  rg 命中行数。同一概念的变体共一行权重；同一文件反复命中同一词不另加 \(df\)。覆盖至少三态——不同匹配文件的覆盖完整 /
  只知道下界 / 无法判断——跟着权重走：完整且稀有的词拿 \(\ln\frac{N+1}{df+1}\) 额外区分度，覆盖不完整的词只保留普通匹配贡献
  \(1\)，不拿未经证明的稀有奖励。`partial` 里的 per-file 命中帽不影响去重文件数；`filesDropped` 与不完整后端扫描才把覆盖标成下界。
  这张表写进 `details.distinctiveness`，状态对下一轮观察可见。已核验且符合所求关系的直接证据仍优先（tier 0）。其余候选比较
  加权覆盖 \(L(f)=\sum_g w(g)\,m(g,f)\)：同组不重复累加，完整原词 \(m=1\)、回退变体 \(m=0.5\)（D-154）。
  文件角色是有界偏好，只在加权覆盖之后作区分，不再先于内容证据。明确目标仍有效（查依赖锁定版本优先 lockfile、查设计取舍优先设计文档），
  但弱相关源码不能再凭角色墙压过强相关的清单或配置。区分度在 \(L(f)\) 里，不放在比较器末位 tie-break。
  `explore` 与 `related` 共用同一份 `classifyFileRole` 与依据（文件名模式 / 项目声明 / 未知）；角色是查询期装饰，
  不写入符号图（D-164）。项目声明（`package.json`、`tsconfig*.json` 等）仍是 `other`，只把依据标成 `project-declaration`，
  不新增会改 `fileRoleFit` 的角色。
  物化之后用同一张表重算局部证据：在正文里核对全部有效词组（原词才能新开窗口，回退变体只更新文件级覆盖），
  相距超过一个小容器的同组命中各算一簇，由结构切成可读块后再评分。打包看三段——这段自身的加权覆盖、
  相对已选片段（含同一文件已选窗口）新增的词组价值、正文成本——不再把换文件当奖励，也不再让
  `sameFile * 40` 无条件挡住同文件互补块。  入口窗口已经占住的词组，同一文件里尚未选中的函数/方法
  仍算本地互补（机制片段不必再重复对象）。完整对象档只给对象或锚点，普通内容词原词命中是 lexical（D-155）。
  多词邻近性本刀不做。
- *候选预算 ≠ 输出预算*：`limit` 只管最终摘录条数；候选获取用独立的工作预算，兼容的词项合并为批量扫描、重叠搜索根合并，强锚点
  先取得候选，泛词作扩展不淹没精确入口；达到扫描预算时明确返回未搜完，由调用目标决定是否扩大。`explore` 不继承 `grep` 的展示排序
  （命中数 + 测试路径惩罚 + 深度惩罚）作为相关性定义——寻找实现时偏源码、寻找如何验证时测试正是目标，由问题决定（D-090）。
  **预算按文件铺开，不按命中数深度优先吃满**：排名只需每文件每词组一条命中，所以先每个匹配文件取一条、再在余额内逐轮加深；
  否则通用词项会用前几个文件的重复命中吃掉预算，把后面的文件整体挡在排序之外——这与被替换掉的输出 `limit` 是同一形状（D-092）。
  只有匹配文件数本身超过预算才丢文件，丢弃数单独上报、不与命中级裁剪混为一谈；该数是下界而非总数——各词项与各搜索根匹配的文件集
...
- *fan-out*：rg（磁盘）经 search-service 一条路径，传入 actor 与本轮 inputContext，脏路径由后端在计数前排除、草稿命中由同一
  ...
  **反向 import**（谁 import 了这个文件，不需要查询词）。其中只有连线补全是真正的新召回：注册端或请求端可能被 rg
  候选预算裁掉。**只有对象级词项**驱动 `searchDefinitions` 和 `findLinks`；问句内容词不查图（D-144）。
  问题里已经给出的连接值 / 明确符号 / 明确路径在第一次打包之前就查（D-146，修订 D-137）；读后新发现的字面量仍按 D-137 展开。
  `connects` 是更强的待验证线索，`associates` 只是提及候选，二者不能共享同一直接证据等级（D-147）。
  读后展开受问句对象约束：容器切片让一份注册表窗口含有它注册的**全部**字面量，所以「只展开窗口正文里的字面量」还不够——
  与对象无关的字面量排在本对象之后取连接预算，其另一端只给 `support` 档，不与本对象的另一端同级（D-151）。
  无法判定无关不等于已经证明有关：无对象问句可以按内容词、当前局部证据和可行动缺口做有限展开，
  不把整张注册表的连接都当潜在支撑（D-156）。
  每条图线索带到达理由（D-163）：`object-triggered`（问题对象直接触发定义或连线）、`statement-evidence`
  （当前窗口命中行上的字面量产生的取证需求；无对象问句也可以这样从真正参与解释的语句产生新对象）、
  `same-container`（只是与已选代码同容器）。第三种不拿直接线索待遇：不是 tier 1、不进 `isDirectClue`、
  不拿 `connects-clue` 档、不走图补充物化。连接预算数值未改，不用更小上限代替相关性判断。
  图有独立预算，`filesDropped` 跨来源取最大值（D-092）；定义丢弃按去重路径计数（D-148）。读路径只用已经打开的 store，
  未开 / 空目录 / 查询失败分别报 `unavailable` / `empty` / `failed`，rg 结果照出（D-112 同类）。知识库符号搜索是
  对已打开目录的名字扫描 + 分档计分（精确 / 名字含 / 路径含），不是 AC / BM25。
  **语义召回是第三路路径候选**（3.16，D-158）：对仓库按结构单元切块、嵌入、建索引，查询时嵌入问题取最近邻，产出候选路径与
  候选片段，进同一条"答案要求 → 证据单元 → 联合选择"管线——它不绕过打包，也不替代词法。它唯一擅长、词法唯一做不到的事
  是词汇缺口：问题用的词和代码用的词不同。**嵌入后端按 `models.embedding` 槽位选，配了远程就用远程、没配就本地 MiniLM**
  （第 8.5 节）；远程查询失败或超时回退词法加图，工具不因可选来源挂掉而挂掉。**不设信任门、不设花费守卫**：用户配置远程
  提供商这个动作本身就是同意，成本是用户自己的账，harness 只做合理优化（增量重嵌、查询嵌入缓存、批量）不拿它限制设计（D-158）。
  远程放开的能力——多粒度嵌入（符号 / 文件摘要 / 目录摘要）、更大的重排范围、更强或代码专训的模型、查询扩展——都是本地跑不动
  而远程跑得起的东西，是这个槽位存在的理由。
  **切块**：小单元（正文装得进模型有效输入长度）整块编码；大单元按语法块递归切，父单元名与签名始终随块保留，一个分支仍过长
  就继续分，不按固定字符从头截；缺结构能力时用覆盖全文的有重叠行块并记为 fallback。每个代码区域至少被一个块覆盖；重叠只为
  边界上下文，不重复投票。编码材料是相对路径 + 父单元名与签名 + 相关文档注释 + 当前子块原文，**路径和签名不许挤掉正文**。
  切块按所用 tokenizer 的实际有效长度，不按"几字符约一 token"——同名 MiniLM 在不同运行配方下有效长度不同（模型卡默认 256、
  AFT 覆盖为 512），配方进向量空间身份。**AFT 在这里是反例不是参照**：它的嵌入文本是符号名 + 签名（400 字符）+ 正文前 15 行
  （300 字符）、总长钳 1600，本质是模糊符号名索引，`links.push` 那种在大函数后半段的动作永远编不进去（D-161）。
  **身份分三类**（D-161，比 AFT 的单一指纹细）：向量空间身份（provider / endpoint、模型修订、维度、pooling、归一化、
  query/document 用途约定）变了文档与查询都要重算；索引配方身份（切块器、结构提取器、文本装饰、粒度配置、摘要配方）变了重建
  记录但相同编码文本的向量可复用；查询与重排身份（query recipe、扩展版本、重排器、正文视图）变了只失效查询缓存。换 API key
  ...
  **已交付（3.16 第一片索引侧，D-166–D-169）。** 本地空间身份钉 `provider=local`、`all-MiniLM-L6-v2`、384 维、mean pooling、
  ...
  `unavailable`，不是空成功。
  **已交付（3.16 第一片检索侧，D-170）。** 查询嵌入原问题（本片不扩展），近邻块变成 `arrival = semantic` 的召回线索，
  经当前正文 `relocateSemanticFocus` 进入 `focusRanges`：块哈希一致直接采用范围，否则按父符号与块正文重定位，实质变化
  则重切当前单元，不沿用旧范围，也不因找不到查询词丢掉它。同一函数被 rg / 图 / 向量都找到时合并到达理由。
  `rankCandidates` 在池里出现语义线索时按 `tier → RRF → L(f) → roleFit → 路径` 调度读取；词法排名仍是加权覆盖序，
  语义文件排名取该文件最靠前的块，\(k=60\)，缺来源就缺那一项。无语义线索时比较顺序与 3.14 相同，避免把 RRF 零项当成第三票。
  语义单元经同样的结构核验可以 `purpose = primary`。`details.semantic` 报
  `not-requested / ready / empty / unavailable / failed / stale` 与 `coverage` / `generation` / `spaceId` / `scope`，
  `index.lifecycle` 单独一项。语义来源挂了、没建或建一半，explore 仍返回词法加图。重排、查询扩展、远程嵌入仍待第二到四片。
  **固定草稿与线程分支是 overlay**（Piarium 特有，AFT 没有这个问题）：
  ...
  向量计算可共享，路径与视图归属不能错。
- *物化*：先用便宜信息排候选，按需读取当前来源正文。与问题主要对象直接对应的精确定义或连接线索，在既有
  `maxMaterializeReads` 预算内先读，不被泛词总分挤掉；多个锚点轮流获得机会；锚点错了继续词法路径，不做硬过滤（D-146）。
  补充物化看「是否已读取、是否已有对应当前证据」，不看「是否见过路径」——已在 rg 池但未读的图命中仍要读；已读文件复用本轮快照。
  `limit` 是上限不是必填数：定位题找到可用注册片段可早停；「所有注册端」不能找到一个就宣称搜全；how 问句在摘录包已满时
  仍读到 `maxMaterializeReads`，不把输出名额用尽当成停读理由（D-154，数值未改）。定位题已经拿到直接答案时，已标
  `offTopic` 的连线不再展开、也不再填进正文凑满 `limit`（D-156）。因直接线索已验证而未启动的泛词记
  `skippedQueries.reason=direct-verified`，与读预算用尽的 `not-requested` 分开（D-147）。未读候选不混进 `empty`。
...
- *fuse*：任务匹配决定优先级，词法与文件角色只在同级候选间比较（D-145 / D-154）。层次是：当前正文中的直接关系证据
  （`register("explore.search", …)`）强于完整对象的一般提及，强于名字含该对象的其他定义，强于加权内容覆盖。
  同 tier 之内比较 \(L(f)\)，不再数原始覆盖词数；路径序只作同分稳定 tie-break。
  读取调度在语义一路入池后多一层文件级 RRF（D-170）：只吃各来源的文件排名，\(k=60\)，不管最终打包；一个文件的多个语义块只贡献该文件最好的名次。无语义线索时不走 RRF，避免空融合改变 3.14 顺序。这不是回到 D-145。
  文件角色按问题决定（实现/注册优先源码，设计/取舍优先文档，依赖/许可/变更历史才轮到 lockfile），但是**有界偏好**：
  排在加权覆盖之后，不用结构 provider 状态或「图里没有符号」当相关性惩罚（D-148 的角色表仍在，D-154 去掉它当等级墙）。
  D-090「测试路径不默认降权」落实为条件式：问题明确查生产实现/注册时，对象与关系匹配相当的候选中优先非测试；查测试时反过来；
  无法判断保持中性。命中分类只打在已经物化的窗口上（D-095）。
  分差不是答案置信度。没有目标、来源不可用、版本冲突等可观察缺口可提示补查；不伪造语义充分性判定。
- *连接点*：注册点、协议字面量、配置键、事件名是一等检索连接点——`"explore.search"` 把 pi-host 工具、protocol 契约与 Host
  `router.register` 连成一条链，这条边 LSP 给不出。查询时只从少数能可靠识别的调用/注册形状取证；无法确认的同名字符串标为关联
  候选，不伪装成已解析的运行路径。找到定义再找到它真实被接上的位置，比多返回十个相似定义更有价值（D-090）。
- *slice*：切片单位是**与当前问题相关、尽可能自包含的原文单元**，具体是容器（函数/方法/类/接口/枚举/模块/类型），不是普通值绑定或成员签名（D-098）。小容器全文；大容器返回签名、命中所在语法块、
  必要外围条件，明确标记省略区间，并保留完整符号的读取入口。命中落在 `const needle = 1` 这类普通绑定上时切所属函数/类，至少不差于旧的 ±3 窗口；`const foo = () => {}` / `const C = class {}` 这类绑定的就是定义的情形仍是自己的单元。不声称自动选出的片段语义完备——比固定 ±3 行更懂结构，比整个大函数
  更节制。结构来源是带修订绑定的可插拔 provider：第一个实现是第 6.4 节 agent 视图的 `documentSymbol`（它区分 `range` 与
...
- *pack*：按问题所需证据打包，不是按词组覆盖凑数（D-147）。已核验的直接答案片段先占预算，再是提供新关系/新条件的支撑，
  最后才是尚有用的候选差异。图理由和加分只绑定**当前核验成立的那个窗口**，不是文件；同一事实被图、rg 和语义三条路发现时合并到达理由，不复制三份证据。
  `arrival = semantic` 的单元不必先有词法命中就能在核验后成为 `purpose = primary`（D-170）。
  重复片段（同一 fixture 的第二、三、四段）失去边际价值。`connects` 与 `associates` 文案分开；无结构能力核验关系时仍可返回原文，
  理由降成「完整字面量命中；图曾指向这里」，不宣称已确认注册。主答案必须出现在可见正文，不能只在 `details` 里。
  `explore` **在进入通用工具结果之前**按实际可用字节预算完成打包；细粒度 provenance 放 `details`。Host 计 UTF-8 字节（D-090）。
- *模型增强*：按"当前缺的那一步"选一种，不默认 intent → judge → repair 全跑（策略，不是硬轮数不变量）：没有可匹配的仓库词汇 →
  意图转换生成候选词与检索方向；已搜到材料但目标缺席或线索指向新方向 → 受限查询修复，它必须能提出新词项或入口，只选已有候选 ID
  解决不了候选缺席；候选多、都似成立、阅读成本高 → 候选比较或重排。**重排不必是 LLM**（3.16，D-158）：对已在池里的几十到
  几百个候选片段，用交叉编码器逐对打"与问题多相关"的分。**成功完成重排的候选，用重排结果作为主要相关性顺序**；词法与融合
  排名只用于召回、初筛和失败降级，**不再作为若干奖金叠到重排分上**（D-161）——旧 `windowScore` 整个删掉，不留两套评分并存。
  答案要求检查、角色约束、关系完整性、重复证据、字节预算仍由 Host 做，不是重排器的职责。重排输入是原始问题 + 答案要求的
  必要要求与范围 + 候选的**当前代码正文** + 路径/父单元/签名，不是 `why`、符号名或旧 ±3 窗口；对长单元按**候选呈现方案**评分，
  重排看过完整函数就不能最后只展示签名而沿用"很相关"的结论。同一个重排服务可在读取前对索引里已有的版本化块文本先评一次
  帮助调度（分数只用于调度，最终展示前仍读当前来源），输入身份相同则复用分数。不同 provider / 模型的分数不直接混排，
  没有共同评分的尾部候选保留原检索顺序而不补零。后端按 `models.rerank` 槽位选：远程可重排一两百个候选，本地 CPU 按延迟预算收窄。
  **查询扩展**（`models.explore`）改写出的两三个问题是新的搜索表达，不是新的答案要求：各自嵌入作为语义来源内部的查询变体，
  best-rank 归并；改写里的猜测不成为必须命中的对象、不改生产/测试范围；不进原问题的 D-153 区分度表、不抬词法 \(N\)；
  最终重排仍对原始问题与答案要求判断。judge 在候选物化后只返回候选 ID 与理由，未知 ID 拒绝；补查
  只接受类型化搜索/导航操作并重新授权路径，不接受 shell，不运行用户任务，不拿自由文本总结替代源码。正文标为不可信数据。迟到结果
  是否采用由 Coordinator 决定，调用结局与采用状态分开记录。需要重建因果假设或结合运行结果继续验证时交回主 agent 或 `retrieval`
  角色——不把一段多轮推理藏进 `explore` 再贴"快速"标签。向 UI 流式显示早期命中不等于主模型已能消费结果，不作为等待时间减少的证据。
...
失败、超时、迟到但成功均按实际记录，不增加辅助调用费用统计（D-080）。每个候选有 ID、来源版本、provenance 与关系，授权后才入 OutputStore。需要特别明确：`OutputStore` 句柄是经鉴权的会话局部临时存储（session-local ephemeral storage），用于长输出跨工具调用的安全分页读取，生命周期跟随会话进程与世代，不是持久模型资源或永久知识条目。句柄保存完整打包正文与未展示候选的引用清单，是展开长材料的后备入口，不是每次 explore 的固定下一步；工具文字不得暗示"所有被裁掉的候选都在里面"，除非确实如此（D-090）。
...
| `models.embedding` | explore 语义召回的嵌入（3.16） | **本地 `all-MiniLM-L6-v2`** | 不存在"未配置"：本地是默认值 |
| `models.rerank` | explore 打包前的候选重排（3.16） | **本地交叉编码器，范围按延迟收窄** | 不存在"未配置"：本地是默认值 |

**嵌入与重排槽位和聊天槽位有两点不同**（D-158）。一，它们有本地默认值：聊天模型不能默认在本机跑，所以聊天槽位未配置就退化为
无 LLM 路径；嵌入和重排模型几十兆到几百兆，默认本地跑得动，所以这两个槽位永远有值，配远程只是换成更好的。二，它们不是 LLM，
不受"永不静默回退主模型"约束——本来就没有主模型可回退。远程后端接进**现有 provider 配置**：同一个 provider 下另一种 kind 的
model（`embedding` / `rerank`），复用 `baseUrl` 与凭据，不另起配置文件；嵌入按 OpenAI 兼容 `/v1/embeddings` 接口，重排没有
事实标准（Cohere / Jina / Voyage 各异，本地 TEI 仿 Cohere），需要一层适配器、按需求逐家接。本地模型是一个**包**——ONNX 权重、
tokenizer、配置、pooling/归一化约定、运行配方——内容寻址清单覆盖整套，走 3.11 第 5 步为语法包建好的存储与按需下载；默认小模型
随发行版交付。推理在 Host 进程管理的实例里跑，不进 renderer，实例按绑定复用；嵌入与重排各自"限半核"仍会互相争用，线程池按
Host 实际模型实例协调。接口至少区分 `query` 与 `document` 用途（部分模型两者编码方式不同）、可取消、报实际维度与批次对应；
`knowledge/embedding.ts` 已有七家远程适配器（知识库侧建的，一直占位模式），3.16 是把它的 `embed(texts)` 接口加厚，不是从零建。
**`all-MiniLM-L6-v2` 模型卡标 English**：它作本地默认没问题，但不兑现跨语言——中文原型放进去不会得到中英意图对齐，
零样本分类的跨语言能力要按当前模型如实表达，选多语言远程模型时才启用（D-161 纠正 D-160 的"天然跨语言"）。
