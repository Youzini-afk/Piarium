...
**后续增强（D-315，待实施）。** [Web 与科研检索](web-research-search-design.md) 定义目标驱动的连续搜索、
固定原文视图、材料复用及学术发现/关系/结构阅读；沿现有工具和线程实施，不把普通调用改成必经深度研究。
本节现有 Web 行为与阶段 L 新增能力分开记，实际交付见 status。
...
**Web/科研扩展入口（D-315，待实施）。** 本章现有代码检索继续有效；
...
其中允许自然语言报告、将 `submit_facts` 作为按需结构化交付是阶段 L 的合同调整，当前生产仍使用原事实协议。
...
### 8.4 容量驱动的后台续接压缩（D-284 / D-314）

D-284 的固定候选/容量检查/原文接续与 D-314 的后台压缩 Agent 已交付。D-314 实现
[后台压缩 Agent 与语义续接设计](context-compaction-agent-design.md)：独立内部子进程按需查阅历史、输出与任务记录，
共用首次/更新提示，以近期原文理解工作位置，按语义整理有效用户要求。它取代 8.4.3 原单次无执行器调用，
不改变本节的 Pi 历史权威、候选范围及 B/N 原文保留。
...
#### 8.4.3 后台压缩 worker 与查询执行

D-314 的当前实现：同一固定压缩任务由专用 `compaction` worker 执行——broker 派生独立 pi-host 子进程并
`pinSession` 到父会话承载事件身份；它不是会话 worker、不进父会话请求队列，父 worker 退出时经
session→auxiliary 映射回收。worker 内跑真实 `Agent` loop，`ModelRuntime` 复用同一 agentDir 的凭据与
`ProviderConfigurationManager`，使用冻结的模型与执行配置，不自动换成廉价模型、不降低 thinking。
...
不可用调用不执行动作、不将空结果提交为摘要。工具执行边界由代码实现，不能仅靠提示词声称工具已禁用。
...
上表记录当前预设。D-315 / 阶段 L 将沿同一 retrieval Thread 支持自然语言报告及可选结构化 facts，
并补齐实际通信工具；设计见 [检索线程与报告](web-research-search-design.md#7-检索子线程动态探索与报告)。
实施前 `submit_facts` 的现行交付要求不变；实施时保留来源、scope、Run 和耐久引用的核对，不能只改提示。
