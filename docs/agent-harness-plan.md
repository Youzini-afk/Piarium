...
**下一刀（尚未实施）**：查询内区分度与片段证据贯通。词组权重按去重候选文件数而不是命中行数计算，覆盖不完整的词不得获得未经
证明的稀有奖励且该状态要在 `details` 可见；加权覆盖进入主比较，文件角色退回有界偏好；同一张权重表进入窗口等级与打包，去掉
`windowsFor` 把普通内容词当完整对象的混淆。注意顺序耦合：单修混淆会让带对象的错误窗口赢得更稳，所以加权必须同批落地。
仍不引入 embedding / BM25 / 持久词法索引 / NLU；多词邻近性留到下一次观察再决定。
