...
3. **当前：窗口读取与 explore（3.2）**：自动消息来源、固定 dirty snapshot、draft-aware explore/grep/read/find/ls、线程草稿基线、
   ...
   **3.14 检查点一、二已接**（D-153 / D-154）：查询内词组权重表可见；加权覆盖进入主比较，角色退回有界偏好。
...
`related` 已接线（3.12）。3.13 已让已有精确线索被验证并进入可见正文。3.14 检查点一、二已让查询内区分度可见并进入主比较；局部选择与打包仍待检查点三。仓库级词法索引 / 桥接 / embedding 仍等观察决定，不在本刀。
