...
D-301 的后续 7G 将该环境增量接到每次实际模型请求前，并在其后附完整当前团队快照；两类材料的保留/游标规则分别处理。
本项现有交付不代表 7G 已实现；先完成并验收当前 D-300 执行任务，再接续改造，详见 7.11。
...
2. 本机/专用远程受管机器由执行服务确认并跟踪资源；Slurm 等目标由原生 scheduler 确认，Piarium 不另发一份与它冲突的 allocation。
...
6. 可按需要派一个普通资源协调 Thread，查看相同资源、讨论研究优先级和放置取舍；它的决定通过相同工具落实，不能绕过准入。
   不默认常驻管理模型，不为每次心跳或作业变化调用它。没有该 Agent 时仍可正常运行。

动态矩阵由 Agent 决定探索方向，由程序批量提交、追踪、收集，不为每个 seed 启动一个 Agent。
...
| 原生计算集群 | Slurm 等原生 scheduler | 本地 intent 对应原生 job/allocation，排队、取消、抢占和终态以其事实为准 |

首个现有集群适配目标为 Slurm；Kubernetes Job / Ray Jobs 留作有具体部署需求后的适配，不要求第一轮全做，
也不把安装集群框架设为本机/SSH 使用前提。7D 整体交付包含受管远程和多目标资源纵切，不能以本机 Shell demo 标记完成。
...
先用干净 batch 命令贯通代码/数据计算和矩阵作业；后续持久 Python/Notebook 仅在真实需求下接入。
暂停不承诺释放 GPU，恢复不承诺重建未保存内存；持久 worker 的生命周期与科学判断、调度和 Agent loop 分开。

### 7.8 每次实际模型输入前的现状更新

执行边界：以下初始表/后续增量描述保留为当前 D-300 执行任务的基线。D-301 已接受的后续目标是
“环境增量留史 + 每请求完整团队快照附尾”，由 7.11 的独立 7G 实施；当前执行完成后验收时一并提出，不中途扩大原任务。
...
3. **多目标与原生集群。** 在实际多目标环境验证资源可见、放置、排队、冲突和释放；接通首个原生作业后端 Slurm。
   遥测、Piarium 预留与 scheduler allocation 分开；明确共享机器外部进程的边界。资源协调 Agent 是可选使用方式，无需另建常驻服务。
4. **研究消费者。** 在科研工作台/普通工具中查看机器、作业、日志、指标和结果，渐进展开；
   一条开放问题的基线/变体在本机与远端实际运行、对比并继续。缺少真实远端或集群环境时保留未验证范围，不把完整 7D 标为已验收。

持久 Notebook 和其余集群框架不是本阶段前置；局部 mock 不能替代进程/远程/资源合同的实际验收。

**7E：通用多 Agent 交流、持续现状与研究消费。**

当前执行按 D-300 的原任务交付；下列增量现状路径在验收时记录实际行为，再按 D-301 / 7G 接续改造。
不能因 7G 已写入计划就把其新要求当作原执行任务承诺，也不能将 7E 已接线当作 7G 已完成。
...
完整评估（真实开放问题贯通、三形态对比、费用记录）尚未执行；受管远程与集群后端缺位时不把 7F 标为已验收。

**7G：请求前上下文统一准备与完整实时现状表（D-301，当前执行任务之后）。**

状态：设计已接受，尚未实施/验收。先让当前执行 Agent 完成已接收的 7B 余项、7C–7E 和可完成的 7F；
主代理验收该交付时一并给出本阶段要求，再在其成果上实施。可复用的投影、消息账本、原文引用和请求准备接缝继续使用，
...
验收当前执行交付时，先记录这些接缝已达到什么程度和实际缺项，再组织 7G 修改；本轮文档不宣称已修复运行时。

**7H：工具并行调度与后台长任务收口（D-302，通用 Harness 后续阶段）。**

状态：2026-09-20 设计已接受，尚未实施/验收。当前 D-300 执行任务先按原范围交付；本阶段与 7G 接续，
启动/调度改造可独立实施，完成通知必须接入 7G 的同一请求准备入口，不再建立只在回合开始运行的通知副本。
适用于 coding 与科研；不以远端/Slurm 实测完成为前置，也不以本阶段替代 7D 的耐久实验与资源管理。
...
以下是原 7A–7F 任务的完成判据；D-301 / 7G 与 D-302 / 7H 单独按 7.11 验收，不能把原任务通过等同于后续阶段已交付：
...
- 本机、受管远程和首个原生集群后端贯通真实作业、资源确认、取消/重连和产物路径，未具备的后端能力如实显示；
