...
| R1 | Partial（WorkingState、retrieval 与 checkpoint/turn/mutation direct kernel path 已 wired；combined operation/integration 与文件 apply/物化仍待接管）format v6、typed path state、AVL immutable root/trie、blob durability/source authority、固定 revision、强制 publish CAS/事务 operation、workspace/actor-scoped typed record identity、分页 root read、pin/GC/recovery root、durable cleanup retry | Application Host 的 branch/draft/result/publish/CAS/materializer 输入、verification/review、retrieval artifact/receipt/evidence、checkpoint/turn/mutation 已直接调用 kernel records；combined recovery operation/operation-file、Integration journal、root-oriented WorkingState adapter 和旧 SQL writer 仍待删除，不把 transient facade 当作交付 |
