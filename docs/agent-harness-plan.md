...
| R1 | Partial（WorkingState/retrieval and checkpoint/turn/mutation seam wired；combined Recovery/Integration not fully wired）format v6、typed path state、AVL immutable root/trie、blob durability/source authority、固定 revision、强制 publish CAS/事务 operation、精确 owner、typed domain record/reference、分页 root read、pin/GC/recovery root、durable cleanup retry | Application Host 的 branch/draft/result/publish/CAS/materializer 输入、retrieval artifact/receipt 和 checkpoint/turn/mutation record 已切到 kernel；combined recovery operation/operation-file 与 Integration durable journal 仍待切换，旧 recovery SQLite writer 不能宣称已删除 |
