...
3. **当前：快速检索（3.2/3.15/3.16，D-173–D-188）**：固定窗口来源、结构切片、图查询、本地语义召回与工具链已接。
   3.15 A–D 已接入公开 `explore`；独立验收补齐 actor scope、取消/截止、真实来源状态、终态、稳定视图、单元排名、required 组与到达即读，见 status 3.15 与 D-182–D-188。
   3.16A 已提交（`37b12e8e`、`8752e039`）。远程嵌入绑定（3.16B）可独立推进，不等待重排或整个索引阶段结束；向量复用与
   前台优先（3.16C）、草稿/线程覆盖（3.16D）、独立重排（3.16E）按各自契约接入。已实现的旧 3.15①②④ 接口继续使用。
...
| models.explore 经公开 explore 接入查询理解/搜索表达和候选判断 | 沿用 Pi 会话 ModelRuntime；真实槽位质量观察与 3.16B–E 仍待做 |
| 结构切片、图定义/连线另一端/反向 import、带版本的 focusRanges | 图只给可核验导航或关联；语义焦点不依赖查询词命中 |
| 词法加权覆盖、本地向量、开放候选按真实来源名次 RRF | 单元自身依据；3.16E 重排另接，不与 LLM 默认串跑 |
| 原文、provenance、OutputStore 与 UTF-8 预算 | 一次决定实际呈现，原文省略项与最终记录一致 |

3.15 A–D 的查询上下文、开放排名、呈现前单元与 LLM 消费者已交付，见 status 3.15 与 D-182–D-188。工具原生链、Actor 路径、
固定草稿与输出句柄的已验证事实看 status；3.16B–E 与真实 explore 模型观察仍待做。
...
**D. LLM 搜索计划、成组选段与局部补查（D-174–D-188，已交付）。** 消费已有 models.explore 槽位，在 pi-host 中复用当前会话的
...
A–D 已由 D-176–D-188 接入同一引擎与公开工具。不把 3.16 的索引/远程优化做成模型启用前置，也不依赖 3.16B 的后台远程绑定。
...
LLM 查询理解/表达与候选判断已由 3.15D 接线；扩散模型/后训练、全仓生成式摘要与零样本路由仍留后续，不作为当前能力的前置。
