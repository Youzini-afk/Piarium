...
3. **当前：快速检索（3.2/3.15/3.16，D-173–D-179）**：固定窗口来源、结构切片、图查询、本地语义召回与工具链已接。
   3.15 A–D 已接入公开 `explore`（短生命周期查询、分组计划、呈现前单元、成组选段、可选补查、共享取消/截止）。
...
3.15 A–D 的查询上下文、开放排名、呈现前单元与 LLM 消费者已接线，见 status 3.15。工具原生链、Actor 路径、
...
**D. LLM 搜索计划、成组选段与局部补查（D-174–D-178，已接线）。** 消费已有 models.explore 槽位，在 pi-host 中复用当前会话的
...
A–D 已由 D-176–D-179 接入同一引擎与公开工具。不把 3.16 的索引/远程优化做成模型启用前置，也不依赖 3.16B 的后台远程绑定。
