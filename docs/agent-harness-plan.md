...
| R1 | Partial（WorkingState、retrieval 与 Recovery durable catalog 已 wired；文件 apply/物化仍属 R2/R3）format v6、typed path state、AVL immutable root/trie、blob durability/source authority、固定 revision、强制 publish CAS/事务 operation、精确 owner、typed domain record/reference、分页 root read、pin/GC/recovery root、durable cleanup retry | Application Host 的 branch/draft/result/publish/CAS/materializer 输入、verification/review、retrieval artifact/receipt、checkpoint/turn/mutation、combined recovery operation/operation-file 和 Integration journal 均通过 kernel records 持久化；TS 仅保留短生命周期 SQL-shaped orchestration view，旧磁盘 recovery catalog 不再由生产路径打开 |
