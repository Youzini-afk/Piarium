...
3. **当前：快速检索（3.2/3.15/3.16，D-173–D-175）**：固定窗口来源、结构切片、图查询、本地语义召回与工具链已接，历史验证见 status。
   收口本会话的性能/发行修复（3.16A）；查询侧接入 LLM 查询理解与候选判断（3.15D），同时完成来源平等竞争（3.15A）、
   当前单元选择与必需范围呈现（3.15B）、保留来源机会的读取调度（3.15C）。先随 D 的分组搜索计划接短生命周期查询上下文，
   再接 B/D 的成组选段和有依据的局部补查；共用来源/取消/截止条件，不把新模型放在旧打包结果后。远程嵌入绑定（3.16B）
   可与查询侧独立推进，不等待重排或整个索引阶段结束；向量复用与前台优先（3.16C）、
   草稿/线程覆盖（3.16D）、独立重排（3.16E）按各自契约接入。已实现的旧 3.15①②④ 接口继续使用，不重做整个检索工具。
   explore 负责快速提供当前代码，较长开放追踪由 retrieval 承担；扩散模型与后训练留待后续，不增加当前实施依赖。
...
当前 protocol/UI 提供十个普通 Harness 槽位，embedding/rerank 不在其中，相关消费者缺口见 3.16B/E。继续使用用户所属的 Pi
...
| 已有能力 | 下一步实际改动 |
| --- | --- |
| 自动 surface snapshot、Documents 当前正文、固定草稿的 read/grep/find/ls 与线程基线 | 沿同一来源实现语义草稿/线程覆盖，不另建读取权威 |
| question/anchors/paths、对象与词组提取、候选按文件轮转、真实 rg | 保留明确范围和线索，收窄隐式意图判断；路径/符号明确时直接导航 |
| models.explore 已有配置槽位，尚无生成式消费者 | 3.15D 接查询理解/搜索表达和候选判断，沿用 Pi 会话 ModelRuntime，不新开检索子会话 |
| 结构切片、图定义/连线另一端/反向 import、带版本的 focusRanges | 图只给可核验导航或关联；语义焦点不依赖查询词命中 |
| 词法加权覆盖、本地向量、文件级 RRF | 取消开放候选的来源 tier，分别处理文件调度与当前单元相关性 |
| 原文、provenance、OutputStore 与 UTF-8 预算 | 一次决定实际呈现，原文省略项与最终记录一致 |

当前仍有来源统一等待、图补充读取的独立预算、`windowScore` 等旧逻辑，不能把本表目标当成已实现。工具原生链、Actor 路径、
固定草稿与输出句柄的已验证事实看 status；验收变化聚焦新的候选机会、当前原文与最终可见材料。
精确匹配继续用 grep，较长开放问题由 retrieval 处理；快速 explore 用 LLM 做局部语义决策，算法执行批量搜索/读取，不复制
完整自主子 agent 循环。查询表达与候选判断已有当前消费者目标；全仓生成式摘要和新词法索引按各自实际需要另行设计。
...
**D. LLM 搜索计划、成组选段与局部补查（D-174/D-175，当前交付项）。** 消费已有 models.explore 槽位，在 pi-host 中复用当前会话的
...
D 的搜索计划随共同查询上下文与 A/C 的必要接缝先交付；B/D 的成组选段随后连同局部补查完成。先打通真实模型改变搜索与
最终原文的链路，不把 A/C 的所有优化做成模型启用前置，也不依赖 3.16B 的后台远程绑定。A/B/C/D 的共享契约由同一整合者协调。
保留既有来源、路径、修订与 OutputStore 正确性测试。十问用于回归，必要时加入真实新问题；不先建设完整评测项目，
也不从后续 read/edit 自动学权重。
...
**A. 收口现有性能与发行修复。** 本会话已有未提交工作：真实批推理、切块尺寸查找复杂度修复、存储增量计数/批事务与中断恢复、
模型配方固定修订、构建准备和 Electron 实际加载 smoke。沿 status 中已取得的定向证据交付，不为文档更新重复全套检查。
完整冷扫时间仍未量得，不能把部分语料或中断扫描外推成全仓性能。Node 线程参数、长行编码完整性和查询竞争仍由 C 完成。

**B. 嵌入配置到远程调用的完整路径。** 先接 OpenAI 兼容入口，复用 `knowledge/embedding.ts` 的已有适配器逻辑及 Pi provider
配置；新增 embedding kind 与用户设置，解析不含秘密的有效空间绑定。当前 provider/settings/auth 都依赖 Pi session runtime，
须将同一配置解析与远程执行提供给独立于聊天寿命的 Pi runtime 后台上下文；凭据保留在那里，Host 传已授权正文、接向量，
不从首个活动 session 借模型，也不读写第二份 auth 配置。关闭聊天、重启 Host、用户切换绑定时有明确恢复/失效路径。
统一接口携 query/document 用途、所选模型输入长度/计数能力、取消及批次身份，校验返回数量/次序/维度；本地后端适配同一
调用契约。新空间的查询与文档索引同时切换，未完成时查新空间已发布部分，不混入旧空间；凭据轮换不重嵌。
验证 Settings→配置解析→真实协议→provider 请求→发布→查询链，以及最后会话关闭、绑定切换时旧任务不能继续发布。
这一步独立于重排，不要求 C 的全部优化完成，也不把知识库仍为 embedding:null 与代码语义后端混为一事。

**C. 复用、编码覆盖和前台优先。** 向量以空间、用途与实际 embedText 为键，查询缓存同样绑定实际输入与模型；内容不变的块
更新位置/修订而不重嵌，缓存不会绕过当前 scope/来源授权。每块独立按所选 tokenizer 适配输入，长行按 token/字符位置续切；
先前“每行落在某块”的测试不能证明正文未被截掉。体积上限来自真实 provider/运行时，不能把本地 512 套到所有模型。
同范围扫描与变更合并，初始化共享在飞请求；由模型实例队列在当前批完成后优先服务前台，并保留后台进展，不能把整仓后台
任务先排满队列再仅在扫描循环 yield。查询开始时无可查部分，可在本轮来源接收阶段消费首个兼容发布事件启动语义查询，不逐批
重查或延长本轮截止条件；已结束查询不被唤醒。Node ORT 实际 session 设置线程数，不能仅设置 WASM 参数。枚举/正文/结构结果
按修订复用，检查点/计数不逐文件扫全库；新旧异步结果按修订发布，
重启恢复已发布批，删除与读取失败在结束时对账，coverage 与结构可用性、查询 empty 分开。验证一处编辑只重嵌变化文本、
单条长行与后续高密度窗口没有丢正文、冷扫中查询能执行、旧修订不覆盖新修订。

**D. 固定草稿与线程语义覆盖。** 沿既有捕获与分支 authority 建覆盖层：正文先固定，嵌入后台完成；同路径旧磁盘向量立即
遮蔽，新向量尚未可用时报告缺口，词法/读取继续使用已捕获原文。线程读取固定基线加自身变化，不借 live 父目录作为基线。
复用 C 的编码文本向量与范围发布，不共享路径权限或分支归属。验证 dirty-only 新文件、捕获后修改、写入终结旧草稿和兄弟
分支隔离；缺正文不假装只有磁盘，缺向量不等同于正文不可读。

**E. 独立重排。** 复用 B 的模型配置/调用边界并增加实际选定的 rerank 后端，输入消费 3.15B 的当前代码单元与拟呈现正文。
它与 3.15D 共享当前正文输入，但普通 reranker 输出顺序/分数，不能假装具备成组互补、必需范围或缺口判断。同批不默认串跑。
对符合实际输入长度的可展示视图排序，保留实际被评估的完整视图或事先构造较小视图；不让后端截断后仍声称评估完整函数，
不在评分后任意裁掉正文。同批以有效顺序排序，不叠加来源分数，未评估候选不补零，不混用不同模型分数。后端接通即按有效
绑定使用，不预先承诺未知本地交叉编码器的默认能力；重排失败保留已取得的可读材料。验证真实消费的文本/候选身份、
重排部分失败与最终原文一致。

A 收口后，B 可与 3.15 独立推进；B 的调用契约供 C/D/E 复用，E 另依赖 3.15B 的呈现输入。工作区仍是包含陌生文件的范围，
注意力只改变建设顺序，不改成 sketch 替代全文或仅热点正文。性能分别看首个有用结果、热查询、编辑更新和完整建设。
LLM 查询理解/表达与候选判断由 3.15D 当前实施；扩散模型/后训练、全仓生成式摘要与零样本路由仍留后续，不作为当前能力的前置。
