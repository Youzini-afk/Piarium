...
4. **带呈现方案的证据单元 + 一次联合选择**（同一件事的两面，一起做）。`sliceSymbol` 拆成"给出候选范围与各自字节成本"和
   "按选定方案渲染"两步；一个已物化单元在本次调用内保留：当前正文与 revision、所属容器、支持哪些要求、可选阅读深度
   （定位片段 / 机制片段 / 完整单元）、各方案实际字节。打包改两层：先构造满足要求的主证据（夹具不与明确要求的生产入口竞争、
   请求端不顶替注册端、同一要求的重复证据不加分），再按相关性选开放候选与支撑。十一项各归其位：`roleFit` →答案范围，
   `newEnd` →哪项关系要求未满足，`GRADE_RANK` →核验状态，`sameFact` →去重，`cost` →呈现预算，只有 `windowWeight` 留作相关性基线。
...
### 3.16 语义来源：嵌入召回、重排、查询扩展（D-158）

设计 6.1 已留位（"配置的向量"、"第三路路径候选"），一直没填。填它的理由是四轮观察已经把词法能修的修完，剩下的失败类是词汇缺口。
AFT 证明这是几个模块的工程量，不是研究：MiniLM-L6 本地 ONNX、RRF 混合、指纹绑定索引、远程后端可选。

1. **后端抽象与降级路径**（设计先定，与 3.15 设计同批）。`models.embedding` / `models.rerank` 槽位（设计 8.5），本地默认、
   远程可选，接现有 provider 配置的新 model kind，复用 `baseUrl` 与凭据。索引指纹 = 后端 + 模型 + 维度 + 切块版本，指纹变即作废，
   期间只有词法加图并如实报状态。远程查询失败 / 超时回退词法加图，查询超时与后台建索引超时分开。**不设信任门、不设花费守卫**——
   用户配置远程提供商就是同意，成本是用户的账；合理优化（增量重嵌、查询嵌入缓存、批量）不拿来限制设计。
2. **本地默认**。`all-MiniLM-L6-v2` 经 `@huggingface/transformers` 在 Host 内跑，模型文件走语法包的内容寻址存储与按需下载；
   intra-op 线程限到核数一半；切块按结构单元（容器）不按字符窗口；索引以 Documents 修订为准增量更新。Electron asar 打包
   （原生模块）必须验，这是已欠的验证债的一部分。
3. **嵌入作为第三路路径候选**。产出候选路径与候选片段，进 3.15 的"取证 → 证据单元"管线，不绕过答案要求与联合选择；
   `details` 标来源为 `semantic`，与 rg / graph 同级别报 `ready / empty / unavailable / failed / stale`。
4. **重排**。对已在池里的候选片段逐对打分，替掉 `windowScore` 的相关性基线部分，不替掉要求与证据检查。远程可重排一两百个，
   本地按延迟预算收窄。OpenAI 兼容嵌入先接（一个接口通吃）；重排适配器按需求接第一家。
5. **查询扩展**（可选，`models.explore` 已有的槽位）。把问题改写成两三个措辞全部嵌入去搜，直接对着词汇缺口。
6. **远程放开的能力**按需接：多粒度嵌入（符号 / 文件摘要 / 目录摘要）、代码专训模型、更大维度（上限按 AFT 设 4096）。

十问里当前只有 3 和 5 部分属于词汇缺口类，不足以评判这一刀；3.15 第 6 步的 held-out 问题里要有明确的词汇缺口案例。
不声称检索质量提升，量到的进 status 标对照数字。
