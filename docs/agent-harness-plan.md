...
3. **当前：窗口读取与 explore（3.2）**：自动消息来源、固定 dirty snapshot、draft-aware explore/grep/read/find/ls、线程草稿基线、
   ...
   **3.14 检查点一已接**（D-153）：查询内词组权重表与截断三态在 `details` 可见；排名尚未改。
...
`related` 已接线（3.12）。3.13 已让已有精确线索被验证并进入可见正文。3.14 检查点一已让查询内区分度可见；加权覆盖与局部打包仍待本刀后两个检查点。仓库级词法索引 / 桥接 / embedding 仍等观察决定，不在本刀。
...
**3.14 查询内区分度与片段证据贯通**（进行中，D-153 起）：
...
1. **检查点一（已接，D-153）。** 每次调用建一张查询内词组权重表：\(N\) 与 \(df(g)\) 都是本池去重文件数；覆盖三态
   ...
   并排除自身问题文本。排名比较器未改，问题 4/6 预期仍不翻转。
2. **检查点二（未实施）。** 加权覆盖进入主比较；`roleFit` 退回有界偏好，不再先于内容证据。
3. **检查点三（未实施）。** 同一张表进入局部选择与打包；修掉内容词当完整对象的混淆；定位题已有直接答案时
   `offTopic` 不再只因还有 `limit` 名额而展开或填正文。多词邻近性仍留到下一次观察。
