...
### L3：材料集合、结构阅读与实践交接（基础已交付；复杂 PDF 结构阅读待实施）
...
- L3-P0 先接页面图、区域裁剪、原件/解析版本和取消/重试；L3-P1 接可替换版面/OCR 适配器，恢复多栏顺序并输出坐标结构；
  L3-P2 再按需接表格单元格、公式、图表视觉增强、集合内结构检索和阅读器 UI。较重运行时沿可选组件或配置服务接入，
  不自动增加主包模型体积。
