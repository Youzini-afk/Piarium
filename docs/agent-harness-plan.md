...
**阶段 N：会话下一步选择（D-325，待实施）。** 将输入框上方默认启用的单条建议改为用户主动配置的模型分工能力：
Agent 结束后发起一次短输入、短输出的后台模型请求，可返回零条或多个不同方向的草稿；移除自动会话回顾。
知识库的 `models.suggestions` 属于另一项已交付能力。实施范围与完成判据见下文阶段 N，当前行为仍以代码为准。
...
## 阶段 N：会话下一步选择（D-325，待实施）

当前 `pi-session-automation/runtime.ts` 在 `agent_settled` 后等待约 60 秒，从最近一组用户/助手文本调用通用小模型，
同次生成 recap 和一条 suggestion；`sessionSuggestionEnabled` 在 UI Store 默认开启，Host 只在明确为 false 时停止。
`PiAssistBar` 将建议放在空输入框上方，点击填入草稿。模型分工的 `models.suggestions` 实际用于知识库建议，
...
- 将现有 `models.suggestions` 在协议、设置目录、UI 与消费者中明确命名为“知识建议”，与“下一步选择”分开。
...
- 实际完成前以当前代码与 status 为准；实施时同步 `agent-harness.md` 的模型槽位说明、设置目录、i18n、模块文档与 status，
  删除旧分支和过时测试，不为新功能建立第二套会话历史或模型凭据栈。
...
验证模型请求与 UI 消费的真实通路和耗时边界；真实模型建议质量需实际使用观察，不把测试夹具的文本当质量证明。
