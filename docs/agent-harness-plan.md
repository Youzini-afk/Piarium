...
1. **工作状态与集成（3.4/3.5，核心已交付）**：固定结果读取、原生结果、可撤销集成、Git/非 Git 物化、安全回收以及 dispatch
   ...
   同名 read/grep/find/ls/explore 的 WorkingState 只读视图已接入真实 Thread Run（D-212）。虚拟写入、dispatch 瞬时基线与嵌套线程仍待后续纵切。
...
tombstone 隐藏路径，父 drift 不能补读，scope 仍由 Host 拒绝。无目录写入、物化切换和整仓 dispatch 快照尚未交付。
