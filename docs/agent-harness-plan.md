...
3. **当前：窗口读取与 explore（3.2）**：自动消息来源、固定 dirty snapshot、draft-aware explore/grep/read/find/ls、线程草稿基线、
   ...
   **3.14 已接**（D-153–D-156，验收补 D-157）：查询内词组权重表可见；加权覆盖进入主比较；同一张表进入局部选择与打包。
   **3.15①② 已接**（D-163 / D-164）：图线索带到达理由；`explore` 与 `related` 共用查询期文件角色与依据。
   **接下来三刀并行排期，不再写"等观察决定"**（D-158 / D-159 / D-160）：**3.15** 按设计复查把 explore 收敛到
   "答案要求 → 取证 → 证据单元 → 一次联合选择 → 正文与呈现记录"五段，拆掉十一项打分公式（①②已接，④与记录项待做）；**3.16** 语义来源——嵌入召回、
...
`related` 已接线（3.12）。3.13 已让已有精确线索被验证并进入可见正文。3.14 已让查询内区分度可见、进入主比较，并进入局部选择与打包。语义来源（嵌入 / 重排 / 查询扩展）排期 3.16，与 3.15 管线收敛并行；持久词法索引仍等观察——rg 在两千文件上够快，五万文件时再议。
...
**下一刀的独立材料**：命中驱动的正文组装装不进没有任何查询词匹配的行（问题 5 的 `links.push` 在第 220 行、落进
`unit.omitted` 的 220-238）。加权无法弥补这类缺口，需要单独决定容器正文如何选行；不在 3.14 内——它是 3.15 设计三的直接输入。

不做：embedding、BM25、词法索引、新评测框架、NLU、`related`、结构切片、符号图写入侧、TriviumDB。不改 D-090「测试路径不默认降权」
原则本身。不声称检索质量或速度提升。

### 3.15 explore 收敛：要求、证据、呈现分开决策（D-159）

3.13/3.14 验收暴露的模式：文件角色一个旋钮调了三轮（D-148 加 → D-154 降 → D-157 升回 rank），`windowScore` 累积到十一项
手调系数，量具每轮被抓到一次。设计复查把七条张力归成三个缺口——答案要求没贯穿决策、选择与呈现没共同决策、引擎没有统一
表达处理结果——目标形态见设计 6.1"目标形态"。**这是 explore 后半段的一次真实重构，不是加几个字段。** 顺序按依赖：

1. **图展开带到达理由**（已接，D-163）。每条图线索记录它为什么被走：问题对象直接触发 / 当前相关语句产生的取证需求 /
   只是与已选代码同容器。第三种不得拿直接线索待遇；`candidateTier` 与 `isDirectClue` 里"任何 connection → tier 1"改为按
   到达理由。无对象问句可以从真正参与解释的语句产生新对象，不能机械确认相关性的展开只是普通补充。保留现有预算数值，
   不用更小的连接上限代替相关性判断。
2. **共享文件角色规则**（已接，D-164）。`explore` 与 `related` 用同一份分类与依据（文件名模式 / 项目声明 / 未知），角色是查询期
   装饰不持久化到图——角色随测试根配置变，持久化要管失效。
3. **`AnswerRequest`**。由 `parseExploreQuery` 输出经一个小而保守的解释步骤形成：形式（定位 / 机制 / 关系）、对象及来源、
   所需证据（如"当前正文中的 register 调用，参数等于 X"）、范围（优先非测试实现，并记这是推断还是用户明说）、完整性
   （找到一个足以定位的即可 / 需列全）。判定结果三态：符合 / 明确不符 / 证据不足。规格作用于一组证据，不只一个窗口——
   裸 `explore.search` 要的是**同一连接值**的注册端和请求端，不是"有一个 register 加有一个 request"（现 `hasBothConnectsEnds`
   只按 callee 集合判）。无对象问句有部分规格：交付材料的形状（实现代码、机制级阅读、目标未绑定），不假装有判定完整答案的谓词。
4. **带呈现方案的证据单元 + 一次联合选择**（同一件事的两面，一起做）。**先做接口**（D-161，这是 3.16 能接进来的前提）：
   `windowsFor` / `sliceStructureWindows` 的入口从 `hitLines` 泛化为 `focusRanges`——当前正文里的重点范围，命中行只是它的一种
   来源；`sliceSymbol` 拆成"给出候选范围与各自字节成本"和"按选定方案渲染"两步。证据单元上 `arrival / assessment / purpose`
   三字段分开，替掉揉在一起的 `EvidenceGrade`。一个已物化单元在本次调用内保留：当前正文与 revision、所属容器、各路到达理由、
   支持哪些要求、可选阅读深度（定位片段 / 机制片段 / 完整单元）、各方案实际字节。打包改两层：先构造满足要求的主证据（夹具不与
   明确要求的生产入口竞争、请求端不顶替注册端、同一要求的重复证据不加分），再按相关性选开放候选与支撑。**`windowScore` 整个删掉**，
   十一项各归其位：`roleFit` →答案范围，`newEnd` →哪项关系要求未满足，`GRADE_RANK` →`assessment`，`sameFact` →去重，`cost` →呈现预算；
   剩余相关性顺序在 3.16 接重排前用 `windowWeight`（加权覆盖）占位，接了重排就换成重排顺序，**两套不并存**。`rankCandidates`
   改成"答案要求的直接取证优先 + 文件级排名融合"（3.16 第一片接入语义一路后融合有两个来源，之前只有词法一路）。
   选择用真实 UTF-8 长度含标题、状态、句柄提示；formatter 只渲染已决定的内容，不再自行丢弃主证据。`limit` 与 `byteBudget` 保留，
   但只有一次决定展示什么。选行原则：保住直接证据 + 使其含义成立的局部控制关系；机制题优先完整语法块；完整单元放得下就不早裁
   （问题 5 的 `loadGraphFacts` 全文 4481 字节，不是预算问题，是预算先给了无关窗口）。**不建程序切片器。**
5. **三类查询内记录**：文件记录（路径、来源、revision、读取结果）、单元与证据记录（属于哪个快照、支持哪些要求、核验状态、
   候选范围与呈现方案、采用或未采用的理由）、最终呈现记录（哪些原文范围实际写进正文、位于哪些字节区间、哪些要求有了可见支撑、
   哪些仍缺）。`notRequested` / `omitted` 等模型可见摘要从这里派生；`stoppedBecause` 写实际决策原因（`read-budget` /
   `duplicate-evidence` / `byte-budget` / `query-requirement-contradicted`），不写"irrelevant"。逐候选完整记录只在观察模式输出。
   量具改读最终呈现记录，不再从裁剪前的 `snippets` 推断可见性。
6. **held-out 观察**。由没参与本轮调参的人预先写少量新问题与最小证据（换目标、换模块、换代码形状：另一个有同名夹具的入口、
   答案行不含问题词的机制、需两个单元共同解释的行为、真正需要图展开的问题），不写进被检索语料，实现冻结后跑一次；一旦据此
   修改就降为回归集。十问继续作为开发回归集，五个变体算一个入口。

**修正记录**（D-159）：读预算不是统一的 23——两次普通物化共用一个计数器对 23 比，图补充物化是独立切片不对任何预算比，
上限接近 46；`relationRoleRank` 只查等级不查对象与关系，且 `preferTests === false` 是对实现域 register 问句的推断不是用户明说；
管线不是线性七级，正文组装在打包前发生；`rankCandidates` 的"任何 connection → tier 1"是把取舍写成不可补偿等级，不是零系数；
量具 `visible #N` 来自字节裁剪前的 `snippets`。

不做：embedding / BM25 / 持久索引 / NLU 作为这些问题的答案（它们是 3.16 的事，不是 3.15 的）；不改 24 KiB、20 条、读预算与
图预算数值——先把预算放到正确的决策位置。来源失败 / 空 / 不可用 / 部分 / 未请求继续分别表达；图失败不拖垮已取得材料；
`filesDropped` 下界语义、权限、固定来源、取消传播、OutputStore 生命周期不变。

### 3.16 语义来源：嵌入召回、重排、查询扩展（D-158，设计细化 D-161）

设计 6.1 已留位（"配置的向量"、"第三路路径候选"），一直没填。填它的理由是四轮观察已经把词法能修的修完，剩下的失败类是词汇缺口。
AFT 证明这是几个模块的工程量，不是研究——但它的切块是反例（符号名 + 签名 + 正文前 300 字符，见设计 6.1），不抄。
起点不是零：`knowledge/embedding.ts` 已有七家远程适配器（OpenAI / Voyage / Mistral / Gemini / Jina / Cohere / OpenAI 兼容），
接口太薄（只有 `embed(texts)`，无 query/document 用途、无取消、无批次对应），加厚它。

**与 3.15 的接缝先冻结**（D-161）：定位——3.15 接受词法行、结构身份、版本化块定位（`focusRanges`），3.16 给带空间、块、来源身份的
语义线索；证据——3.15 给当前单元、要求检查、呈现方案，3.16 给语义到达理由与相关性判断；排序——3.15 管主证据要求、去重、预算选择，
3.16 给来源排名与统一重排顺序；状态——3.15 记实际读取与最终展示，3.16 记索引进度、查询结局、模型绑定与覆盖范围。
这样 3.15 不等远程 adapter，3.16 不给旧 `windowsFor` 填假命中行。

**四片按序做，每片端到端可观察**（串行，D-161）：
...
**第二片：重排 + 远程嵌入。** `models.rerank` 槽位，本地交叉编码器默认、远程适配器接第一家；重排输入含当前正文与呈现方案
（设计 6.1"模型增强"），成功重排的候选用重排顺序作主要相关性、删掉 `windowWeight` 占位，两套不并存；可在读取前对索引块文本
先评一次帮助调度，输入身份相同复用；查询与重排身份落地。远程嵌入接 OpenAI 兼容 `/v1/embeddings`，`embedding.ts` 接口加厚
（query/document、取消、维度、批次对应），provider 配置新 model kind 分流。换 API key 不重嵌、换重排器不重嵌。

**第三片：草稿与线程 overlay。** 基础索引 + 分支变化 + 本轮固定草稿覆盖 − 被替代路径；草稿在**捕获时**嵌入成短生命周期覆盖层，
同路径磁盘块被遮蔽；线程基线不漏进 live 父目录后来新增的文件；候选范围尽量在近邻检索阶段过滤，取回后过滤则补取到工作预算并报不足。

**第四片：查询扩展、多粒度、软路由。** 查询扩展（`models.explore`）：改写各自嵌入、best-rank 归并、不进 D-153 表、不改对象与范围、
重排仍对原问题判断。多粒度：结构概览（确定性拼接）与语义摘要（`models.explore` 生成、后台低优先级、只重做变化文件及祖先）两种数据，
作为同一来源家族里可独立查询的集合，目录命中不广播给后代、沿摘要引用再落到具体单元；摘要命中是导航线索，不是论据。零样本分类：
接在确定性对象提取之后、`AnswerRequest` 完成之前，对象换类型占位后分轴（阅读形式 / 材料领域 / 关系倾向）取最近原型，
结果只作检索计划提示、记为系统推断、显式要求优先、分歧保留混合检索；原型是版本化产品资源，跨语言能力按当前模型如实表达
（MiniLM 是 English，不兑现中英对齐）。

held-out 里要有的词汇缺口形状（准备者照形状写，具体问题不进文档也不进语料）：用行为描述机制而代码用内部术语；函数词法可找但
关键动作在无词汇重合的后半段；需两个单元共同解释；问题语言与代码语言不同；语义相似的夹具/旧实现/相反分支作干扰；高层描述需经
摘要导航。"通过"落在最终呈现记录：由哪条语义查询取得 → 哪个空间/块/修订 → 定位到哪个当前单元 → 重排评估了哪份正文 →
采用了哪种呈现方案 → 必需原文在哪些输出字节区间 → 人读后确认足以推进。机制题只展示函数名不算过。同批 held-out 在实现冻结后
按预定配置（3.15 + 词法/图 → 加语义召回 → 加重排 → 需要时加扩展）对照跑，分清"词法没取得文件 / 取得文件没取得范围 / 都取得
但重排改善选择"三种收益。不声称检索质量提升，量到的进 status 标对照数字。

### 3.17 bash 输出压缩：按命令分派（D-160）

现状是纯字节头尾切（32 KB，头 50% / bash 37.5%，最近换行，全文进 OutputStore），对内容一无所知。目标是设计 5.2 写的五层：
命令专用解析器 → 输出形状嗅探 → 包管理器通配 → 声明式规则 → 通用兜底。先做 vitest、tsc、eslint、git 四个，每个解析器
带真实输出样本的确定性测试；全文照旧进句柄。**不用小模型总结替代**（漏一个失败是静默的）；模型总结只作非结构化输出上的附加，
且要明确标注"这是模型挑的行，不是全部"。
