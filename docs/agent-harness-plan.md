...
| R2 | **Partial**：生产文件权威已接通，D-278 修复物理互斥/定向 coverage/owner/重试/真实 Documents 装配 | 保持 Registry buffer authority；补 low-level pending operation 到 Host 可见状态、重试和保留的恢复闭环；不恢复 TS writer |
| R3 | **Partial**：kernel scan/capture/materialize/measure/remove 和主要 consumers 已接，D-278 修复实际 root/执行身份与安全物化 | 仍需 native operationId/root/writeRevision → Git executionBaseline → Registry/binding 的 durable switch intent/receipt，setup 真实退出与回收交接证据；旧 seam 单测不得充当 native 完成证据 |
