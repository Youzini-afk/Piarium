...
3. **当前：窗口读取与 explore（3.2）**：自动消息来源、固定 dirty snapshot、draft-aware explore/grep/read/find/ls、线程草稿基线、
   ...
   **3.14 已接**（D-153–D-156）：查询内词组权重表可见；加权覆盖进入主比较；同一张表进入局部选择与打包。
...
`related` 已接线（3.12）。3.13 已让已有精确线索被验证并进入可见正文。3.14 已让查询内区分度可见、进入主比较，并进入局部选择与打包。仓库级词法索引 / 桥接 / embedding 仍等观察决定，不在本刀。
...
`unit.omitted` 的 220-238）。加权无法弥补这类缺口，需要单独决定容器正文如何选行；不在 3.14 内。
