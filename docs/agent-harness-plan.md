...
| R3 | **Complete**：固定 Git/非 Git/unborn baseline、kernel scan/capture、durable root materialization、Git execution metadata/filter/index 适配、materialized result writeback、copy/clone backend 事实、managed-directory reclaim/measure | dispatch/queued/nested/virtual→materialized/settle/archive/restore/history release/delete 的生产资源路径已接 kernel；native result root 取代 mandatory snapshot，旧 thread-worktree/materializer/switch 的正文/目录实现只保留测试 seam。Thread/Run policy、Git 语义适配与未保存 Registry authority 留在原领域 |
