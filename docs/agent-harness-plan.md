...
状态：**L0 已交付；L1 连续 Web 搜索/固定快照/材料复用已交付并接线；L2 论文发现与关系展开已交付；L3 材料集合与结构阅读已交付；L4 跨线程材料授权已交付；L5 Web/学术快速决策消费者已交付；L6 收口仍在实施**。设计 authority 为
...
- 沿 L1 读取接结构化章节/页码/表格/图片/附录；轻量解析先可用，复杂布局按需使用合适解析 adapter/视觉读取。
  解析能力不足明确返回，原页/区域可读；较重运行时沿可选组件或配置服务接入，不自动增加主包模型体积。
...
完成判据：用户/Agent 可以在一组论文里找到方法或实验细节，展开上下文/图表并把真实材料交给后续实践，
不能仅凭 PDF 转成一段文本就宣称结构阅读完成。
