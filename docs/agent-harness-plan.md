...
后续新增 **阶段 S：对话式设置与 Agent 管理（D-306）**，字段、领域 action、client Surface 与组合管理已接线并定向验证。
...
阶段 S 之后接续 **阶段 W：会话等待、触发与续接（D-307）**，各来源、远程对账、生命周期收口与 calendar 管理已接线并定向验证。
...
状态：**字段、领域 action、client Surface、组合管理与产品 Skill 纵切已接线并经 D-308 验收返工 / 阶段仍为 Partial**。
S0–S4 的共用目录、查询披露、app/Pi/client/action 写入通路、组合更新与组合 Skill 已经接线；
个别无 owner API 的条目保持 unavailable，外部登录/安装与跨平台 Surface 现场未实测。设计 authority 为
...
状态：**各来源、远程对账、生命周期收口、calendar 管理与 scheduler 恢复纵切已接线并经 D-308 验收返工 / 阶段仍为 Partial**
（2026-09-20）。产物、指标、日志、文件与外部查询来源，受管远程续接，主动 archive/delete 收口与 calendar 任务的
Agent 管理已交付；跨来源 all/any 复合算子与相容观察去重未实现。设计 authority 为
