...
| --- | --- | --- |
| R0 | ✓ Cargo workspace、私有 kernel 进程、schema/generated client、握手/epoch/取消等待/关闭、Electron/Web staging | Host 启动会管理真实子进程；跨平台真机与签名产物仍需 R6 分别验收 |
| R1 | ✓ kernel storage vertical：SQLite/object store、immutable root/trie、blob durability、branch/revision/pin/CAS、operation/recovery、GC | TS WorkingState/Recovery/结果/草稿/evidence 的全部消费者尚未完成切换；旧直接 writer 仍按 status 标记，不能提前宣称 R1 全部完成 |
