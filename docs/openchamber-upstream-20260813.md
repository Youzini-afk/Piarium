...
| Work Status panel | `f2523d0c`, `222057ab`, `630ac299`, `d7231b6d`, `ee9c9d6f`, `fa1cd279`, `83c4c75f`, `5ef828f3` | Adopted as a Pi-native wide-chat panel. It projects `SessionSnapshot`, `SessionStats`, `PiSessionFeatureState`, current tool executions, Fleet RPC, and MCP public status without introducing a second message-context owner. It does not restore OpenCode todo, goal, MCP, or quota stores. Upstream's later empty-panel recovery is structurally unnecessary here: Piarium has no per-section hide preferences, always renders its session section and header, and therefore cannot hide the control needed to restore hidden sections. |
...
| Markdown loops in `.agents/loops` | `0ba330c7`, `8a367382`, `bac56fc7`, `ffef080b`, `d1bb9d3a`, `96c8d477`, `3bbde9af` | Adopted in the Pi-native scheduler. Markdown remains authoritative and uses Pi model, thinking, agent, and goal fields. File-path identity preserves runtime history across frontmatter renames and removes orphan duplicate projections of the same file; revisioned edits prevent agent/UI races; malformed higher-priority loops keep their last-good state without activating a lower duplicate. Piarium's UI edits or deletes the authoritative Markdown rather than disabling those actions. Upstream's 80-character name rejection is not imported because it only compensates for an upstream storage truncation that Piarium deliberately does not have. No OpenCode owner, permission, or variant field is imported. |
...
| Embedded chat loading, context focus, and terminal keepalive | `7a04dd5c`, `687cc17d`, `8702c6d5` | Adopted through Piarium's context panel and Pi-native chat where ownership matches. Only the active persisted session-chat tab mounts a complete embedded application, and closing the panel mounts none. The context-pin focus patch is no longer applicable because Piarium does not own message pinning or context reinjection. Terminal ping cadence is 45 seconds, below Piarium Relay's measured 90-second data-socket reaper while avoiding unnecessary wakeups. |
| Terminal startup before emulator readiness | `6eac705b`, `bbbb30b9` | Adopted with Piarium's lazy Ghostty owner. Mounting the first terminal estimates its dimensions before the emulator JS/WASM and remote font finish loading; the shell starts immediately at that size or 80×24, then receives the measured size. A per-directory/tab in-flight key prevents React Strict Mode from creating duplicate PTYs, and a late completion is accepted only while the tab still owns it. |
...
| Additional OpenChamber UI preferences and shortcuts | `228e5c8b`, `12c14825`, `1dd5e3fa`, `8274dd82`, `d91c2c33`, `01574139`, `2ffb9415` | Reviewed by consumer. Selection-to-chat is adopted against Piarium's real draft store and native textarea: `Ctrl/Cmd+L` captures a focused editor, text control, or rendered Markdown selection and appends it to the exact current session/workspace draft; the sidebar moves to `Ctrl/Cmd+Alt+L`. The collapsed-message setting, numbered context-panel switching, and extracted-document attachment cascade depend on removed stores/models and are not imported as dead settings. Pi tool cards already expose complete arguments—including glob patterns—and Pi timeline entries have no replay-on-bootstrap animation to suppress. |
| New-project landing state | `4ef9ce1c` | Adopted through Piarium's pending-draft workflow. Adding, creating, cloning, or quick-adding a project activates its real project/directory and opens a Pi-native unsaved chat draft; it does not create an empty Pi session or call an OpenCode directory route. |
...
| Git repository discovery independent of server cwd | `8d604397` | Adopted at Piarium's Git service boundary. Every `simple-git` repository client now requires an explicit directory, while global identity reads use the user's home directory as their stable base. This removes the last fallback to the server process working directory without changing Piarium's existing non-repository status response or its nested-worktree resolution. |
| Model selection restoration after agent messages | `2ea14335` | Reviewed, not copied. The upstream repair restores a manually selected OpenCode model after an agent-generated user message reset it to that agent's default. Piarium's main-session model comes from the owning Pi worker snapshot, and subagents/plugins run through separate Pi sessions or plugin contracts; there is no equivalent renderer-side model reset chain to patch. |
| Derived runtime-key hot path | `ff79e918`, `2489c3fb` | Adopted with Piarium's injected runtime globals. Before an endpoint is explicitly initialized, repeated store and render reads reuse the derived key without reparsing URLs; changing either the API base URL or local-origin input invalidates it immediately. Explicit runtime keys remain authoritative. |
| Walkthrough output language | `b92456fe`, `4e8fc1d2` | Already adopted in Piarium's walkthrough owner. Generation records a supported output language, the prompt requests that language, stored variants are selected by model and language, and the UI reports honest fallback when an older or differently generated walkthrough is shown. Piarium supports its complete current locale set and does not import a partial German locale. |
| Lost relay send outcome | `00c19eb9` | Reimplemented at Piarium's transport-neutral runtime client rather than copying OpenCode's message refetch loop. If a runtime socket closes while a request is pending, the error now records the exact method and that the already-sent outcome is unknown. Piarium never blindly retries prompt, steer, or follow-up requests; the composer keeps the draft and tells the user to inspect the session before resending, avoiding both silent loss and duplicate agent runs. |
| Shared worktree ownership | `7bb9b898` | Adopted only at Piarium's affected mobile project browser. Multiple configured checkouts of one repository no longer each render the same sibling topology: the configured primary checkout owns unregistered siblings, or the first configured checkout does when the primary is absent. A checkout already configured as its own project remains visible there and is omitted from the owner's nested list. Desktop Pi sessions already have single ownership through longest matching project paths. |
| Mobile navigation and tablet rework | `fbbbecf4`, `d6848ff7` | Reviewed by user-facing capability, not copied as an OpenCode shell replacement. Piarium already has cross-project Pi sessions, instance management and reconnect recovery, session usage metadata, Files/Changes/Terminal/MCP surfaces, native back handling, safe-area handling, and a live phone/tablet/foldable layout tier. The mobile title now opens Piarium's existing Pi session switcher, so status, attention, project selection, directory, and session opening retain one owner. Upstream's unified workspace drawer and its unverified 600/1000 device thresholds are not imposed over Piarium's current surfaces and measured 700/900 touch tier. |
| Managed runtime bind hostname and provider env aliases | `94a165d3`, `b18ec442` | The bind-host failure mode is adopted at Piarium's own server boundary: CLI and direct server startup reject URL/port/path or malformed-IP values before LAN-auth classification, preserve valid DNS and IP hosts, and unwrap bracketed IPv6 for Node's listener. The OpenCode child-process variable and Google key mirroring are not copied: Piarium does not manage an OpenCode process, and Pi's ModelRuntime owns provider environment/auth discovery. Credentials are not duplicated into a second inferred environment layer. |
