...
| Work Status panel | `f2523d0c`, `222057ab`, `630ac299`, `d7231b6d`, `ee9c9d6f`, `fa1cd279`, `83c4c75f`, `5ef828f3` | Adopted as a Pi-native wide-chat panel. It projects `SessionSnapshot`, `SessionStats`, `PiSessionFeatureState`, current tool executions, Fleet RPC, MCP public status, and pinned branch entries. It does not restore OpenCode todo, goal, MCP, or quota stores. Upstream's later empty-panel recovery is structurally unnecessary here: Piarium has no per-section hide preferences, always renders its session section and header, and therefore cannot hide the control needed to restore hidden sections. |
