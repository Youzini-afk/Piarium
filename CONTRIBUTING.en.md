...
User-facing documentation defaults to Simplified Chinese. The root README, contribution guide,
security policy, and `packages/docs` site source are written in Chinese first. English and the
other locales are translations.

Keep factual behavior, commands, security guidance, and links synchronized across applicable
languages in the same change. Write new docs pages in Chinese first, then add `en/` and the other
locale mirrors. Every localized root document must begin with a language switcher so readers never
need to return to the repository index to change language.

## Before you begin

- Read the [Code of Conduct](CODE_OF_CONDUCT.en.md).
...
- Send vulnerabilities through the private process in [SECURITY.en.md](SECURITY.en.md). Do not publish
