...
const codeBin = process.env.PIARIUM_VSCODE_BIN || 'code';
const workspaceArg = process.argv[2] || process.env.PIARIUM_VSCODE_DEV_WORKSPACE || repoRoot;
const workspacePath = path.resolve(workspaceArg);

const resolveDevServerAddress = () => {
  const configured = process.env.PIARIUM_VSCODE_WEBVIEW_URL;
  if (!configured) {
