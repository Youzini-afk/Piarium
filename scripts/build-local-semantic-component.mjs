...
const architecture = process.env.PIARIUM_TARGET_ARCH || process.arch;
...
const artifact = path.join(outputDirectory, `Piarium-local-semantic-${version}-${process.platform}-${architecture}.tar.gz`);
const staging = await fsp.mkdtemp(path.join(os.tmpdir(), 'piarium-local-semantic-build-'));
...
try {
  ...
  const check = `
    ...
    const result = await extract(['Piarium optional local inference'], { pooling: 'mean', normalize: true });
