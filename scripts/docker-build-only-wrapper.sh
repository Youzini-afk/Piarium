...
print_build_only_notice() {
  cat >&2 <<'EOF'
Varin Docker build-only mode is active.
...
print_version() {
  cat <<'EOF'
Docker build-only compatibility wrapper for Varin
...
reject_command() {
  ...
  cat >&2 <<EOF
Docker command '${command_name}' is disabled in this Varin container.
...
run_build() {
  ...
  export XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/tmp/varin-buildkit-${UID}}"
  ...
  # start buildkitd directly as the non-root varin user when ROOTLESSKIT is
