...
工作台介面。它直接使用 Pi 的公開 SDK，不擷取終端輸出，也不保留永久的 OpenCode 相容層。
...
以下截圖使用隔離的 `demo-workspace` 與匿名範例檔案產生，不包含個人帳號、真實專案、對話或憑證。
...
事件、設定檔案與能力協定——涵蓋子智慧體集群、上下文管理、工作區歷史、MCP 服務、Web 存取、
