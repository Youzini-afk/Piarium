...
- **原生的受影響檔案還原：** 對話回退沿用 Pi 的附加式會話樹；可替換的
  `piarium.workspace-recovery@5` Host 服務負責聯合還原、檢查點、復原/重做、保留策略和崩潰恢復，
  無需掃描整個工作區。
...
| `pi-workspace-history` | 可選且獨立的工作區歷史命令和原生設定；Piarium 還原不會呼叫它 |
