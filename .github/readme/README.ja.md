...
- **主要プラグインの専用設定画面：** メンテナンス対象のプラグインには目的に沿った GUI がありますが、
  ...
  ロジックのままです。
- **変更対象ファイルに限定したネイティブ復元：** 会話のロールバックは Pi の追記専用セッションツリーに
  従います。置き換え可能な `piarium.workspace-recovery@5` Host サービスが、ワークスペース全体を走査せずに、
  会話とファイルの復元、チェックポイント、undo/redo、保持、クラッシュ復旧を管理します。
...
| `pi-workspace-history` | 任意の独立したワークスペース履歴コマンドとネイティブ設定。Piarium の復元処理からは使用しません |
