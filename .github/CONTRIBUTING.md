...
面向用户的文档默认使用简体中文：仓库首页 README、贡献指南、安全策略，以及 `packages/docs`
文档站源码都以中文为源语言。英文和其他语种是译本。

当事实行为、命令、安全建议或链接发生变化时，应在同一次改动中同步适用的语言版本。新的文档页
先写中文，再补齐 `en/` 及其他语种译本。每份本地化根文档都必须以语言切换导航开头，让读者
无需返回仓库首页即可切换语言。
...
- 在 [GitHub Issues](https://github.com/Youzini-afk/Piarium/issues) 中报告可复现的缺陷、提出功能
  建议或展开聚焦的技术讨论。
- 漏洞请通过[安全策略](SECURITY.md)中的私密流程报告。不要把利用细节发布到 Issue、
...
4. **有依据地保留能力。** 维护者的 OpenChamber fork 是只读参考。其优秀的工作区、提供商、云端、
   远程、会话和安全能力应继续保留，除非 Pi 原生替代已经证明等价，或产品决策明确改变了行为。
5. **在可信边界执行权限校验。** 渲染器和远程客户端不能自行授权。文件系统、进程、网络、项目
   信任和凭据操作，必须由真正拥有该能力的宿主验证。
6. **不要添加武断的产品限制。** 避免静默截断、模型数量上限、过短超时和隐藏的并发上限。运维预算
