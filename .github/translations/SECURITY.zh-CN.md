[English](https://github.com/Youzini-afk/Varin/blob/main/.github/SECURITY.md) | 简体中文

# Varin 安全策略

Varin 会接触源码、终端权限、Git 与 SSH 操作、模型提供商凭据、Pi 会话历史、扩展配置和远程连接。
...
Varin 目前仍处于 1.0 之前的开发阶段，不维护 LTS 分支。
...
1. 当仓库向报告者开放时，优先使用 GitHub 的
   [私密漏洞报告表单](https://github.com/Youzini-afk/Varin/security/advisories/new)。
2. 如果 GitHub 没有显示私密表单，请创建一个
   [最小联系 Issue](https://github.com/Youzini-afk/Varin/issues/new?title=Private%20security%20report%20requested)，
...
Varin 目前不提供有合同约束力的响应 SLA，也没有漏洞赏金计划。维护者会尽快确认内容完整的报告、
...
Pi 或第三方扩展本身的漏洞通常应报告给对应项目。但如果 Varin 的集成绕过了自己的信任边界、在
批准前执行代码、暴露了超出界面声明能力的数据，或阻止上游修复生效，仍请同时报告给 Varin。

## 信任模型与预期行为

判断某项行为是否属于 Varin 漏洞时，需要理解以下事实：
...
- **提供商和研究服务是外部数据接收方。** 将用户批准的提示词或工具请求发送给所选提供商属于预期
  行为；发送无关工作区数据或凭据则不是。
- **本机管理员控制操作系统。** Varin 不试图保护数据免受能够读取用户文件和进程内存的系统
  管理员访问。
...
- 未设置强随机 `VARIN_UI_PASSWORD` 时，不要让 Web 服务监听在 loopback 之外。
- 流量离开本机时，应使用可信 TLS 反向代理或经过审核的隧道。
- 将每个 Pi 包和项目内可执行资源都视为代码；批准前检查其来源和请求的能力。
- 及时更新 Varin、Pi、已维护扩展和宿主操作系统。
- 将 Varin 持久化数据、工作区、SSH 材料和部署环境文件放在不可变发布目录之外，并设置合适的
  文件系统权限。
