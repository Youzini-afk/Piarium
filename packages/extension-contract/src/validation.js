import { PIARIUM_EXTENSION_CATALOG_SCHEMA_VERSION, PIARIUM_EXTENSION_MANIFEST_SCHEMA_VERSION, } from "./types.js";
import { PIARIUM_WORKBENCH_REPLACEMENT_TARGETS, parsePiariumWorkbenchProfileSnapshot, parsePiariumWorkbenchShellContributionData, PiariumWorkbenchShellContractError, } from "./workbench.js";
import { parsePiariumTransitionSceneContributionData, PiariumTransitionSceneContractError, } from "./motion.js";
import { parsePiariumExtensionServiceRoutingContext, parsePiariumExtensionServiceRoutingSnapshot, } from "./service-routing.js";
import { isPiariumContributionCompatible, } from "./compatibility.js";
import { parsePiariumContextExpression, PiariumContextExpressionError, } from "./context-expression.js";
...
function positiveRevision(value, path, issues, allowZero = false) {
...
function integrity(value, path, issues) {
    const normalized = text(value);
...
function entryPath(value, path, issues) {
    const normalized = text(value);
...
function uniqueStrings(value, path, issues, id = false) {
    if (value === undefined)
        return [];
...
function jsonValue(value, path, issues) {
    ...
    if (typeof value === "number" && Number.isFinite(value))
        return value;
    if (Array.isArray(value))
        return value.map((item, index) => jsonValue(item, `${path}[${index}]`, issues));
    if (isRecord(value)) {
        const result = {};
        for (const [key, item] of Object.entries(value))
            result[key] = jsonValue(item, `${path}.${key}`, issues);
        return result;
    }
    issues.push(`${path} must contain only JSON values`);
    return null;
}
function activation(value, path, issues) {
    if (value === undefined)
        return undefined;
    const values = uniqueStrings(value, path, issues);
    const result = [];
    for (const item of values) {
        if (!ACTIVATION_EVENTS.has(item))
            issues.push(`${path} contains unsupported event ${item}`);
        else
            result.push(item);
    }
    return result;
}
function entryFile(value, path, issues, required) {
    const normalized = text(value);
    if (!normalized) {
        if (required)
    ...
    if (!ENTRY_PATH_PATTERN.test(normalized))
        issues.push(`${path} must be a relative forward-slash path without parent traversal`);
    return normalized;
}
function parseHostEntrypoint(value, path, issues) {
    if (value === undefined)
        return undefined;
    if (!isRecord(value)) {
    ...
    if (!mode || !HOST_MODES.has(mode))
        issues.push(`${path}.mode must be brokered or native`);
...
function parseSurfaceEntrypoints(value, path, issues) {
    if (value === undefined)
        return undefined;
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
    ...
    value.forEach((raw, index) => {
        ...
        if (ids.has(id))
            issues.push(`${path} contains duplicate entrypoint ${id}`);
        ids.add(id);
        const mode = text(raw.mode);
        if (!mode || !SURFACE_MODES.has(mode))
            issues.push(`${itemPath}.mode is unsupported`);
        ...
        for (const surface of rawSupports) {
            if (!SURFACES.has(surface))
                issues.push(`${itemPath}.supports contains unsupported surface ${surface}`);
            else
                supports.push(surface);
        }
        if (supports.length === 0)
            issues.push(`${itemPath}.supports must contain at least one surface`);
        const activationEvents = activation(raw.activation, `${itemPath}.activation`, issues);
        const isolation = text(raw.isolation);
        if (isolation !== undefined && mode !== "isolated")
            issues.push(`${itemPath}.isolation is only valid for isolated entrypoints`);
        ...
        result.push({
            id,
            mode: SURFACE_MODES.has(mode ?? "") ? mode : "managed",
            supports,
...
function parseServices(value, path, issues, kind) {
    if (value === undefined)
        return undefined;
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
    ...
    value.forEach((raw, index) => {
        ...
        if (keys.has(key))
            issues.push(`${path} contains duplicate service ${key}`);
        keys.add(key);
        const flagKey = kind === "provide" ? "multiple" : "optional";
        const flag = raw[flagKey];
        if (flag !== undefined && typeof flag !== "boolean")
            issues.push(`${itemPath}.${flagKey} must be boolean`);
        const binding = kind === "require" && (raw.binding === "all" || raw.binding === "selected" || raw.binding === "single")
            ? raw.binding
            : undefined;
        if (kind === "require" && raw.binding !== undefined && !binding)
            issues.push(`${itemPath}.binding must be all, selected, or single`);
        result.push({
            id,
            version,
...
function parseContributions(value, path, issues) {
    if (value === undefined)
        return undefined;
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
    ...
    value.forEach((raw, index) => {
        ...
        const id = identifier(raw.id, `${itemPath}.id`, issues);
        if (ids.has(id))
            issues.push(`${path} contains duplicate contribution ${id}`);
        ids.add(id);
        const kind = text(raw.kind);
        if (!kind || !CONTRIBUTION_KINDS.has(kind))
            issues.push(`${itemPath}.kind is unsupported`);
        const data = isRecord(raw.data) ? jsonValue(raw.data, `${itemPath}.data`, issues) : {};
        if (!isRecord(raw.data))
            issues.push(`${itemPath}.data must be an object`);
        ...
        const kindKnown = kind !== undefined && CONTRIBUTION_KINDS.has(kind);
        const versionCompatible = kindKnown
            && isPiariumContributionCompatible(kind, contractVersion);
        ...
        if (raw.placement !== undefined) {
            if (!isRecord(raw.placement)) {
                ...
                if (order !== undefined && (typeof order !== "number" || !Number.isFinite(order)))
                    issues.push(`${itemPath}.placement.order must be finite`);
        ...
        if (raw.replacement !== undefined) {
            if (!isRecord(raw.replacement)) {
                ...
                if (priority !== undefined && (typeof priority !== "number" || !Number.isFinite(priority)))
                    issues.push(`${itemPath}.replacement.priority must be finite`);
                replacement = {
                    target,
        ...
        result.push({
            id,
            kind: CONTRIBUTION_KINDS.has(kind) ? kind : "page",
            contractVersion,
            data,
            supports,
...
function throwIssues(label, issues) {
    if (issues.length > 0)
        throw new PiariumExtensionContractError(`${label} is invalid`, issues);
}
function hostId(value, path, issues) {
    const normalized = text(value);
...
export function parsePiariumExtensionManifest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension manifest is invalid", ["manifest must be an object"]);
    if (value.schemaVersion !== PIARIUM_EXTENSION_MANIFEST_SCHEMA_VERSION)
        issues.push("schemaVersion must be 1");
    const id = identifier(value.id, "id", issues);
    const version = text(value.version) ?? "0.0.0";
    if (!SEMVER_PATTERN.test(version))
        issues.push("version must be a SemVer version");
    ...
    if (!text(rawEngines.piarium))
        issues.push("engines.piarium must be a non-empty compatibility range");
    else if (semver.validRange(piariumEngine) === null)
        issues.push("engines.piarium must be a valid SemVer range");
    const rawEntrypoints = value.entrypoints;
    if (rawEntrypoints !== undefined && !isRecord(rawEntrypoints))
    ...
    if (value.requires !== undefined && !isRecord(value.requires))
        issues.push("requires must be an object");
    const requiredServices = parseServices(rawRequires.services, "requires.services", issues, "require");
    const rawProvides = isRecord(value.provides) ? value.provides : {};
    if (value.provides !== undefined && !isRecord(value.provides))
        issues.push("provides must be an object");
    const providedServices = parseServices(rawProvides.services, "provides.services", issues, "provide");
    ...
    if (value.integrates !== undefined && !isRecord(value.integrates))
        issues.push("integrates must be an object");
    ...
    if (rawMetadata.description !== undefined && !metadataDescription)
        issues.push("metadata.description must be a non-empty string");
    if (rawMetadata.homepage !== undefined && !metadataHomepage)
        issues.push("metadata.homepage must be a non-empty string");
    if (rawMetadata.repository !== undefined && !metadataRepository)
        issues.push("metadata.repository must be a non-empty string");
    let storageSchemaVersion;
    if (value.storage !== undefined) {
        if (!isRecord(value.storage))
            issues.push("storage must be an object");
        else
            storageSchemaVersion = positiveRevision(value.storage.schemaVersion, "storage.schemaVersion", issues);
    }
    const surfaceById = new Map((surfaces ?? []).map((surface) => [surface.id, surface]));
    for (const contribution of contributions ?? []) {
        if (!contribution.id.startsWith(`${id}.`))
            issues.push(`contribution ${contribution.id} must be qualified by extension ID ${id}`);
        if (contribution.entrypoint) {
            const surface = surfaceById.get(contribution.entrypoint);
            if (!surface)
                issues.push(`contribution ${contribution.id} references unknown surface entrypoint ${contribution.entrypoint}`);
            else {
                for (const supported of contribution.supports) {
                    if (!surface.supports.includes(supported))
                        issues.push(`contribution ${contribution.id} supports ${supported} but entrypoint ${surface.id} does not`);
                }
            }
        }
        for (const capability of contribution.requiresCapabilities ?? []) {
            if (!surfaceCapabilities.includes(capability))
                issues.push(`contribution ${contribution.id} requires undeclared surface capability ${capability}`);
        }
    }
...
export function assertPiariumApplicationVersion(piariumVersion) {
...
export function assertPiariumExtensionManifestCompatibility(manifest, piariumVersion) {
...
function parseSource(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
    ...
    if (!display)
        issues.push(`${path}.display must be a non-empty string`);
    if (!specifier)
        issues.push(`${path}.specifier must be a non-empty string`);
    return {
        kind: SOURCE_KINDS.has(kind ?? "") ? kind : "local",
        display: display ?? "Invalid source",
        specifier: specifier ?? "invalid",
    };
}
function parseDesired(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
...
function parseGrants(value, path, issues) {
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
        return [];
    }
    ...
    value.forEach((raw, index) => {
        ...
        if (!realm)
            issues.push(`${itemPath}.realm must be host or surface`);
        const grantKey = `${realm ?? "invalid"}:${capability}`;
        if (seen.has(grantKey))
            issues.push(`${path} contains duplicate capability ${grantKey}`);
        seen.add(grantKey);
        if (typeof raw.granted !== "boolean")
            issues.push(`${itemPath}.granted must be boolean`);
        const manifestVersion = text(raw.manifestVersion) ?? "0.0.0";
        if (!SEMVER_PATTERN.test(manifestVersion))
            issues.push(`${itemPath}.manifestVersion must be SemVer`);
        result.push({
            capability,
...
const capabilityKey = (reference) => `${reference.realm}:${reference.capability}`;
function parseCapabilityReferences(value, path, issues) {
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
        return [];
    }
    ...
    return value.flatMap((raw, index) => {
        ...
        if (!realm)
            issues.push(`${itemPath}.realm must be host or surface`);
        const reference = { capability, realm: realm ?? "surface" };
        const key = capabilityKey(reference);
        if (seen.has(key))
            issues.push(`${path} contains duplicate capability ${key}`);
        seen.add(key);
        return [reference];
    });
}
function parseCapabilityDelta(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
...
function manifestCapabilityReferences(manifest) {
    return ["host", "surface"].flatMap((realm) => ((manifest.capabilities?.[realm] ?? []).map((capability) => ({ capability, realm }))));
}
function validateCandidateCapabilityReview(selectedManifest, candidateManifest, delta, grants, reviewed, path, issues) {
    ...
    if (JSON.stringify(actualAdded) !== JSON.stringify(expectedAdded))
        issues.push(`${path}.capabilityDelta.added does not match the manifest capability change`);
    if (JSON.stringify(actualRemoved) !== JSON.stringify(expectedRemoved))
        issues.push(`${path}.capabilityDelta.removed does not match the manifest capability change`);
    for (const grant of grants) {
        if (grant.manifestVersion !== candidateManifest.version)
            issues.push(`${path}.capabilityGrants must target the candidate manifest version`);
        if (!candidate.has(capabilityKey(grant)))
            issues.push(`${path}.capabilityGrants contains a capability not requested by the candidate manifest`);
    }
    const decisions = new Set(grants.map(capabilityKey));
    const completelyReviewed = expectedAdded.every((key) => decisions.has(key));
    if (reviewed !== completelyReviewed)
        issues.push(`${path}.capabilitiesReviewed does not match added-capability decisions`);
}
function parseCandidateRecord(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return undefined;
    }
    ...
    try {
        ...
        if (error instanceof PiariumExtensionContractError)
            issues.push(...error.issues.map((issue) => `${path}.manifest.${issue}`));
        manifest = { schemaVersion: 1, id: "invalid", version: "0.0.0", engines: { piarium: "*" } };
    }
    const resolvedVersion = text(value.resolvedVersion) ?? "0.0.0";
    if (!SEMVER_PATTERN.test(resolvedVersion))
        issues.push(`${path}.resolvedVersion must be SemVer`);
    if (resolvedVersion !== manifest.version)
        issues.push(`${path}.resolvedVersion must match manifest.version`);
    const resolvedPath = text(value.resolvedPath);
    if (!resolvedPath)
        issues.push(`${path}.resolvedPath must be a non-empty string`);
    if (typeof value.capabilitiesReviewed !== "boolean")
        issues.push(`${path}.capabilitiesReviewed must be boolean`);
    if (typeof value.applyRequested !== "boolean")
        issues.push(`${path}.applyRequested must be boolean`);
...
export function parsePiariumExtensionInstallationRecord(value, path = "installation") {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension installation is invalid", [`${path} must be an object`]);
    let manifest;
    try {
        manifest = parsePiariumExtensionManifest(value.manifest);
    }
    ...
    if (!SEMVER_PATTERN.test(resolvedVersion))
        issues.push(`${path}.resolvedVersion must be SemVer`);
    if (!SEMVER_PATTERN.test(selectedVersion))
        issues.push(`${path}.selectedVersion must be SemVer`);
    if (resolvedVersion !== manifest.version)
        issues.push(`${path}.resolvedVersion must match manifest.version`);
    if (selectedVersion !== resolvedVersion)
        issues.push(`${path}.selectedVersion must match resolvedVersion until candidate updates are supported`);
    ...
    if (candidate) {
        validateCandidateCapabilityReview(manifest, candidate.manifest, candidate.capabilityDelta, candidate.capabilityGrants, candidate.capabilitiesReviewed, `${path}.candidate`, issues);
    }
...
export function parsePiariumExtensionCatalogDocument(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension catalog is invalid", ["catalog must be an object"]);
    if (value.schemaVersion !== PIARIUM_EXTENSION_CATALOG_SCHEMA_VERSION)
        issues.push("schemaVersion must be 1");
        ...
        for (const [key, raw] of Object.entries(value.extensions)) {
            ...
            try {
                ...
                if (record.manifest.id !== id)
                    issues.push(`extensions.${key}.manifest.id must match its catalog key`);
...
export function parsePiariumExtensionHostIdentityDocument(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension host identity is invalid", ["identity must be an object"]);
    if (value.schemaVersion !== PIARIUM_EXTENSION_CATALOG_SCHEMA_VERSION)
        issues.push("schemaVersion must be 1");
...
function parseDiagnostic(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return undefined;
    }
    ...
    const severity = value.severity === "error" || value.severity === "info" || value.severity === "warning"
        ...
        : undefined;
    if (!code)
        issues.push(`${path}.code must be a non-empty string`);
    if (!message)
        issues.push(`${path}.message must be a non-empty string`);
    if (!severity)
        issues.push(`${path}.severity must be error, info, or warning`);
    ...
    if (value.realmId !== undefined && !realmId)
        issues.push(`${path}.realmId must be a non-empty string`);
    return {
        code: code ?? "invalid",
...
function parseDiagnostics(value, path, issues) {
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
        return [];
    }
...
function parseActualStates(value, path, issues) {
    if (!Array.isArray(value)) {
        issues.push(`${path} must be an array`);
        return [];
    }
    ...
    value.forEach((raw, index) => {
        ...
        if (!realmKind)
            issues.push(`${itemPath}.realmKind must be host or surface`);
        const realmId = text(raw.realmId);
        if (!realmId)
            issues.push(`${itemPath}.realmId must be a non-empty string`);
        const entrypointId = identifier(raw.entrypointId, `${itemPath}.entrypointId`, issues);
        const status = text(raw.status);
        if (!status || !ACTUAL_STATUSES.has(status))
            issues.push(`${itemPath}.status is unsupported`);
        const key = `${realmKind ?? "invalid"}:${realmId ?? "invalid"}:${entrypointId}`;
        if (keys.has(key))
            issues.push(`${path} contains duplicate actual state ${key}`);
        keys.add(key);
        states.push({
            ...
            entrypointId,
            status: ACTUAL_STATUSES.has(status) ? status : "failed",
...
function parsePublicCatalogEntry(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return undefined;
    }
    ...
    if (!sourceKind || !SOURCE_KINDS.has(sourceKind))
        issues.push(`${path}.source.kind is unsupported`);
    const sourceDisplay = text(source.display);
    if (!sourceDisplay)
        issues.push(`${path}.source.display must be a non-empty string`);
    const resolvedVersion = text(value.resolvedVersion) ?? "0.0.0";
    const selectedVersion = text(value.selectedVersion) ?? "0.0.0";
    if (!SEMVER_PATTERN.test(resolvedVersion))
        issues.push(`${path}.resolvedVersion must be SemVer`);
    ...
    if (resolvedVersion !== manifest.version)
        issues.push(`${path}.resolvedVersion must match manifest.version`);
    if (selectedVersion !== resolvedVersion)
        issues.push(`${path}.selectedVersion must match resolvedVersion`);
    const selectedIntegrity = value.integrity === undefined
        ? undefined
        : integrity(value.integrity, `${path}.integrity`, issues);
    let candidate;
    if (value.candidate !== undefined) {
        if (!isRecord(value.candidate)) {
            ...
            if (!isRecord(value.candidate.source))
                issues.push(`${path}.candidate.source must be an object`);
            const candidateKind = text(candidateSource.kind);
            const candidateDisplay = text(candidateSource.display);
            if (!candidateKind || !SOURCE_KINDS.has(candidateKind))
                issues.push(`${path}.candidate.source.kind is unsupported`);
            if (!candidateDisplay)
                issues.push(`${path}.candidate.source.display must be a non-empty string`);
            const candidateVersion = text(value.candidate.resolvedVersion) ?? "0.0.0";
            if (!SEMVER_PATTERN.test(candidateVersion))
                issues.push(`${path}.candidate.resolvedVersion must be SemVer`);
            if (candidateVersion !== candidateManifest.version)
                issues.push(`${path}.candidate.resolvedVersion must match candidate manifest.version`);
            if (candidateManifest.id !== manifest.id)
                issues.push(`${path}.candidate.manifest.id must match selected manifest.id`);
            ...
            const candidateDelta = parseCapabilityDelta(value.candidate.capabilityDelta, `${path}.candidate.capabilityDelta`, issues);
            const candidateGrants = parseGrants(value.candidate.capabilityGrants, `${path}.candidate.capabilityGrants`, issues);
            validateCandidateCapabilityReview(manifest, candidateManifest, candidateDelta, candidateGrants, value.candidate.capabilitiesReviewed === true, `${path}.candidate`, issues);
            candidate = {
    ...
    return {
        manifest,
        source: {
            kind: SOURCE_KINDS.has(sourceKind ?? "") ? sourceKind : "local",
...
export function parsePiariumExtensionCatalogSnapshot(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension catalog snapshot is invalid", ["snapshot must be an object"]);
    if (value.schemaVersion !== PIARIUM_EXTENSION_CATALOG_SCHEMA_VERSION)
        issues.push("schemaVersion must be 1");
    const storageState = text(value.storageState);
    if (!storageState || !STORAGE_STATES.has(storageState))
        issues.push("storageState is unsupported");
    if (typeof value.authoritative !== "boolean")
        issues.push("authoritative must be boolean");
    if (storageState === "stale" && value.authoritative !== false)
        issues.push("a stale snapshot cannot be authoritative");
    const extensions = [];
    const ids = new Set();
    if (!Array.isArray(value.extensions)) {
        ...
        value.extensions.forEach((raw, index) => {
            ...
            if (ids.has(entry.manifest.id))
                issues.push(`extensions contains duplicate extension ${entry.manifest.id}`);
    ...
    for (const entry of extensions) {
        for (const actual of entry.actual) {
            if (actual.hostId !== parsedHostId)
                issues.push(`actual state for ${entry.manifest.id} belongs to another host`);
            if (actual.desiredRevision !== entry.desired.revision)
                issues.push(`actual state for ${entry.manifest.id} has a stale desired revision`);
        }
    }
...
export function parsePiariumExtensionCatalogAvailability(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension catalog response is invalid", ["response must be an object"]);
    ...
    if (value.supported !== true)
        issues.push("supported must be boolean");
    if (value.status === "ready") {
        ...
        throwIssues("Piarium extension catalog response", issues);
        return { supported: true, status: "ready", snapshot: snapshot };
    }
    if (value.status === "error") {
        ...
        if (!message)
            issues.push("error.message must be a non-empty string");
        if (typeof rawError.retryable !== "boolean")
            issues.push("error.retryable must be boolean");
        throwIssues("Piarium extension catalog response", issues);
        return {
            supported: true,
            status: "error",
            error: { code: code, message: message, retryable: rawError.retryable === true },
        };
    }
...
export function isPiariumExtensionId(value) {
    return ID_PATTERN.test(value);
}
const BASE64_PATTERN = /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/;
function parseArtifactSlot(value, path, issues) {
...
export function parsePiariumExtensionAssetRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension asset request is invalid", ["request must be an object"]);
    const result = {
...
export function parsePiariumExtensionManagedEntrypointRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension entrypoint request is invalid", ["request must be an object"]);
    const result = {
...
function parseAssetPayload(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return {
    ...
    if (!contentType)
        issues.push(`${path}.contentType must be a non-empty string`);
    return {
...
export function parsePiariumExtensionAssetPayload(value) {
    const issues = [];
    const result = parseAssetPayload(value, "asset", issues);
    throwIssues("Piarium extension asset payload", issues);
    return result;
}
export function parsePiariumExtensionManagedEntrypointPayload(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension entrypoint payload is invalid", ["payload must be an object"]);
...
export function parsePiariumExtensionCandidateSelectionRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension candidate selection request is invalid", ["request must be an object"]);
    const result = {
...
export function parsePiariumExtensionCandidateCapabilityReviewRequest(value) {
    const issues = [];
    if (!isRecord(value)) {
...
function parseCapabilityDecisions(value, path, issues) {
    const decisions = Array.isArray(value) ? value.flatMap((raw, index) => {
        ...
        if (typeof raw.granted !== "boolean")
            issues.push(`${itemPath}.granted must be boolean`);
        const normalizedRealm = realm ?? "surface";
        return [{ capability, granted: raw.granted === true, realm: normalizedRealm }];
    }) : (issues.push(`${path} must be an array`), []);
    const keys = decisions.map(capabilityKey);
    if (new Set(keys).size !== keys.length)
        issues.push(`${path} contains duplicate capabilities`);
    return decisions;
}
export function parsePiariumExtensionCapabilityReviewRequest(value) {
    const issues = [];
    if (!isRecord(value)) {
...
export function parsePiariumExtensionPackageSource(value) {
    const issues = [];
    const result = parseSource(value, "source", issues);
    throwIssues("Piarium extension package source", issues);
    return result;
}
export function parsePiariumExtensionPackageInstallRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension install request is invalid", ["request must be an object"]);
    const result = {
        expectedRevision: positiveRevision(value.expectedRevision, "expectedRevision", issues, true),
...
export function parsePiariumExtensionLocalSourceReloadRequest(value) {
    const issues = [];
    if (!isRecord(value)) {
...
export function parsePiariumExtensionLocalSourceReloadResult(value) {
    const issues = [];
    if (!isRecord(value)) {
    ...
    if (!outcome)
        issues.push("outcome must be staged or unchanged");
    let snapshot;
    try {
        snapshot = parsePiariumExtensionCatalogSnapshot(value.snapshot);
    }
    ...
    if (outcome === "staged") {
        ...
        return { candidateIntegrity, outcome, snapshot: snapshot };
    }
    if (value.candidateIntegrity !== undefined)
        issues.push("candidateIntegrity is only valid for a staged reload");
    throwIssues("Piarium extension local source reload result", issues);
    return { outcome: "unchanged", snapshot: snapshot };
}
export function parsePiariumExtensionRemoveRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension remove request is invalid", ["request must be an object"]);
    const result = {
...
export function parsePiariumExtensionActualState(value) {
    const issues = [];
    const states = parseActualStates([value], "actual", issues);
    throwIssues("Piarium extension actual state", issues);
    return states[0];
}
function parseServiceProvider(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return undefined;
    }
    const descriptor = parseServices([value.descriptor], `${path}.descriptor`, issues, "provide")?.[0];
    const status = value.status === "active" || value.status === "candidate" || value.status === "draining" ? value.status : undefined;
    if (!status)
        issues.push(`${path}.status must be active, candidate, or draining`);
    const providerId = text(value.providerId);
    if (!providerId)
        issues.push(`${path}.providerId must be a non-empty string`);
    const providerKey = text(value.providerKey);
    if (!providerKey)
        issues.push(`${path}.providerKey must be a non-empty string`);
    const extensionVersion = text(value.extensionVersion) ?? "0.0.0";
    if (!SEMVER_PATTERN.test(extensionVersion))
        issues.push(`${path}.extensionVersion must be SemVer`);
    if (!descriptor)
        return undefined;
    return {
        descriptor,
...
export function parsePiariumExtensionCandidatePreparationResult(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension candidate preparation is invalid", ["result must be an object"]);
    const providers = Array.isArray(value.providers)
        ? value.providers.flatMap((provider, index) => {
            const parsed = parseServiceProvider(provider, `providers[${index}]`, issues);
            if (parsed && parsed.status !== "candidate")
                issues.push(`providers[${index}].status must be candidate`);
            return parsed ? [parsed] : [];
        })
        : (issues.push("providers must be an array"), []);
    const integrity = text(value.integrity);
    if (!integrity)
        issues.push("integrity must be a non-empty string");
    const result = {
        extensionId: identifier(value.extensionId, "extensionId", issues),
...
export function parsePiariumExtensionServiceCatalogSnapshot(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension service catalog is invalid", ["catalog must be an object"]);
    const providers = Array.isArray(value.providers)
        ? value.providers.flatMap((provider, index) => {
            const parsed = parseServiceProvider(provider, `providers[${index}]`, issues);
            return parsed ? [parsed] : [];
    ...
    for (const provider of providers) {
        if (providerIds.has(provider.providerId))
            issues.push(`providers contains duplicate provider ${provider.providerId}`);
        providerIds.add(provider.providerId);
    }
    const selections = {};
    if (!isRecord(value.selections))
        issues.push("selections must be an object");
    else {
        for (const [service, provider] of Object.entries(value.selections)) {
            const selected = text(provider);
            if (!selected)
                issues.push(`selections.${service} must be a non-empty provider ID`);
            else
...
export function parsePiariumExtensionServiceInvocationRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension service invocation is invalid", ["request must be an object"]);
    ...
    if (value.providerId !== undefined && !providerId)
        issues.push("providerId must be a non-empty string");
    let routing;
    if (value.routing !== undefined) {
        try {
            routing = parsePiariumExtensionServiceRoutingContext(value.routing, { allowEmpty: true });
        }
        catch (error) {
            issues.push(`routing.${error instanceof Error ? error.message : String(error)}`);
        }
    }
    const result = {
        args,
...
export function parsePiariumExtensionServiceSelectionRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension service selection is invalid", ["request must be an object"]);
    const providerId = value.providerId === null ? null : text(value.providerId);
    if (value.providerId !== null && !providerId)
        issues.push("providerId must be a non-empty string or null");
    const result = {
...
export function parsePiariumExtensionHostStateWaitRequest(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension host-state wait request is invalid", ["request must be an object"]);
    const result = {
        hostId: hostId(value.hostId, "hostId", issues),
        revision: positiveRevision(value.revision, "revision", issues, true),
    };
...
export function parsePiariumExtensionHostStateSnapshot(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension host-state snapshot is invalid", ["snapshot must be an object"]);
    let catalog;
    let services;
    let routing;
    let workbench;
    try {
        catalog = parsePiariumExtensionCatalogSnapshot(value.catalog);
    }
    catch (error) {
        if (error instanceof PiariumExtensionContractError)
            issues.push(...error.issues.map((issue) => `catalog.${issue}`));
        else
            throw error;
    }
    try {
        services = parsePiariumExtensionServiceCatalogSnapshot(value.services);
    }
    catch (error) {
        if (error instanceof PiariumExtensionContractError)
            issues.push(...error.issues.map((issue) => `services.${issue}`));
        else
            throw error;
    }
    try {
        routing = parsePiariumExtensionServiceRoutingSnapshot(value.routing);
    }
    catch (error) {
        issues.push(`routing.${error instanceof Error ? error.message : String(error)}`);
    }
    try {
        workbench = parsePiariumWorkbenchProfileSnapshot(value.workbench);
    }
    catch (error) {
        issues.push(`workbench.${error instanceof Error ? error.message : String(error)}`);
    }
    if (catalog && services && catalog.hostId !== services.hostId)
        issues.push("catalog and services must belong to the same application host");
    if (catalog && routing && catalog.hostId !== routing.hostId)
        issues.push("catalog and routing must belong to the same application host");
    if (catalog && workbench && catalog.hostId !== workbench.hostId)
        issues.push("catalog and workbench must belong to the same application host");
    const revision = positiveRevision(value.revision, "revision", issues, true);
...
export function parsePiariumExtensionStorageOpenRequest(value) {
    const issues = [];
    if (!isRecord(value)) {
        throw new PiariumExtensionContractError("Piarium extension storage open request is invalid", ["request must be an object"]);
    }
    if (value.extensionId !== undefined)
        issues.push("extensionId is assigned by the Piarium Host and must not be supplied");
    const key = text(value.key);
    if (!key)
        issues.push("key must be a non-empty string");
    const scope = text(value.scope);
    if (!scope || !STORAGE_SCOPES.has(scope))
        issues.push("scope is unsupported");
    const schemaVersion = value.schemaVersion === undefined
        ? undefined
        : positiveRevision(value.schemaVersion, "schemaVersion", issues, true);
    const result = {
        key: key ?? "invalid",
        scope: STORAGE_SCOPES.has(scope ?? "") ? scope : "application",
        ...(schemaVersion !== undefined ? { schemaVersion } : {}),
    };
    throwIssues("Piarium extension storage open request", issues);
    return result;
}
export function parsePiariumExtensionStorageAddress(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension storage address is invalid", ["address must be an object"]);
    const key = text(value.key);
    if (!key)
        issues.push("key must be a non-empty string");
    const scope = text(value.scope);
...
function parseStorageDocument(value, path, issues) {
    if (!isRecord(value)) {
        issues.push(`${path} must be an object`);
        return { data: {}, revision: 0, schemaVersion: 0, updatedAt: new Date(0).toISOString() };
    }
    const data = isRecord(value.data) ? jsonValue(value.data, `${path}.data`, issues) : {};
...
export function parsePiariumExtensionStorageSnapshot(value) {
    const issues = [];
    if (!isRecord(value))
        throw new PiariumExtensionContractError("Piarium extension storage snapshot is invalid", ["snapshot must be an object"]);
    ...
    if (typeof value.exists !== "boolean")
        issues.push("exists must be boolean");
    if (storageState === "stale" && value.authoritative !== false)
        issues.push("stale storage cannot be authoritative");
    const result = {
        address: (() => {
            try {
                return parsePiariumExtensionStorageAddress(value.address);
            }
            catch (error) {
                if (error instanceof PiariumExtensionContractError)
                    issues.push(...error.issues.map((issue) => `address.${issue}`));
