...
The material below is laid out as: the previous summary S0 (if present), the history your summary will replace (A), a boundary marker, then the recent original text that stays verbatim (B). Messages appended after this task was frozen stay after B and are handled by the session system; do not cover them.
...
- B stays verbatim; do not copy it into the summary. Use it to understand where the work stopped and add only the background needed to interpret it.
...
- The material may begin with an elision notice naming the oldest entries of the replaced range that were omitted to fit this task's window. That prefix is still part of the range your summary replaces: read it with the history tool before writing, and never guess its contents.
...
/** Head-of-material notice when the oldest part of A was paged out. */
const elisionNotice = (throughEntryId: string): string =>
  `[Material notice — the oldest entries of the replaced range, up to and including entry ${throughEntryId}, were omitted here so this task fits the model window. They are still part of the range your summary replaces and remain in this session's authorized history: read them with the history tool (entry ${throughEntryId} and earlier) before writing the summary.]`;

const INSTRUCTION = `Write the continuation summary now. It replaces the previous summary and the replaced range above the marker; everything below the marker stays verbatim.`;
...
export function compactionMaterialMessages(spec: CompactionTaskSpec): AgentMessage[] {
  ...
  if (spec.elidedSummarizedThroughEntryId !== undefined) {
    messages.push(markerMessage(elisionNotice(spec.elidedSummarizedThroughEntryId)));
  }
