...
export const VARIN_HARNESS_FLEET_PROVIDER_ID = "varin-harness";
...
const projectEntry = (thread: ThreadListItem): PiFleetEntry => {
  ...
  return {
    ...
    name: thread.preset ?? thread.id,
    providerId: VARIN_HARNESS_FLEET_PROVIDER_ID,
    ...(thread.preset ? { role: thread.preset } : {}),
...
export class VarinHarnessFleetAdapter implements FleetProviderAdapter {
  readonly id = VARIN_HARNESS_FLEET_PROVIDER_ID;
  ...
  async action(request: PiFleetProviderActionRequest): Promise<PiFleetProviderActionResult> {
    ...
    throw new HostError("invalid_params", `Unknown Varin thread action ${request.action}`);
  }
}
