...
import { VarinHarnessFleetAdapter } from "../../src/fleet/varin-harness-adapter.js";

describe("Varin harness Fleet adapter", () => {
  it("projects Host-owned threads and routes transcript/kill actions", async () => {
    ...
    const adapter = new VarinHarnessFleetAdapter(bridge as never);
    ...
    assert.equal(status.entries[0]?.providerId, "varin-harness");
