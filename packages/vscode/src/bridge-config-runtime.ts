  ...
  switch (type) {
    ...
    case 'api:openagent:config:save': {
      const request = (payload || {}) as {
        ...
        disabled_mcps?: unknown[];
        disabled_providers?: unknown[];
        mcp_env_allowlist?: unknown[];
        default_mode?: unknown;
        hashline_edit?: unknown;
      ...
      try {
        saveOpenAgentConfig({
          ...
          disabled_commands: request.disabled_commands,
          disabled_tools: request.disabled_tools,
          disabled_mcps: request.disabled_mcps,
          disabled_providers: request.disabled_providers,
          mcp_env_allowlist: request.mcp_env_allowlist,
          default_mode: request.default_mode,
          hashline_edit: request.hashline_edit,
          model_fallback: request.model_fallback,
          runtime_fallback: request.runtime_fallback,
          background_task: request.background_task,
    ...
    case 'api:magic-context:config:save': {
      ...
      try {
        saveMagicContextConfig({
          ...
          sourceMtimeMs: request.sourceMtimeMs ?? null,
          directory: workingDirectory,
          config: request.config ?? {},
        });
        await ctx?.manager?.restart();
