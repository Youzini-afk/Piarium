...
const fetchGoogleQuota = async (): Promise<ProviderResult> => {
  ...
  for (const source of authSources) {
    ...
    if (!accessToken || (typeof source.expires === 'number' && source.expires <= now)) {
      ...
      const oauthClient = resolveGoogleOAuthClient(source.sourceId);
      if (!oauthClient) {
        const env = GOOGLE_OAUTH_CLIENT_ENV[source.sourceId];
        sourceErrors.push(`${source.sourceLabel}: Set ${env.clientId} and ${env.clientSecret} to refresh OAuth`);
        continue;
      }
