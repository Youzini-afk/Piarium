...
const fixture = (manifestName, artifactName, fields) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'piarium-manifest-test-'));
...
for (const [manifestName, artifactName] of [
  ['latest-linux.yml', 'Piarium-1.15.0-linux-x86_64.AppImage'],
  ['latest-linux-arm64.yml', 'Piarium-1.15.0-linux-arm64.AppImage'],
]) {
...
test('accepts electron-builder field ordering and optional blockMapSize', () => {
  const artifactName = 'Piarium-1.15.0-linux-x86_64.AppImage';
  const bytes = Buffer.from(`artifact:${artifactName}`);
...
test('rejects a manifest that points at the other architecture artifact', () => {
  const value = fixture('latest-linux-arm64.yml', 'Piarium-1.15.0-linux-arm64.AppImage');
  try {
    const x64Artifact = path.join(value.root, 'Piarium-1.15.0-linux-x86_64.AppImage');
