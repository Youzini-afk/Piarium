...
1. Build both versions on the native target architecture. For N and N+1, set the
   ...
   ```bash
   PIARIUM_TARGET_ARCH=x64 PIARIUM_UPDATER_E2E_BUILD=1 bun run bundle:main
   PIARIUM_TARGET_ARCH=x64 node ./scripts/package.mjs --linux --x64 --publish=never
   ```

   Use `PIARIUM_TARGET_ARCH=arm64` and `--arm64` on an arm64 host. Keep the N and
...
`PIARIUM_E2E=1`, the loopback URL set by the harness, and the build-time marker.
