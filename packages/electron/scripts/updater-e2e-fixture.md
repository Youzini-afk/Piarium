...
1. Build both versions on the native target architecture. For N and N+1, set the
   ...
   ```bash
   VARIN_TARGET_ARCH=x64 VARIN_UPDATER_E2E_BUILD=1 bun run bundle:main
   VARIN_TARGET_ARCH=x64 node ./scripts/package.mjs --linux --x64 --publish=never
   ```

   Use `VARIN_TARGET_ARCH=arm64` and `--arm64` on an arm64 host. Keep the N and
...
2. Launch N against a loopback fixture containing N+1:
   ...
   bun run updater:e2e:fixture -- run \
     --arch x64 \
     --current /absolute/path/Varin-N-linux-x86_64.AppImage \
     --next /absolute/path/Varin-N+1-linux-x86_64.AppImage \
     --version N+1 \
     --dir /tmp/varin-updater-e2e
   ```
...
`VARIN_E2E=1`, the loopback URL set by the harness, and the build-time marker.
Normal packages omit the build-time marker and always use `Youzini-afk/Varin`.
