...
export const LINUX_CLI_BY_APP_ID: Record<string, string> = {
  vscode: 'code',
  cursor: 'cursor',
  vscodium: 'codium',
  windsurf: 'windsurf',
...
export interface LinuxDiscoveryOptions {
...
const desktopBoolean = (value: unknown): boolean => String(value || '').trim().toLowerCase() === 'true';
const unescapeDesktopValue = (value: unknown): string => String(value || '')
  .replace(/\\s/g, ' ')
  .replace(/\\n/g, '\n')
  .replace(/\\t/g, '\t')
  .replace(/\\r/g, '\r')
  .replace(/\\\\/g, '\\');
const normalizeComparable = (value: unknown): string => String(value || '')
  .toLowerCase()
  .replace(/\.desktop$/i, '')
  .replace(/[^a-z0-9]+/g, ' ')
  .trim();
...
export const linuxApplicationDirs = ({ env = process.env, homeDir = os.homedir() }: LinuxDiscoveryOptions = {}): string[] => {
...
const parseDesktopValues = (content: unknown): Map<string, string> => {
...
export const parseDesktopEntry = (content: unknown, filePath = ''): LinuxDesktopEntry | null => {
...
const collectDesktopFiles = async (dir: string): Promise<string[]> => {
  let entries;
  try {
...
export const readLinuxDesktopEntries = async (options: LinuxDiscoveryOptions = {}): Promise<LinuxDesktopEntry[]> => {
...
export const desktopEntryMatchesApp = (entry: LinuxDesktopEntry, appName: unknown, appId: unknown = ''): boolean => {
...
const parseExecCommand = (exec: unknown): string[] => {
  const args: string[] = [];
  let current = '';
...
export const buildCommandFromDesktopExec = (entry: LinuxDesktopEntry, targetPath: string): { args: string[]; program: string } | null => {
...
const commandExists = (program: string, env: NodeJS.ProcessEnv = process.env): boolean => {
...
const findEntry = (entries: LinuxDesktopEntry[], appId: string, appName: string): LinuxDesktopEntry | null => entries.find((entry) => desktopEntryMatchesApp(entry, appName, appId)) || null;

const isTerminalEmulatorEntry = (entry: LinuxDesktopEntry): boolean => {
...
export const filterLinuxInstalledApps = async (apps: unknown, options: LinuxDiscoveryOptions = {}): Promise<string[]> => {
...
const pathExistsSync = (candidate: string): boolean => {
  try {
...
export const linuxIconThemeDirs = ({ env = process.env, homeDir = os.homedir() }: LinuxDiscoveryOptions = {}): string[] => {
  const dataHome = typeof env.XDG_DATA_HOME === 'string' && env.XDG_DATA_HOME.trim()
    ? env.XDG_DATA_HOME.trim()
    : path.join(homeDir || os.homedir(), '.local', 'share');
  const dataDirs = typeof env.XDG_DATA_DIRS === 'string' && env.XDG_DATA_DIRS.trim()
...
const listThemeNames = (iconsRoot: string): string[] => {
  let entries;
  try {
...
const lookForIconInTheme = (themeRoot: string, iconName: string): string | null => {
  let pngMatch: string | null = null;
  let svgMatch: string | null = null;
...
export const resolveLinuxIconFile = (iconName: unknown, options: LinuxDiscoveryOptions = {}): string | null => {
...
export const resolveDefaultLinuxFileManagerId = ({ env = process.env, execFileSyncImpl = execFileSync }: LinuxDiscoveryOptions = {}): string | null => {
  try {
...
export const findLinuxFileManagerEntry = (entries: LinuxDesktopEntry[], options: LinuxDiscoveryOptions = {}): LinuxDesktopEntry | null => {
...
const iconFileToDataUrl = (filePath: string | null): string | null => {
...
const resolveIconDataUrlForName = (iconNames: unknown[], options: LinuxDiscoveryOptions = {}): string | null => {
...
const isLinuxFileManagerName = (name: unknown): boolean => {
...
const knownLinuxAppIdForName = (name: unknown): string | null => {
  const normalized = normalizeComparable(name);
  ...
  if (knownIdByName.has(normalized)) return knownIdByName.get(normalized) ?? null;
...
export const buildLinuxInstalledApps = async (apps: unknown, options: LinuxDiscoveryOptions = {}): Promise<InstalledAppInfo[]> => {
  const entries = options.entries || await readLinuxDesktopEntries(options);
...
export const fetchLinuxAppIcons = async (apps: unknown[] = [], options: LinuxDiscoveryOptions = {}) => {
