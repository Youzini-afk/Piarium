...
import { PIARIUM_EXTENSION_MANIFEST_FILE, assertPiariumApplicationVersion, assertPiariumExtensionManifestCompatibility, isPiariumExtensionId, parsePiariumExtensionManifest, } from "@piarium/extension-contract";
import { createDefaultExtensionPackageSourceRegistry, resolveNpmLaunchTarget, runExtensionSourceCommand, } from "./package-sources.js";
const INDEX_FILE = "artifact-index.json";
const MANAGED_RUNTIME_DIRECTORY = "runtime/surface";
const buildModuleWithEsbuild = async (options) => {
...
const artifactIntegrityForIndex = (index) => sha256(canonicalJson(index));
const toLogicalPath = (value) => value.split(sep).join("/");
...
const isMissing = (error) => (typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT");
...
const assertSafeTree = async (root) => {
    const visit = async (directory) => {
        for (const entry of await readdir(directory, { withFileTypes: true })) {
            ...
            if (info.isDirectory())
                ...
                throw new Error(`Unsupported Piarium extension package entry: ${toLogicalPath(relative(root, path))}`);
        }
    };
    await visit(root);
};
const shouldCopyPackagePath = (sourceRoot, path) => {
    ...
    const visit = async (directory) => {
        for (const entry of await readdir(directory, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                ...
                files.push(toLogicalPath(relative(root, path)));
            else
                throw new Error(`Immutable Piarium extension artifact contains an unsupported entry: ${path}`);
        }
    };
    await visit(root);
    return files.sort();
...
const assertExactKeys = (record, expected, label) => {
...
const parseIndex = (value) => {
    if (!isRecord(value))
        throw new Error("Piarium extension artifact index is invalid");
    const hasHost = value.host !== undefined;
    assertExactKeys(value, hasHost
        ? ["artifactIntegrity", "entrypoints", "files", "host", "manifest", "schemaVersion"]
        : ["artifactIntegrity", "entrypoints", "files", "manifest", "schemaVersion"], "Piarium extension artifact index");
    ...
    for (const [entrypointId, rawEntrypoint] of Object.entries(value.entrypoints)) {
        ...
        const styles = rawEntrypoint.styles;
    ...
    if (hasHost) {
        if (!isRecord(value.host))
            throw new Error("Piarium extension artifact Host entrypoint record is invalid");
...
const readManifest = async (sourceRoot) => {
    const raw = JSON.parse(await readFile(join(sourceRoot, PIARIUM_EXTENSION_MANIFEST_FILE), "utf8"));
    return parsePiariumExtensionManifest(raw);
};
const hasDependencies = async (sourceRoot) => {
    try {
        const value = JSON.parse(await readFile(join(sourceRoot, "package.json"), "utf8"));
...
export class ExtensionArtifactStore {
    ...
    async builtinDistributionFingerprint(extensionId, fingerprintFile) {
        const sourceRoot = this.#builtinRoots.get(extensionId);
        if (!sourceRoot || !/^[A-Za-z0-9._-]+$/.test(fingerprintFile))
            return null;
        try {
    ...
    async builtinArtifactMatchesDistribution(input) {
        if (!/^[A-Za-z0-9._-]+$/.test(input.fingerprintFile)
            || !/^sha256-[0-9a-f]{64}$/.test(input.distributionFingerprint))
            return false;
        try {
            const selected = await this.readAsset(input.artifactRoot, input.artifactIntegrity, `package/${input.fingerprintFile}`, input.manifest);
        ...
        try {
            ...
            if (!(await exists(join(sourceRoot, "node_modules"))) && await hasDependencies(sourceRoot)) {
                if (source.kind === "local") {
                    throw new Error(`Local Piarium extension dependencies are not installed: ${source.display}. Run npm install in the extension project before reloading it. Piarium will not modify the working tree.`);
                }
            ...
            for (const entrypoint of manifest.entrypoints?.surfaces ?? []) {
                if (entrypoint.mode === "declarative")
                    continue;
                if (!entrypoint.file)
                    throw new Error(`Executable Surface entrypoint has no file: ${entrypoint.id}`);
                ...
                if (!result.metafile)
                    throw new Error(`Surface entrypoint build returned no metadata: ${entrypoint.id}`);
            ...
            const index = { artifactIntegrity, ...indexContent };
    ...
    async readAsset(artifactRoot, artifactIntegrity, logicalPath, expectedManifest) {
    ...
    async readManagedEntrypoint(artifactRoot, artifactIntegrity, entrypointId, expectedManifest) {
        const { index, root } = await this.#readVerifiedIndex(artifactRoot, artifactIntegrity, expectedManifest);
        const entrypoint = index.entrypoints[entrypointId];
        if (!entrypoint)
            throw new Error(`Managed Surface entrypoint is not present in the artifact: ${entrypointId}`);
        const readPayload = async (logicalPath) => {
            const { bytes, record } = await this.#readVerifiedFile(root, index, logicalPath);
            return {
                artifactIntegrity,
                bytesBase64: bytes.toString("base64"),
    ...
    async resolveBrokeredHostEntrypoint(artifactRoot, artifactIntegrity, expectedManifest) {
        const { index, root } = await this.#readVerifiedIndex(artifactRoot, artifactIntegrity, expectedManifest);
        if (!index.host)
            throw new Error("Host entrypoint is not present in the artifact");
    ...
    async #readVerifiedIndex(artifactRoot, artifactIntegrity, expectedManifest, verifyAllFiles = false) {
        const root = await this.#assertArtifactRoot(artifactRoot, artifactIntegrity);
        const index = parseIndex(JSON.parse(await readFile(join(root, INDEX_FILE), "utf8")));
        const indexContent = {
        ...
        const artifactManifest = parsePiariumExtensionManifest(JSON.parse(manifestBytes.toString("utf8")));
    ...
    async #readVerifiedFile(root, index, logicalPath) {
        const record = index.files[logicalPath];
        if (!record)
            throw new Error(`Piarium extension artifact does not contain asset: ${logicalPath}`);
        ...
        const info = await lstat(path);
        if (!info.isFile())
            throw new Error(`Piarium extension artifact asset is not an immutable file: ${logicalPath}`);
        const bytes = await readFile(path);
        if (sha256(bytes) !== record.integrity)
            throw new Error(`Piarium extension asset integrity failed: ${logicalPath}`);
        return { bytes, path, record };
    }
    async #assertArtifactRoot(artifactRoot, artifactIntegrity) {
        ...
        if (actualRoot !== actualExpected)
            throw new Error("Piarium extension artifact path does not match its content address");
        return actualRoot;
    }
}
