import { assertPiariumApplicationVersion, parsePiariumExtensionActualState, parsePiariumExtensionCandidateCapabilityReviewRequest, parsePiariumExtensionCandidateSelectionRequest, parsePiariumExtensionCapabilityReviewRequest, parsePiariumExtensionHostStateWaitRequest, parsePiariumExtensionLocalSourceReloadRequest, parsePiariumExtensionPackageInstallRequest, parsePiariumExtensionRemoveRequest, parsePiariumExtensionServiceInvocationRequest, parsePiariumExtensionServiceSelectionRequest, parsePiariumWorkbenchProfileApplyRequest, resolvePiariumExtensionServiceRouting, } from "@piarium/extension-contract";
import { PIARIUM_BUILTIN_EXTENSION_DEFINITIONS, PIARIUM_BUILTIN_EXTENSION_PREFIX, } from "@piarium/extension-builtins";
import { ApplicationExtensionCatalog } from "./application-catalog.js";
import { BrokeredHostSupervisor } from "./broker-supervisor.js";
...
export class ApplicationExtensionRuntime {
    ...
    constructor(options, hostId) {
        ...
        if (this.services.hostId !== hostId)
            throw new Error("Extension service registry belongs to another application host");
        ...
        if (this.routing.hostId !== hostId)
            throw new Error("Service routing store belongs to another application host");
        this.workbench = options.workbench ?? new WorkbenchProfileStore({ hostId, storage: this.storage });
        if (this.workbench.hostId !== hostId)
            throw new Error("Workbench profile store belongs to another application host");
    ...
    async start() {
        await this.#mutate(async () => {
            const snapshot = await this.catalog.reconcileBuiltins(PIARIUM_BUILTIN_EXTENSION_DEFINITIONS, PIARIUM_BUILTIN_EXTENSION_PREFIX);
    ...
    async state() {
        for (;;) {
            ...
            if (before === after)
                return { catalog, revision: after, routing, services, workbench };
        }
    }
    subscribe(listener) {
        if (this.#stopped)
            throw new Error("Application extension runtime is stopped");
        this.#listeners.add(listener);
        return () => this.#listeners.delete(listener);
    }
    async waitForState(requestValue, signal) {
        const request = parsePiariumExtensionHostStateWaitRequest(requestValue);
        const current = await this.state();
        if (request.hostId !== current.catalog.hostId || request.revision !== current.revision)
            return current;
        return new Promise((resolveWait, rejectWait) => {
            ...
            const onChange = () => {
                void this.state().then((next) => finish(() => resolveWait(next)), (error) => finish(() => rejectWait(error)));
            };
    ...
    installOrStage(requestValue, signal) {
    ...
    reloadLocalSource(requestValue, signal) {
    ...
    removeExtension(requestValue) {
        ...
        return this.#mutateCatalog(async () => {
            ...
            if (entry.source.kind === "builtin")
                throw new Error(`Built-in Piarium extensions are managed by the distribution: ${request.extensionId}`);
            if (entry.desired.enabled)
                throw new Error(`Disable the Piarium extension before removing it: ${request.extensionId}`);
            ...
            try {
                ...
                if (request.deleteData) {
                    try {
                        ...
                        throw new ExtensionStorageError("storage_write_failed", `Piarium extension ${request.extensionId} was removed, but its namespaced storage could not be deleted`, { cause: error });
                    }
                }
                return removed;
            }
    ...
    activateForEvent(event) {
        return this.#mutate(async () => {
            ...
            if (!snapshot.authoritative)
                throw new Error("Cannot activate extensions from a stale catalog");
            for (const entry of snapshot.extensions) {
                if (!entry.desired.enabled || !entry.manifest.entrypoints?.host?.activation?.includes(event))
                    continue;
    ...
    discardCandidate(requestValue) {
        const request = parsePiariumExtensionCandidateSelectionRequest(requestValue);
        return this.#mutateCatalog(async () => {
            const current = await this.catalog.snapshot();
            if (current.revision !== request.expectedRevision) {
                return this.catalog.discardCandidate(request.extensionId, request.candidateIntegrity, request.expectedRevision);
            }
            await this.supervisor.discardPreparedCandidate(request.extensionId, request.candidateIntegrity);
            try {
                const snapshot = await this.catalog.discardCandidate(request.extensionId, request.candidateIntegrity, request.expectedRevision);
                await this.supervisor.reconcile(snapshot);
                return snapshot;
            }
            catch (error) {
    ...
    selectCandidate(requestValue) {
        const request = parsePiariumExtensionCandidateSelectionRequest(requestValue);
        return this.#mutateCatalog(async () => {
            const selected = await this.supervisor.selectCandidate(request.extensionId, request.candidateIntegrity, request.expectedRevision);
            await this.supervisor.reconcile(selected);
            return this.catalog.snapshot();
        });
    }
    requestCandidateApplication(requestValue) {
        const request = parsePiariumExtensionCandidateSelectionRequest(requestValue);
        return this.#mutateCatalog(() => this.catalog.requestCandidateApplication(request.extensionId, request.candidateIntegrity, request.expectedRevision));
    }
    reviewCandidateCapabilities(requestValue) {
    ...
    reviewCapabilities(requestValue) {
    ...
    reportActualState(extensionId, stateValue) {
    ...
    setServiceSelection(requestValue) {
    ...
    upsertServiceRoutingRule(request) {
        return this.#mutate(async () => {
    ...
    removeServiceRoutingRule(request) {
        return this.#mutate(async () => {
    ...
    selectWorkbenchProfile(request) {
        return this.#mutate(async () => {
    ...
    upsertWorkbenchProfile(request) {
        return this.#mutate(async () => {
    ...
    removeWorkbenchProfile(request) {
        return this.#mutate(async () => {
    ...
    applyWorkbenchProfile(requestValue) {
        const request = parsePiariumWorkbenchProfileApplyRequest(requestValue);
        return this.#mutateCatalog(async () => {
            const workbench = await this.workbench.read();
            if (!workbench.authoritative)
                throw new Error("Cannot apply a stale workbench profile");
            ...
            if (!profile.extensionIds)
                throw new Error(`Workbench profile does not define an extension set: ${request.profileId}`);
    ...
    async #invokeRegisteredService(request, signal) {
        const parsed = parsePiariumExtensionServiceInvocationRequest(request);
        ...
        if (legacySelection)
            return this.services.invoke({ ...parsed, providerId: legacySelection }, signal);
        const candidates = services.providers.filter((provider) => (provider.status === "active"
    ...
    async #ensureBuiltinArtifact(extensionId) {
        const definition = PIARIUM_BUILTIN_EXTENSION_DEFINITIONS.find((candidate) => (candidate.manifest.id === extensionId && candidate.manifest.entrypoints?.host));
        if (!definition)
            return;
        const snapshot = await this.catalog.snapshot();
        await this.packages.reconcileBuiltinArtifacts([definition], snapshot);
    }
    async #ensureBuiltinServiceArtifacts(request) {
        const snapshot = await this.catalog.snapshot();
        const enabled = new Set(snapshot.extensions
            .filter((entry) => entry.desired.enabled)
            .map((entry) => entry.manifest.id));
        const definitions = PIARIUM_BUILTIN_EXTENSION_DEFINITIONS.filter((definition) => (enabled.has(definition.manifest.id)
            && definition.manifest.entrypoints?.host
            && definition.manifest.provides?.services?.some((service) => (service.id === request.serviceId && service.version === request.version))));
        if (definitions.length > 0)
            await this.packages.reconcileBuiltinArtifacts(definitions, snapshot);
    }
    #mutate(operation) {
...
//# sourceMappingURL=application-runtime.js.map