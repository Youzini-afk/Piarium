...
const readInjectedBearerToken = (): string => {
  ...
  const injected = (window as typeof window & { __PIARIUM_CLIENT_TOKEN__?: string }).__PIARIUM_CLIENT_TOKEN__;
...
export const getRuntimeExtraHeadersSync = (): Record<string, string> => {
  ...
  if (typeof window === 'undefined') return {};
  const injected = (window as typeof window & { __PIARIUM_RUNTIME_HEADERS__?: Record<string, string> }).__PIARIUM_RUNTIME_HEADERS__;
