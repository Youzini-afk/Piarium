...
// Control Center control (iOS 18+): tap the Piarium logo to start a new session.
//
...
@available(iOS 18.0, *)
struct PiariumNewSessionControl: ControlWidget {
    var body: some ControlWidgetConfiguration {
        StaticControlConfiguration(kind: "PiariumNewSessionControl") {
            ControlWidgetButton(action: OpenNewSessionIntent()) {
                ...
                Label("New Session", image: "PiariumLogoSymbol")
            }
        }
        .displayName("New Session")
        .description("Start a new Piarium session.")
    }
}

@available(iOS 18.0, *)
struct OpenNewSessionIntent: AppIntent {
    static let title: LocalizedStringResource = "New Piarium Session"
    static let openAppWhenRun: Bool = true
    static let isDiscoverable: Bool = true

    @MainActor
    func perform() async throws -> some IntentResult & OpensIntent {
        return .result(opensIntent: OpenURLIntent(URL(string: "piarium://new")!))
    }
}
