...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const resolveWslExecutablePath = () => {
    if (typeof deps.resolveWslExecutablePath === 'function') {
      return deps.resolveWslExecutablePath();
    }
  ...
  const applyOpencodeBinaryFromSettings = async (options = {}) => {
    ...
    try {
      ...
      if (explicitWslPath && explicitWslPath[1] && explicitWslPath[1].trim().length > 0) {
        const resolved = applyWslOpencodeResolution({ opencodePath: explicitWslPath[1].trim(), source: 'settings' });
        if (resolved) {
          return resolved;
        }
        clearWslOpencodeResolution();
        ...
        console.warn(`Configured settings.opencodeBinary uses WSL but WSL could not be resolved: ${raw}`);
        return null;
      }

      if (process.platform === 'win32' && (isWslExecutableValue(raw) || isWslExecutableValue(normalized || ''))) {
        const probed = probeWslForOpencode();
        const resolved = probed
          ? applyWslOpencodeResolution({ ...probed, source: 'settings' })
          : applyWslOpencodeResolution({ wslBinary: normalized || raw, opencodePath: 'opencode', source: 'settings' });
        if (resolved) {
          return resolved;
        }
        clearWslOpencodeResolution();
        ...
        console.warn(`Configured settings.opencodeBinary points to WSL but WSL could not be resolved: ${raw}`);
        return null;
      }
