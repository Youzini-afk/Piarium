...
export const createOpenCodeLifecycleRuntime = (deps) => {
  const {
    ...
    globalEventHub = null,
    spawnProcess = importedSpawn,
    spawnSyncProcess = importedSpawnSync,
  ...
  const killProcessOnPort = (port) => {
    ...
    try {
      const result = spawnSyncProcess('lsof', ['-ti', `:${port}`], { encoding: 'utf8', timeout: 5000, windowsHide: true });
      ...
      for (const pidStr of output.split(/\s+/)) {
        ...
        if (pid && pid !== myPid) {
          try {
            spawnSyncProcess('kill', ['-9', String(pid)], { stdio: 'ignore', timeout: 2000 });
          } catch {
          }
        }
      }
  ...
  const terminateChildProcess = async (child) => {
    ...
    if (process.platform === 'win32') {
      ...
      try {
        spawnSyncProcess('taskkill', ['/pid', String(pid), '/t'], {
          stdio: 'ignore',
          timeout: 3000,
          windowsHide: true,
        });
      ...
      try {
        spawnSyncProcess('taskkill', ['/pid', String(pid), '/f', '/t'], {
          stdio: 'ignore',
          timeout: 5000,
          windowsHide: true,
        });
  ...
  const createManagedOpenCodeServerProcess = async ({ hostname, port, timeout, cwd, env: processEnv, shellEnvKeysCount = 0 }) => {
    ...
    if (process.platform === 'win32' && state.useWslForOpencode) {
      binary = state.resolvedWslBinary || binary;
      const opencodePath = state.resolvedWslOpencodePath || 'opencode';
      args = typeof buildWslExecArgs === 'function'
        ? buildWslExecArgs([opencodePath, ...args], state.resolvedWslDistro)
        : ['--exec', opencodePath, ...args];
      launchWrapperType = 'wsl';
    }
