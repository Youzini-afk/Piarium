...
describe('OpenCode env runtime', () => {
  ...
  it('does not auto-detect the Windows OpenCode desktop app as a CLI', () => {
    ...
    fs.writeFileSync(desktopBinary, '');
    process.env.LOCALAPPDATA = localAppData;
    process.env.APPDATA = createTempDir('openchamber-empty-appdata-');
    process.env.PATH = createTempDir('openchamber-empty-path-');
  ...
  it('detects an npm OpenCode shim from APPDATA when PATH is unavailable', () => {
    setPlatform('win32');
    const appData = createTempDir('openchamber-appdata-');
    const shim = path.join(appData, 'npm', 'opencode.cmd');
    fs.mkdirSync(path.dirname(shim), { recursive: true });
    fs.writeFileSync(shim, '@echo off\r\nexit /b 0\r\n');
    process.env.APPDATA = appData;
    process.env.LOCALAPPDATA = createTempDir('openchamber-empty-localappdata-');
    process.env.PATH = createTempDir('openchamber-empty-path-');
    process.env.SystemRoot = createTempDir('openchamber-empty-systemroot-');
    delete process.env.OPENCODE_BINARY;
    const { runtime, state } = createRuntime({}, {
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    ...
    const launchSpec = runtime.resolveManagedOpenCodeLaunchSpec(shim);
    expect([
      {
        binary: nativeBinary,
        args: [],
