...
describe('OpenCode lifecycle', () => {
  ...
  it('adds managed OpenChamber tool environment without allowing it to replace launch invariants', async () => {
    ...
    const managedConfig = JSON.parse(options.env.OPENCODE_CONFIG_CONTENT);
    expect(managedConfig.plugin[0]).toBe('file:///tool.js');
    expect(managedConfig.plugin[1]).toMatch(/openai-stream-normalizer-plugin\.mjs$/);
