...
describe('terminal runtime', () => {
  ...
  it('resolves workspace-relative paths for create and restart while enforcing lockdown', async () => {
    ...
    const harness = createHarness({
      fs: {
        promises: {
          mkdir: async () => {},
          realpath: async (targetPath) => path.resolve(targetPath),
    ...
    try {
      ...
      expect(harness.processes[0].options.cwd).toBe(path.resolve(workspaceRoot, 'project'));
      ...
      expect(harness.processes[1].options.cwd).toBe(path.resolve(workspaceRoot, 'other-project'));
  ...
  it('creates client-identified sessions and forwards bounded resize operations', async () => {
    ...
    try {
      ...
      expect(harness.processes[0].options.cwd).toBe(path.resolve('/repo'));
  ...
  it('restarts atomically with the same identity and closes the previous process', async () => {
    ...
    try {
      ...
      expect(harness.processes[1].options.cwd).toBe(path.resolve('/other'));
    } finally { await harness.runtime.shutdown(); }
  });
  ...
  it('serializes concurrent restarts without orphaning replacement processes', async () => {
    ...
    try {
      ...
      expect(harness.processes[2].options.cwd).toBe(path.resolve('/second'));
    } finally { await harness.runtime.shutdown(); }
  });
