...
async function createTempRepo() {
  ...
  const git = {
    add: async (file) => runGit(tmpDir, ['add', file]),
    commit: async (message) => {
      runGit(tmpDir, ['commit', '-m', message]);
      return { commit: runGit(tmpDir, ['rev-parse', 'HEAD']).trim() };
    },
    checkout: async (ref) => runGit(tmpDir, ['checkout', ref]),
    checkoutBranch: async (branch, startPoint) => runGit(tmpDir, ['checkout', '-b', branch, startPoint]),
    log: async () => ({ latest: { hash: runGit(tmpDir, ['rev-parse', 'HEAD']).trim() } }),
    raw: async (...args) => runGit(tmpDir, args.flat()),
    status: async () => {
