...
describe('settings helpers', () => {
  ...
  it('includes transient desktop LAN access runtime status in desktop settings response', () => {
    const helpers = createTestHelpers();
    const previousRuntime = process.env.PIARIUM_RUNTIME;
    const previousActive = process.env.PIARIUM_DESKTOP_LAN_ACCESS_ACTIVE;
    const previousReason = process.env.PIARIUM_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    try {
      process.env.PIARIUM_RUNTIME = 'desktop';
      process.env.PIARIUM_DESKTOP_LAN_ACCESS_ACTIVE = 'false';
      ...
      if (typeof previousRuntime === 'string') process.env.PIARIUM_RUNTIME = previousRuntime;
      else delete process.env.PIARIUM_RUNTIME;
      if (typeof previousActive === 'string') process.env.PIARIUM_DESKTOP_LAN_ACCESS_ACTIVE = previousActive;
      else delete process.env.PIARIUM_DESKTOP_LAN_ACCESS_ACTIVE;
      if (typeof previousReason === 'string') process.env.PIARIUM_DESKTOP_LAN_ACCESS_BLOCKED_REASON = previousReason;
      else delete process.env.PIARIUM_DESKTOP_LAN_ACCESS_BLOCKED_REASON;
    }
  });
