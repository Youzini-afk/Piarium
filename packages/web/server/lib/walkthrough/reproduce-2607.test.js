...
process.env.PIARIUM_DATA_DIR = path.join(TEMP_HOME, '.config', 'piarium');
...
describe('issue 2607 —walkthrough blocks unauthenticated providers', () => {
  beforeAll(async () => {
    setupGitRepo();
    fs.mkdirSync(process.env.PIARIUM_DATA_DIR, { recursive: true });
    fs.writeFileSync(
      path.join(process.env.PIARIUM_DATA_DIR, 'settings.json'),
  ...
  it('callSmallModel throws a structured no-provider-login error', async () => {
    ...
    expect(error.message).toBe('No Pi credential found for provider "deepseek"');
