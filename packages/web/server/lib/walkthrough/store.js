...
// This is storage pressure control, not a limit on how many reviews a user may
// keep. Walkthroughs are model output and can otherwise grow without bound; the
// byte budget tracks the resource actually consumed, while a count limit would
// evict hundreds of tiny, still-useful entries for no resource reason.
const MAX_TOTAL_BYTES = 50 * 1024 * 1024;
// The model is currently allowed at most 32k output tokens (roughly 128 KiB of
// ordinary text). Four MiB leaves ample room for JSON structure and unusually
...
/**
 * Bound the cache by total size, dropping least-recently-used
...
// for seconds. Async calls wait without holding the loop. Every pointer is
// visited: a fixed prefix cap would make entries beyond that prefix permanently
// ineligible for cleanup because this housekeeping runs only once per process.
...
export const __testing = { WALKTHROUGH_DIR, ENTRIES_DIR, POINTERS_DIR, MAX_TOTAL_BYTES };
