...
const readContextState = (session) => {
  ...
  const messages = Array.isArray(piarium.context_obligatory_messages)
    ? piarium.context_obligatory_messages.filter((item) =>
      isRecord(item)
      && typeof item.id === 'string'
      && typeof item.createdAt === 'number'
      && (item.role === 'user' || item.role === 'assistant'))
    : [];
  return { metadata, piarium, messages };
};
...
export const createContextObligatoryRuntime = ({
  ...
  const tick = async (sessionId, directory) => {
    ...
    if (state.piarium.context_obligatory_last_compaction_message_id === summary.id) return;
