  ...
  it('injects pinned text in chronological order after compaction and records the summary cursor', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async (input, init = {}) => {
      ...
      if (url.pathname === '/session/ses_1') {
        ...
        return json({
          id: 'ses_1',
          metadata: { piarium: { context_obligatory_messages: [
    ...
    expect(JSON.parse(patch.body).metadata.piarium.context_obligatory_last_compaction_message_id).toBe('msg_summary');
