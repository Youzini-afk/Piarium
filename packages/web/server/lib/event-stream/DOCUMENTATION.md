...
- Global synthetic events such as `piarium:session-status`, `piarium:session-activity`, `piarium:notification`, and `piarium:heartbeat` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
