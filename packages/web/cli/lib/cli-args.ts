...
function findClosestMatch(input: unknown, candidates: readonly string[], maxDistance = 3): string | null {
...
function splitOptionToken(arg: string): { inlineValue?: string | undefined; long: boolean; name: string } | null {
...
export interface ParsedCliArgs {
  ...
  options: CliOptions;
  removedFlagErrors: string[];
  scheduleAction: string | null;
  sessionAction: string | null;
  startupAction: string | null;
  subcommand: string | null;
  tunnelAction: string | null;
  versionRequested: boolean;
}

function parseArgs(argv: readonly string[] = process.argv.slice(2)): ParsedCliArgs {
  ...
  const consumeValue = (index: number, inlineValue: string | undefined): {
    nextIndex: number;
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'thinking': {
        const { value, nextIndex } = consumeValue(i, inlineValue);
        i = nextIndex;
        options.thinking = typeof value === 'string' ? value : options.thinking;
        break;
      }
      case 'variant': {
        const { value, nextIndex } = consumeValue(i, inlineValue);
...
function generateCompletionScript(shell: unknown): string | null {
...
_piarium_tunnel() {
  ...
  prev="\${COMP_WORDS[COMP_CWORD-1]}"
    ...
    commands="serve stop restart status schedule session models projects tunnel startup logs connect-url update"
