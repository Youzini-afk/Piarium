...
async function statusCommand(options: CliOptions = {}): Promise<void> {
  ...
  const toPasswordProtectionLabel = (value: unknown): string => {
  ...
  const cliInstances: StatusInstance[] = runningInstances
  ...
  const instances: StatusInstance[] = desktopOnly ? [...cliInstances, desktopOnly] : [...cliInstances];
  if (explicitDesktop) {
    instances.push({
      runtime: 'desktop',
