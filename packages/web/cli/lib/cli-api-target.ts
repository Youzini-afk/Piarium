...
const uniquePorts = (entries: Array<number | PortEntry>): number[] => {
...
async function resolveTargetPort(options: CliOptions = {}, deps: {
  discoverDesktopInstance?: () => Promise<PortEntry | null>;
  discoverLifecycleInstances?: (options: CliOptions) => Promise<PortEntry[]>;
  isServerHealthReady?: (port: number, timeoutMs?: number) => Promise<boolean>;
} = {}): Promise<number> {
  if (options.explicitPort && typeof options.port === 'number' && Number.isFinite(options.port) && options.port > 0) {
    return options.port;
  }
