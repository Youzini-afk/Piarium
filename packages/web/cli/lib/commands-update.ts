...
import type { CliOptions, ServeCommand } from './cli-types.js';
...
interface PackageManagerModule<Manager> {
  checkForUpdates(): Promise<UpdateInfo>;
  detectPackageManager(): Manager;
  executeUpdate(manager: Manager, options: { silent: boolean }): { exitCode: number | null; success: boolean };
  getCurrentVersion(): string;
}

function createUpdateCommand<Manager>({ importFromFilePath, packageManagerPath, serveCommand }: {
  importFromFilePath(filePath: string): Promise<PackageManagerModule<Manager>>;
  packageManagerPath: string;
  serveCommand: ServeCommand;
}) {
  return async function updateCommand(options: CliOptions = {}): Promise<void> {
