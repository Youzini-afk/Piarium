...
function createServeCommand({
  ...
  bunBin: string;
  getPreferredServerRuntime(): 'bun' | 'node';
  serverPath: string;
  setForegroundServerActive(value: boolean): void;
  setForegroundShutdown(handler: ForegroundShutdown | null): void;
}): ServeCommand {
async function serveCommand(options: CliOptions): Promise<number | void> {
    const showOutput = shouldRenderHumanOutput(options);
    const jsonMessages: CliNotice[] = [];
    const emitNotice = (notice: CliNotice): void => {
    ...
    if (options.foreground) {
      ...
      if (suppressServerOutput) {
        const logStream = fs.createWriteStream(initialLogPath, { fd: logFd, autoClose: false });
        const writeToLog = (
          chunk: string | Uint8Array,
          encodingOrCallback?: BufferEncoding | ((error?: Error | null) => void),
          callback?: (error?: Error | null) => void,
        ): boolean => {
          if (typeof encodingOrCallback === 'function') return logStream.write(chunk, encodingOrCallback);
          if (encodingOrCallback && callback) return logStream.write(chunk, encodingOrCallback, callback);
          if (encodingOrCallback) return logStream.write(chunk, encodingOrCallback);
          if (callback) return logStream.write(chunk, callback);
          return logStream.write(chunk);
        };
        Reflect.set(process.stdout, 'write', writeToLog);
        Reflect.set(process.stderr, 'write', writeToLog);
      } else {
    ...
    // Best-effort operation; continue when it is unavailable.
      ...
      if (isQuietMode(options)) {
        if (!options.suppressQuietOutput) {
          realStdoutWrite(autoGeneratedUiPassword
      ...
      const shutdownForegroundServer: ForegroundShutdown = async (signal = 'SIGTERM') => {
    ...
    try {
      resolvedPort = await new Promise<number>((resolve, reject) => {
        let settled = false;
    ...
    writePidFile(pidFilePath, childPid, emitNotice);
    ...
    if (isQuietMode(options)) {
      ...
      process.stdout.write(autoGeneratedUiPassword
        ? `${resolvedPort} pass:${effectiveUiPassword}\n`
        : `${resolvedPort}\n`);
      return resolvedPort;
    }
