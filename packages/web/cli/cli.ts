...
let onCancelCleanup: CancelCleanup | null = null;
let activeCommandOptions: CliOptions | null = null;
let foregroundServerActive = false;
let foregroundShutdown: ForegroundShutdown | null = null;

function setCancelCleanup(handler: CancelCleanup | null): void {
  onCancelCleanup = typeof handler === 'function' ? handler : null;
}

function shouldWarnForTunnelProfileFile(): boolean {
...
function importFromFilePath(filePath: string) {
...
function getPreferredServerRuntime(): 'bun' | 'node' {
...
const lifecycleCommands = createLifecycleCommands({ serveCommand });
const tunnelCommand = createTunnelCommand({
  serveCommand,
  stopCommand: lifecycleCommands.stop,
  setCancelCleanup,
  boldText,
  ensureTunnelProfilesMigrated,
});
const connectUrlCommand = createConnectUrlCommand({
  serveCommand,
});
...
type StandardCommand = (options: CliOptions) => Promise<unknown>;
type ActionCommand = (options: CliOptions, action?: string) => Promise<unknown>;
type TunnelCommand = (options: CliOptions, subcommand?: string | null, action?: string | null) => Promise<unknown>;
interface CliCommands extends Record<string, StandardCommand | ActionCommand | TunnelCommand> {
  'connect-url': StandardCommand;
  logs: StandardCommand;
  models: ActionCommand;
  projects: ActionCommand;
  restart: StandardCommand;
  schedule: ActionCommand;
  serve: ServeCommand;
  session: ActionCommand;
  startup: ActionCommand;
  status: StandardCommand;
  stop: StopCommand;
  tunnel: TunnelCommand;
  update: StandardCommand;
}

const commands: CliCommands = {
  serve: serveCommand,
  'connect-url': connectUrlCommand,
  stop: lifecycleCommands.stop,
  restart: lifecycleCommands.restart,
...
async function main(): Promise<void> {
  ...
  if (command === 'startup') {
    await commands.startup(options, startupAction ?? undefined);
    return;
  }

  if (command === 'schedule') {
    await commands.schedule(options, scheduleAction ?? undefined);
    return;
  }

  if (command === 'session') {
    await commands.session(options, sessionAction ?? undefined);
    return;
  }
