...
const EXTERNAL_CLIENT_TTL_MIN_MS = 60 * 1000;
const EXTERNAL_CLIENT_TTL_MAX_MS = 365 * 24 * 60 * 60 * 1000;
...
function buildClientConnectionPayload({ serverUrl, token, label, profile }) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (name) {
      ...
      case 'expires-in': {
        const { value, nextIndex } = consumeValue(i, inlineValue);
        i = nextIndex;
        options.ttl = typeof value === 'string' ? value : options.ttl;
        break;
      }
      case 'capability':
      case 'capabilities': {
        ...
        i = nextIndex;
        if (typeof value === 'string') {
          options.capabilities.push(...value.split(',').map((entry) => entry.trim()).filter(Boolean));
        }
        break;
      }
      case 'json':
...
OPTIONS:
  ...
  --profile <profile>     Token profile: client, readonly, external-agent, full-control
  --ttl <duration>        Expire the token after a duration such as 1h, 24h, 7d
  --capability <value>    Add explicit capabilities (repeat or comma-separate)
...
EXAMPLES:
  ...
  openchamber connect-url --profile full-control --ttl 24h --qr
...
const commands = {
  ...
  async 'connect-url'(options = {}) {
    ...
    const profile = typeof options.profile === 'string' && options.profile.trim()
      ...
      : undefined;
    const ttlMs = typeof options.ttl === 'string'
      ? parseTtlMsOrThrow(options.ttl, {
          flagName: '--ttl',
          minMs: EXTERNAL_CLIENT_TTL_MIN_MS,
          maxMs: EXTERNAL_CLIENT_TTL_MAX_MS,
        })
      : null;
    const expiresAt = Number.isFinite(ttlMs) ? new Date(Date.now() + ttlMs).toISOString() : undefined;
    const label = options.name || (profile ? `OpenChamber ${profile} ${serverUrl}` : `OpenChamber ${serverUrl}`);
    ...
    const result = await runtime.createClient({
      label,
      profile,
      expiresAt,
      capabilities: options.capabilities,
      clientKind: profile ? `profile:${profile}` : undefined,
    });
    const connectUrl = buildClientConnectionPayload({ serverUrl, token: result.token, label, profile: result.client.profile });
    ...
    if (result.client.profile && result.client.profile !== 'client') {
      clackLog.info(`Profile: ${result.client.profile}`);
    }
    if (result.client.expiresAt) {
      clackLog.info(`Expires: ${result.client.expiresAt}`);
    }
