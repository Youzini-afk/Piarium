...
- `cli-lifecycle.js`
  ...
  - An explicit Windows stop may recover a registered but HTTP-unreachable process when command-line identity is unavailable, but only when both the PID file and matching per-port instance metadata exist and the recorded port is still occupied.
