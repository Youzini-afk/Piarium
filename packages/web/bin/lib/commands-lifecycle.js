...
async function stopCommand(options) {
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('info', `no Piarium instance found on port ${options.port}`);
      ...
      if (explicitInstance.source === 'probe') {
        ...
        if (showOutput && !unmanagedStopSpin) {
          logStatus('info', `found unmanaged Piarium instance on port ${options.port}`, 'attempting shutdown');
        }
        unmanagedStopSpin?.start(`Stopping unmanaged Piarium on port ${options.port}...`);
        ...
        if (stopped) {
          unmanagedStopSpin?.stop(`Stopped unmanaged Piarium on port ${options.port}`);
          jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
          if (isJsonMode(options)) {
            printJson({ stoppedCount: 1, results: jsonResults });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('success', `stopped Piarium on port ${options.port}`);
          ...
          unmanagedStopSpin?.error(`Could not stop Piarium on port ${options.port}`);
          ...
          if (isJsonMode(options)) {
            printJson({
              ...
              stoppedCount: 0,
              results: jsonResults,
              messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop Piarium on port ${options.port}.` }],
            });
          }
          if (showOutput && !unmanagedStopSpin) {
            logStatus('error', `could not stop Piarium on port ${options.port}`);
            finish('failed');
          }
          printQuietStopResults();
        }
      ...
      if (explicitInstance.source === 'registry-unconfirmed') {
        ...
        if (showOutput && !unconfirmedStopSpin) {
          logStatus('info', `found unconfirmed Piarium pid ${explicitInstance.pid} on port ${options.port}`, 'HTTP shutdown endpoint is unreachable; stopping by PID');
        }
        unconfirmedStopSpin?.start(`Stopping unconfirmed Piarium on port ${options.port}...`);
