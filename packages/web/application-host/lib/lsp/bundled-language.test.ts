...
import { PIARIUM_BUILTIN_LANGUAGE_SERVERS_EXTENSION_ID } from '@piarium/extension-builtins';
import { PIARIUM_BUILTIN_EXTENSION_PACKAGE_ROOTS } from '@piarium/extension-builtins/host';
...
if (!hasNativeProcessKernel && process.env.PIARIUM_REQUIRE_RELEASE_KERNEL === '1') throw new Error('Bundled language acceptance needs the release kernel');
...
it.skipIf(!hasNativeProcessKernel)('serves real Python symbols and type diagnostics through Host and Rust process pipes', async () => {
  const packageRoot = PIARIUM_BUILTIN_EXTENSION_PACKAGE_ROOTS.get(PIARIUM_BUILTIN_LANGUAGE_SERVERS_EXTENSION_ID);
  ...
  const content = 'def greet(name: str) -> str:\n    return name\n\nvalue: int = greet("Piarium")\n';
  try {
    await fs.writeFile(path.join(harness.workspaceRoot, 'hello.py'), content);
    language.registerProvider({ providerId: 'piarium.python-language', command: process.execPath, args: [path.join(packageRoot, 'runtime/pyright-langserver.cjs'), '--stdio'], languageIds: ['python'], source: 'builtin' });
