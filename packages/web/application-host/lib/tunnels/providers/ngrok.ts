...
export function createNgrokTunnelProvider(): TunnelProvider {
  const inspectAuthtoken = checkNgrokAuthtokenConfigured as (
    executablePath?: string | null,
  ) => ReturnType<typeof checkNgrokAuthtokenConfigured>;
  return {
    ...
    diagnose: async () => {
      ...
      const authtoken = await inspectAuthtoken(
        typeof dependency.path === 'string' ? dependency.path : null,
      );
    ...
    start: async (request, context) => {
      ...
      const controller = await startNgrokQuickTunnel({ port: context.activePort });
