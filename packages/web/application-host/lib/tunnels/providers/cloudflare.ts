...
export const cloudflareTunnelProviderCapabilities: TunnelProviderCapabilities = {
...
export function createCloudflareTunnelProvider(): TunnelProvider {
  const validateTokenShape = (value: unknown): { detail: string; ok: boolean } => {
  ...
  const createModeSummary = (checks: DiagnosticCheck[]) => {
  ...
  const describeMode = ({ mode, checks }: { mode: string; checks: DiagnosticCheck[] }) => {
  ...
  return {
    ...
    diagnose: async (request = {}) => {
      ...
      const managedLocalChecks: DiagnosticCheck[] = [
        {
          id: 'startup_readiness',
          label: 'Provider startup readiness',
          status: startupReady ? 'pass' : 'fail',
        ...
        },
        {
          ...
          detail: managedLocalInspection.ok
            ...
            : managedLocalInspection.error ?? 'Managed local config is invalid',
        },
      ];
      ...
      const managedRemoteChecks: DiagnosticCheck[] = [
        {
          id: 'startup_readiness',
          label: 'Provider startup readiness',
          status: startupReady ? 'pass' : 'fail',
    ...
    start: async (request, context) => {
      if (request.mode === TUNNEL_MODE_MANAGED_REMOTE) {
        ...
        return { ...controller, mode: TUNNEL_MODE_MANAGED_REMOTE };
      }
      ...
      if (request.mode === TUNNEL_MODE_MANAGED_LOCAL) {
        ...
        return { ...controller, mode: TUNNEL_MODE_MANAGED_LOCAL };
      }
      ...
      const controller = await startCloudflareQuickTunnel({
        originUrl: context.originUrl,
      });
      return { ...controller, mode: TUNNEL_MODE_QUICK };
    },
    stop: (controller) => {
      controller?.stop?.();
    },
