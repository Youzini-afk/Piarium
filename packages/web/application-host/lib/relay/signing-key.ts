...
// `settings.relaySigningKey = { privateJwk, publicJwk }` 鈥?existing installs'
...
export const getOrCreateRelaySigningKeypair = async ({ crypto, readSettingsFromDisk, updateSettingsOnDisk }) => {
  ...
  // Loud on purpose: a new signing key means a new serverId 鈥?every previously
 ...
 * relay's deriveServerId 鈥?this id is the routing key for both relays.
...
/**
 * ECDSA-SHA256, IEEE P1363 (raw r||s) signature 鈥?the form WebCrypto verifies.
 * @param {{ crypto: typeof import('node:crypto') }} deps
