...
export interface CreateWorkspaceRecoveryEngineOptions {
  ...
  /** Bound by the production Rust facade. Local SQLite fixtures leave this unset. */
  durableRecoveryStore?: RecoveryDurableOperationPort | undefined;
}
...
export interface DurableRecoveryBindableEngine extends WorkspaceRecoveryEngine {
  bindDurableRecoveryStore(store: RecoveryDurableOperationPort): void;
}

export interface WorkspaceRecoveryStorageContext {
  /** Local SQLite fixture handle. Production durable contexts omit it at runtime. */
  ...
  /** Rust-owned durable operation/checkpoint port. */
...
const parseKernelCombinedOperation = (operation: Record<string, unknown>): CombinedOperationRecord => {
  const data = isRecord(operation.data) ? operation.data : {};
  const result = isRecord(operation.result) ? operation.result : {};
  const record = parseCombinedOperationRecord({
    id: requireString(operation.operationId, 'Recovery operation id'),
    workspace_id: requireString(operation.workspaceId, 'Recovery operation workspace'),
    state: requireString(operation.state, 'Recovery operation state'),
    data_json: JSON.stringify({ ...data, ...result }),
  });
  for (const value of Array.isArray(operation.files) ? operation.files : []) {
    ...
    if ((value.phase === 'target-observed' || value.phase === 'compensate-intent') && !record.appliedPaths.includes(value.path)) {
      record.appliedPaths.push(value.path);
    }
  }
  return record;
};
...
export const createLocalSqliteWorkspaceRecoveryEngine = (
  options: CreateWorkspaceRecoveryEngineOptions,
): DurableRecoveryBindableEngine => {
  const {
    authorityId,
    dataDir,
    defaultRecoveryDir,
  ...
  let durableRecoveryStore = options.durableRecoveryStore;
  ...
  const runWorkspace = <T>(
    ...
    const current = previous.catch(() => undefined).then(async () => {
      ...
      if (!leaseOptions || durableRecoveryStore) return operation();
  ...
  const requireDurableStore = (): RecoveryDurableOperationPort => {
    if (!durableRecoveryStore) {
      throw new RecoveryPrimitiveError('unavailable', 'Rust recovery store is not bound', {
        origin: 'storage',
        retryable: true,
      });
    }
    return durableRecoveryStore;
  };

  const durableOperation = async (workspaceId: string, operationId: string): Promise<Record<string, unknown>> => {
    const operation = await requireDurableStore().getOperation(workspaceId, operationId);
    if (!operation) {
  ...
  const durableFileRows = (operation: Record<string, unknown>): OperationFileRow[] => (
    (Array.isArray(operation.files) ? operation.files : []).map((value, ordinal) => {
      const file = requireRecord(value, 'Recovery operation file');
      return {
        operation_id: requireString(operation.operationId, 'Recovery operation id'),
        ordinal,
        path: requireString(file.path, 'Recovery operation path'),
        expected_json: typeof file.expectedJson === 'string' ? file.expectedJson : null,
        target_json: typeof file.targetJson === 'string' ? file.targetJson : null,
        safety_json: typeof file.safetyJson === 'string' ? file.safetyJson : null,
        phase: requireString(file.phase, 'Recovery operation file phase') as OperationFilePhase,
        observed_fingerprint: typeof file.observedFingerprint === 'string' ? file.observedFingerprint : null,
        updated_at: typeof operation.updatedAt === 'string' ? operation.updatedAt : new Date(0).toISOString(),
      };
    })
  );

  const transitionDurableFile = async (
    record: CombinedOperationRecord,
    relativePath: string,
    phase: OperationFilePhase,
    fields: { expected?: RecoveryState; target?: RecoveryState; safety?: RecoveryState } = {},
  ): Promise<void> => {
    const operation = await durableOperation(record.plan.workspaceId, record.id);
    const file = (Array.isArray(operation.files) ? operation.files : [])
      .map((value) => requireRecord(value, 'Recovery operation file'))
      .find((value) => value.path === relativePath);
    ...
    if (file.phase === phase && Object.keys(fields).length === 0) return;
    await requireDurableStore().updateOperationFile({
      operationId: record.id,
      workspaceId: record.plan.workspaceId,
      path: relativePath,
      expectedRevision: Number(file.revision ?? 1),
      expectedPhase: requireString(file.phase, 'Recovery operation file phase'),
      phase,
      ...fields,
      sessionId: record.plan.sessionId,
    });
  ...
  const persistDurableOperation = async (record: CombinedOperationRecord): Promise<void> => {
    record.updatedAt = new Date().toISOString();
    const operation = await durableOperation(record.plan.workspaceId, record.id);
    if (operation.state === record.state && isRecord(operation.result)
      && operationRevision(operation.result) === operationRevision(record)) return;
    await requireDurableStore().completeOperation({
      operationId: record.id,
      workspaceId: record.plan.workspaceId,
  ...
  const createDurableCombinedOperation = async (record: CombinedOperationRecord): Promise<void> => {
    await requireDurableStore().createOperation({
      operationId: record.id,
      workspaceId: record.plan.workspaceId,
      kind: 'combined',
      state: record.state,
  ...
  const durableChangesForEntries = async (
    workspaceId: string,
    sessionId: string,
    entryIds: string[],
  ): Promise<ReturnType<typeof changesForEntries>> => {
    if (entryIds.length === 0) return { changes: [], hasIncompleteCheckpoint: false, uncoveredPaths: [], uncoveredReasons: [] };
    const store = requireDurableStore();
    if (!store.listChanges) throw new RecoveryPrimitiveError('unavailable', 'Rust recovery change reader is unavailable', { origin: 'storage' });
    const selected = await store.listChanges({ workspaceId, sessionId, entryIds });
    const changes: SequencedRecoveryChange[] = selected.changes
      .filter((change) => !sameState(change.before, change.after))
      .map((change) => ({
        after: parseRecoveryState(change.after),
        before: parseRecoveryState(change.before),
    ...
    for (const turn of selected.turns.filter((value) => value.status !== 'ready')) {
      const source: WorkspaceRecoveryUncoveredPath['source'] = turn.activeWriterScopes.some((scope) => scope.startsWith('process/'))
        ? 'shell'
        : turn.activeWriterScopes.some((scope) => scope.startsWith('external/'))
          ? 'external'
          : 'unknown';
      for (const relativePath of turn.unrecordedResourceIds) {
        if (!uncovered.has(relativePath)) uncovered.set(relativePath, { path: relativePath, source });
      }
      if (typeof turn.failure?.message === 'string' && turn.failure.message) reasons.add(turn.failure.message);
    }
    return {
      changes,
      hasIncompleteCheckpoint: selected.turns.some((value) => value.status !== 'ready'),
      uncoveredPaths: [...uncovered.values()].sort((a, b) => a.path.localeCompare(b.path)),
  ...
  const locateOperation = async (operationId: string): Promise<LocatedOperation> => {
    ...
    if (durableRecoveryStore) {
      for (const registration of registrations) {
        try {
          const operation = await durableRecoveryStore.getOperation(registration.workspaceId, operationId);
          if (!operation || operation.kind !== 'combined') continue;
          const identity = await inspectIdentity(registration.workspaceId);
          return { identity, record: parseKernelCombinedOperation(operation), root: identity.canonicalRoot };
        } catch (error) {
          inspectionFailure ??= error;
        }
      }
      if (inspectionFailure) {
        throw new RecoveryPrimitiveError('storage-malformed', 'Recovery operation lookup could not inspect every workspace', {
  ...
  const prepareCombinedInternal = async (
    ...
    if (durableRecoveryStore) {
      ...
      const resolver = durableRecoveryStore as RecoveryDurableOperationPort & {
        resolveEntry?: (target: WorkspaceRecoveryEntryTarget) => Promise<WorkspaceRecoveryEntryBindingResult>;
      };
      if (resolver.resolveEntry) {
        const resolved = await resolver.resolveEntry(input);
        if (resolved.status !== 'ready') {
          throw new RecoveryPrimitiveError('checkpoint-incomplete', 'The selected conversation entry has no ready checkpoint', {
            origin: 'coverage',
          });
        }
      }
      const loaded = await durableChangesForEntries(input.workspaceId, input.sessionId, removedEntryIds);
      ...
      try {
        conflicts = await buildConflicts(identity, identity.canonicalRoot, targets);
      } finally {
      ...
      const hasCoverageGap = loaded.uncoveredPaths.length > 0 || loaded.hasIncompleteCheckpoint;
      const coverage: WorkspaceCombinedRecoveryCoverage = !hasCoverageGap
        ? 'ready'
        : affectedPaths.length > 0 ? 'partial' : 'none';
      ...
      await createDurableCombinedOperation({
        appliedPaths: [],
  ...
  const compensateDurableCombined = async (
    record: CombinedOperationRecord,
    identity: RecoveryIdentity,
    root: string,
  ): Promise<void> => {
    record.state = 'compensating-files';
    await persistDurableOperation(record);
    const rows = durableFileRows(await durableOperation(record.plan.workspaceId, record.id)).reverse();
    for (const row of rows) {
      ...
      await resourceOperationGateFor(identity.workspaceId).run([{ resourceId: relativePath, scope: 'subtree' }], async () => {
        ...
        if (!sameState(current, targetStatesFor(record, relativePath).target)) {
          await transitionDurableFile(record, relativePath, 'needs-attention');
          throw new RecoveryPrimitiveError('needs-attention', `Cannot compensate a file changed after recovery: ${relativePath}`, {
            origin: 'storage',
          });
        }
        if (row.phase !== 'compensate-intent') await transitionDurableFile(record, relativePath, 'compensate-intent');
        await fileStore.applyState(identity, root, relativePath, safetyStateFor(record, relativePath));
        const restored = (await fileStore.captureState(identity, root, relativePath, { store: false })).state;
        if (!sameState(restored, safetyStateFor(record, relativePath))) {
          await transitionDurableFile(record, relativePath, 'needs-attention');
          throw new RecoveryPrimitiveError('needs-attention', `Compensated file did not match safety state: ${relativePath}`, {
            origin: 'storage',
          });
        }
        await transitionDurableFile(record, relativePath, 'safety-observed');
      });
    }
  ...
  const applyDurableCombined = async (
    located: LocatedOperation,
    input: WorkspaceCombinedRecoveryApplyInput,
  ): Promise<WorkspaceCombinedRecoveryOperation> => {
    const { identity, root } = located;
    const record = parseKernelCombinedOperation(await durableOperation(identity.workspaceId, input.operationId));
    ...
    try {
      if (record.plan.revision !== input.expectedRevision) {
        throw new RecoveryPrimitiveError('stale-plan', 'Recovery plan changed before it was applied', {
          origin: 'conflict', retryable: true,
        });
      }
      ...
      if (record.state === 'planned') {
        for (const relativePath of Object.keys(record.targets)) {
          ...
          await transitionDurableFile(record, relativePath, 'apply-intent', { safety: current.state });
        }
      ...
      try {
        const rows = durableFileRows(await durableOperation(identity.workspaceId, record.id));
        for (const row of rows) {
          const relativePath = row.path;
          if (row.phase === 'needs-attention') {
            throw new RecoveryPrimitiveError('needs-attention', `File ${relativePath} is in needs-attention state and blocks recovery`, { origin: 'storage' });
          }
          ...
          await resourceOperationGateFor(identity.workspaceId).run([{ resourceId: relativePath, scope: 'subtree' }], async () => {
            ...
            if (!sameState(current, safetyStateFor(record, relativePath))) {
              ...
              throw new RecoveryPrimitiveError('stale-plan', `File changed after safety capture: ${relativePath}`, {
                details: { paths: [relativePath] }, origin: 'conflict', retryable: true,
              });
            }
            ...
            if (!sameState(verified, targetStatesFor(record, relativePath).target)) {
              await transitionDurableFile(record, relativePath, 'needs-attention');
              throw new RecoveryPrimitiveError('needs-attention', `Restored file did not match its checkpoint: ${relativePath}`, { origin: 'storage' });
            }
            await transitionDurableFile(record, relativePath, 'target-observed');
          });
          if (!record.appliedPaths.includes(relativePath)) record.appliedPaths.push(relativePath);
          await persistDurableOperation(record);
        }
        ...
        if (record.appliedPaths.length > 0) {
          try {
            await compensateDurableCombined(record, identity, root);
        ...
        const phases = durableFileRows(await durableOperation(identity.workspaceId, record.id));
        if (phases.some((row) => row.phase === 'needs-attention')) {
      ...
      try {
        ...
        // A navigation response can be lost after the session already committed
        // the operationId. Keep the operation resumable and never compensate
        // files while the conversation outcome is unknown.
        ...
        throw new RecoveryPrimitiveError('navigation-conflict', error instanceof Error ? error.message : 'Conversation navigation failed', {
          cause: error, operationId: record.id, origin: 'navigation',
        });
      }
      record.failure = null;
  ...
  const applyLocatedOperation = async (
    ...
    if (durableRecoveryStore) return applyDurableCombined(located, input);
    const { identity, root } = located;
  ...
  const cancelOperationInternal = async (operationId: string): Promise<WorkspaceCombinedRecoveryOperation> => {
    const located = await locateOperation(operationId);
    if (durableRecoveryStore) {
      const record = parseKernelCombinedOperation(await durableOperation(located.identity.workspaceId, operationId));
      if (record.state === 'planned') {
  ...
  const prepareUndoInternal = async (operationId: string): Promise<WorkspaceCombinedRecoveryPlan> => {
    const located = await locateOperation(operationId);
    if (durableRecoveryStore) {
      const original = parseKernelCombinedOperation(await durableOperation(located.identity.workspaceId, operationId));
      ...
      for (const relativePath of original.plan.affectedPaths) {
        ...
        targets[relativePath] = { expected: originalStates.target, target: safetyStateFor(original, relativePath) };
      }
      const id = randomUUID();
      const createdAt = new Date().toISOString();
      ...
      const draft: Omit<WorkspaceCombinedRecoveryPlan, 'revision'> = {
        ...
        changedBytes: Object.values(targets).reduce((total, states) => total + Math.max(stateByteLength(states.expected), stateByteLength(states.target)), 0),
        conflicts,
        coverage: 'ready',
        createdAt,
  ...
  const listOperationsInternal = async (workspaceId: string): Promise<WorkspaceCombinedRecoveryOperation[]> => {
    if (durableRecoveryStore) {
      const summaries = await durableRecoveryStore.listOperations(workspaceId, 'combined');
      const operations = await Promise.all(summaries.map((summary) => (
        durableRecoveryStore!.getOperation(workspaceId, requireString(summary.operationId, 'Recovery operation id'))
      )));
      return operations.filter((value): value is Record<string, unknown> => Boolean(value))
        .map(parseKernelCombinedOperation)
  ...
  const agentMutationFailuresInternal = async (workspaceId: string): Promise<WorkspaceRecoveryFailure[]> => {
    if (durableRecoveryStore) {
      const rows = await durableRecoveryStore.listOperations(workspaceId, 'agent-mutation');
      return rows.filter((row) => row.state === 'needs-attention').map((row) => {
        const data = isRecord(row.data) ? row.data : {};
        const paths = Array.isArray(data.needsAttentionPaths)
          ? data.needsAttentionPaths.filter((value): value is string => typeof value === 'string')
          : [];
        const operationId = requireString(row.operationId, 'Agent mutation operation id');
        return recoveryFailure(new RecoveryPrimitiveError('needs-attention', `Agent surface mutation ${operationId} requires attention`, {
          details: { operationId, paths }, operationId, origin: 'storage',
        }), 'needs-attention');
      });
    }
    const { identity } = await inspectStorageIdentity(workspaceId);
  ...
  const resumeUnfinished = async (): Promise<WorkspaceCombinedRecoveryOperation[]> => {
    ...
    if (durableRecoveryStore) {
      for (const registration of registrations) {
        ...
        await runWorkspace(workspaceId, async () => {
          const identity = await inspectIdentity(workspaceId);
          const summaries = await durableRecoveryStore!.listOperations(workspaceId, 'combined');
          for (const summary of summaries) {
            if (['complete', 'aborted', 'compensated', 'needs-attention'].includes(String(summary.state))) continue;
            const operationId = requireString(summary.operationId, 'Recovery operation id');
            const operation = await durableOperation(workspaceId, operationId);
            const record = parseKernelCombinedOperation(operation);
            if (record.state === 'planned') continue;
            try {
              if (record.state === 'files-restored' || record.state === 'navigating-conversation') {
                const rows = durableFileRows(operation);
              ...
              for (const row of durableFileRows(operation)) {
                let phase = row.phase;
                if (row.phase === 'apply-intent' || row.phase === 'compensate-intent') {
                  ...
                  if (row.phase === 'apply-intent') {
                    if (sameState(current, targetStatesFor(record, row.path).target)) {
                      await transitionDurableFile(record, row.path, 'target-observed');
                      phase = 'target-observed';
                      if (!record.appliedPaths.includes(row.path)) record.appliedPaths.push(row.path);
                    } else if (!sameState(current, safetyStateFor(record, row.path))) {
                      await transitionDurableFile(record, row.path, 'needs-attention');
                      phase = 'needs-attention';
                    }
                  } else if (sameState(current, safetyStateFor(record, row.path))) {
                    await transitionDurableFile(record, row.path, 'safety-observed');
                    phase = 'safety-observed';
                  } else if (!sameState(current, targetStatesFor(record, row.path).target)) {
                    await transitionDurableFile(record, row.path, 'needs-attention');
                    phase = 'needs-attention';
                  }
                }
              ...
              if (phases.includes('needs-attention')) {
                ...
                await compensateDurableCombined(record, identity, identity.canonicalRoot);
              ...
              record.fileState = 'needs-attention';
              record.state = 'needs-attention';
              await persistDurableOperation(record).catch(() => undefined);
            }
            resolved.push(publicOperation(record));
          }
          ...
          const agentMutations = await reconcileInterruptedAgentMutations(context);
          if (agentMutations.needsAttention.length > 0) {
            rememberFailure(workspaceId, new RecoveryPrimitiveError(
              'needs-attention',
              `Agent surface mutation recovery requires attention (${agentMutations.needsAttention.join(', ')})`,
              { details: { operationIds: agentMutations.needsAttention }, operationId: agentMutations.needsAttention[0], origin: 'storage' },
            ), 'needs-attention');
          }
        }, { mode: 'exclusive', purpose: 'recovery-crash-reconciliation' }).catch((error) => {
  ...
  return {
    locations,
    bindDurableRecoveryStore: (store: RecoveryDurableOperationPort): void => {
      if (durableRecoveryStore && durableRecoveryStore !== store) {
        throw new Error('A different durable recovery store is already bound');
      }
