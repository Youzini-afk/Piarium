...
import {
  ...
  writeOperationRow,
  type InspectRecoveryJournalCatalogResult,
  type SqliteDatabase,
...
const createV4 = (root: string): SqliteDatabase => {
...
const requireInspection = (
  value: InspectRecoveryJournalCatalogResult,
): Exclude<InspectRecoveryJournalCatalogResult, null> => requireValue(value, 'Catalog inspection');
...
describe('recovery journal catalog', () => {
  ...
  it('migrates v3 transactionally, preserves rows, and rebuilds normalized object references', async () => {
    ...
    try {
      ...
      const reopened = requireValue(await openRecoveryJournalCatalog(root, { create: false }), 'Reopened catalog');
      try {
  ...
  it('retires an older root intact and restores retired-history status on reopen', async () => {
    ...
    try {
      ...
      try {
        ...
        const retiredRoot = requireValue(retired.retiredCatalogs[0], 'Retired catalog root');
        expect(await fs.promises.readFile(path.join(retiredRoot, 'legacy-object'), 'utf8'))
          .toBe('preserve-history');
        const retiredDatabase = new Database(path.join(retiredRoot, 'catalog.sqlite'), { readonly: true });
        try {
  ...
  it('inspects a v3 catalog read-only without migrating it', async () => {
    ...
    try {
      createV3(root).close();
      const inspected = requireInspection(await inspectRecoveryJournalCatalog(root));
  ...
  it('inspects a current v5 catalog and returns a readonly database', async () => {
    ...
    try {
      ...
      requireValue(inspected.database, 'Inspected catalog database').close();
    } finally {
      await fs.promises.rm(root, { force: true, recursive: true });
    }
  });
  ...
  it('tracks operation file phases through apply and compensate transitions', async () => {
    ...
    try {
      ...
      try {
        ...
        expect(requireValue(rows.find((r) => r.path === 'a.txt'), 'a.txt operation row').phase).toBe('target-observed');
        expect(requireValue(rows.find((r) => r.path === 'b.txt'), 'b.txt operation row').phase).toBe('pending');
        ...
        rows = operationFileRows(database, operationId);
        expect(requireValue(rows.find((r) => r.path === 'a.txt'), 'a.txt operation row').phase).toBe('safety-observed');
      } finally {
        database.close();
      }
    } finally {
