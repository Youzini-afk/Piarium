...
const sizing = ({ fileCount, hunkCount }) => {
  ...
  const targetChapters = hunkCount <= 4
    ? 1
    : Math.max(1, Math.ceil(targetStops / 3));

  return `This change has ${fileCount} file(s) and ${hunkCount} reviewable hunk(s).

Aim for about ${targetStops} stop(s) across about ${targetChapters} chapter(s). Fewer, denser stops beat many thin ones, but do not omit an important behavior merely to hit this estimate.
