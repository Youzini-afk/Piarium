...
import { getPullRequestDiff, type PullRequestDependencies } from './pull-request.js';
...
describe('getPullRequestDiff', () => {
  let request: Mock<(route: string, parameters: Record<string, unknown>) => Promise<{ data: string }>>;
  let getOctokitOrNull: Mock<() => { request: typeof request } | null>;
  let resolveGitHubRepoFromDirectory: Mock<(directory: string) => Promise<{
    remoteUrl: string | null;
    repo: { owner: string; repo: string } | null;
  }>>;
  let deps: PullRequestDependencies;
