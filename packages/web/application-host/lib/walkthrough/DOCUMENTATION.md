...
walkthrough changes only that review, just like the header picker; recovering
from a refusal does not silently redefine later reviews or commit messages.

A Pi `settings.json`/`models.json` `smallModel` (or legacy `small_model`) override can still name a provider
...
server's public normalization function. A locale added to the interface alone
...
- Entries are bounded by actual disk use (50 MB), not by an arbitrary number of
  reviews, and evicted least-recently-used after a write crosses that budget.
  Nothing is dropped for being merely old: a small entry stays reachable if the
  working tree ever returns to that state.
...
- Pointers are never evicted by size. They are pruned only when their repository
  is **provably gone**, deferred off the request path, and fully asynchronous.
...
- Their diffs go through `getUntrackedDiffs`, which resolves the repository once
  for the whole batch, instead of one `getDiff` per file each re-resolving the
  repository.
...
Some older or mismatched hosts answer an unknown `/api/*` path with the embedded
web UI — HTML, sometimes with status **200** — instead of a JSON 404. Parsing
that as JSON produces `Unexpected token '<', "<!doctype "...`, which names
