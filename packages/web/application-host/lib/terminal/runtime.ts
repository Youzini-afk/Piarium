...
export function createTerminalRuntime({
  ...
  app.post('/api/terminal/create', async (req, res) => {
    ...
    catch (error) { res.status(error?.statusCode || (error?.message === 'Maximum terminal sessions reached' ? 429 : 400)).json({ error: error?.message || 'Failed to create terminal session' }); }
  });
  ...
  app.post('/api/terminal/:sessionId/restart', async (req, res) => {
    ...
    } catch (error) { res.status(error?.statusCode || 400).json({ error: error?.message || 'Failed to restart terminal' }); }
