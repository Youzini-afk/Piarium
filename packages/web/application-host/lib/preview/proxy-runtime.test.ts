...
interface PreviewTargetRegistration {
  id: string;
...
const requirePreviewTargetRegistration = (value: unknown): PreviewTargetRegistration => {
  if (
    typeof value !== 'object'
    || value === null
    || !('id' in value)
    || typeof value.id !== 'string'
    || !('proxyBasePath' in value)
    || typeof value.proxyBasePath !== 'string'
    || !('previewToken' in value)
    || typeof value.previewToken !== 'string'
    || !('expiresAt' in value)
    || typeof value.expiresAt !== 'number'
  ) {
    throw new TypeError('Expected a preview target registration');
  }
  return {
    id: value.id,
    proxyBasePath: value.proxyBasePath,
...
type PreviewRuntimeDependencies = Parameters<typeof createPreviewProxyRuntime>[0];
type PreviewRuntime = ReturnType<typeof createPreviewProxyRuntime>;
type PreviewApplication = Parameters<PreviewRuntime['attach']>[0];
type PreviewAttachDependencies = Parameters<PreviewRuntime['attach']>[1];
type PreviewPostHandler = Parameters<PreviewApplication['post']>[1];
type PreviewUseHandler = Parameters<PreviewApplication['use']>[1];
type PreviewProxyOptions = Parameters<PreviewRuntimeDependencies['createProxyMiddleware']>[0];
type PreviewProxyMiddleware = ReturnType<PreviewRuntimeDependencies['createProxyMiddleware']>;
type PreviewInterceptorHandler = Parameters<PreviewRuntimeDependencies['responseInterceptor']>[0];
type PreviewBodyKind = Parameters<typeof rewritePreviewBody>[0]['kind'];

interface UnitPreviewRequest {
...
type UnitRouteHandler = (
  req: UnitPreviewRequest,
  res: TestResponse,
  next?: () => void,
) => unknown;

const requireRouteHandler = (
  handlers: readonly unknown[] | undefined,
  index = 0,
): UnitRouteHandler => {
  const handler = handlers?.[index];
  if (typeof handler !== 'function') {
    throw new TypeError(`Expected route handler at index ${index}`);
  }
  return handler as UnitRouteHandler;
};

const createAttachedPreviewRuntime = () => {
  let responseHandler: PreviewInterceptorHandler | undefined;
  const postRoutes = new Map<string, PreviewPostHandler[]>();
  const useRoutes = new Map<string, PreviewUseHandler[]>();
  let randomByte = 0;
  const runtime = createPreviewProxyRuntime({
    ...
    URL,
    createProxyMiddleware(options: PreviewProxyOptions) {
      void options;
      const middleware: PreviewProxyMiddleware = async () => {};
      middleware.upgrade = () => {};
      return middleware;
    },
    responseInterceptor: (handler: PreviewInterceptorHandler) => {
      responseHandler = handler;
      return async () => {};
    },
  });
  const app: PreviewApplication = {
    post(path: string, ...handlers: PreviewPostHandler[]) {
      postRoutes.set(path, handlers);
    },
    use(path: string, ...handlers: PreviewUseHandler[]) {
      useRoutes.set(path, handlers);
    },
  };
  const attachDependencies: PreviewAttachDependencies = {
    server: { on() {} },
    express,
  ...
  runtime.attach(app, attachDependencies);

  return {
    postRoutes,
    responseHandler: (): PreviewInterceptorHandler => {
      if (!responseHandler) throw new Error('Preview response interceptor was not installed');
...
const rewrite = (bodyText: string, kind: PreviewBodyKind): string => rewritePreviewBody({
  bodyText,
  kind,
...
describe('preview target failure signaling', () => {
  it('marks missing and expired targets instead of relying on the HTTP status alone', () => {
    ...
    const guard = requireRouteHandler(useRoutes.get('/api/preview/proxy'));
    ...
    for (const [originalUrl, code, error] of cases) {
  ...
  it('marks invalid target tokens and accepts a registered target token', async () => {
    ...
    const registerTarget = requireRouteHandler(postRoutes.get('/api/preview/targets'), 1);
    const guard = requireRouteHandler(useRoutes.get('/api/preview/proxy'));
  ...
  it('removes the reserved target-error marker from upstream responses', async () => {
    const { postRoutes, responseHandler, useRoutes } = createAttachedPreviewRuntime();
    const registerTarget = requireRouteHandler(postRoutes.get('/api/preview/targets'), 1);
    const registrationResponse = createResponse();
    await registerTarget({ body: { url: 'http://127.0.0.1:4323/' }, secure: false }, registrationResponse);
    const { id, previewToken } = requirePreviewTargetRegistration(registrationResponse.body);
    ...
    await handleResponse(
      ...
      { headers: { 'content-type': 'application/json' } } as IncomingMessage,
      request as IncomingMessage & UnitPreviewRequest,
      response as ServerResponse & TestResponse,
    );
...
describe('preview Inertia header passthrough', () => {
  it('forwards Inertia request headers to the preview target', () => {
    const forwarded = new Map<string, HeaderValue>();
    const proxyReq = {
      setHeader: (name: string, value: HeaderValue) => forwarded.set(name, value),
    };
