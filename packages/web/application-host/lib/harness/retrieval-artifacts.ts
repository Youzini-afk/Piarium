...
export const createRetrievalArtifactAccess = (workingStates: WorkspaceWorkingStateAccess) => {
  ...
  const releaseThreadEvidence = async (workspaceId: string, threadId: string): Promise<void> => withKernel(workingStates, workspaceId, "retrieval-evidence-release", async (_store, context) => { for (const record of await context.records.list({})) { let authorityThread: string | undefined; try { const payload = JSON.parse(record.payloadJson) as { authority?: { threadId?: unknown } }; authorityThread = typeof payload.authority?.threadId === "string" ? payload.authority.threadId : undefined; } catch { /* Rust already validates payload shape; malformed rows are released by identity only. */ } if ((record.recordType.startsWith("retrieval.evidence.") || record.recordType === "retrieval.artifact" || record.recordType === "retrieval.receipt") && (record.threadId === threadId || authorityThread === threadId)) await context.records.release(`thread-release:${threadId}:${record.recordId}`, record.recordId); } });
