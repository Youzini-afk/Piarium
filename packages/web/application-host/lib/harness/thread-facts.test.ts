...
describe("thread.facts.set", () => {
  it("validates facts on the retrieval child and exposes them through read_thread and settle", async () => {
    ...
    try {
      ...
      expect(submitted.evidence.facts[0]?.status).toBe("verified");
      ...
      expect(parent?.pendingEvidence?.facts[0]?.status).toBe("verified");
      ...
      expect(settled?.report?.evidence?.facts[0]?.status).toBe("verified");
      expect(settled?.report?.conclusion).toContain("verified");
  ...
  it("keeps submitted facts when the Run is cancelled", async () => {
    ...
    try {
      ...
      expect(cancelled?.report?.evidence?.facts[0]?.status).toBe("verified");
    } finally {
      await registry.dispose();
      rmSync(dataDir, { force: true, recursive: true });
    }
  ...
  it("keeps pending facts on lost and clears them when a new Run starts", async () => {
    ...
    try {
      ...
      expect(lost?.pendingEvidence?.facts[0]?.status).toBe("verified");
