...
const previousDataDir = process.env.VARIN_DATA_DIR;
const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'openchamber-go-routes-'));
process.env.VARIN_DATA_DIR = temporaryDirectory;

afterAll(() => {
  if (previousDataDir === undefined) delete process.env.VARIN_DATA_DIR;
  else process.env.VARIN_DATA_DIR = previousDataDir;
...
describe('OpenCode Go credential routes', () => {
  it('parses a JSON credential payload before validation', async () => {
    ...
    globalThis.fetch = vi.fn(async () => new Response('rollingUsage:$R[1]={usagePercent:25,resetInSec:60}'));
    const app = express();
    registerQuotaRoutes(app, {
      getQuotaProviders: async () => ({
        listConfiguredQuotaProviders: () => [],
        fetchQuotaForProvider: async () => ({ error: 'unused' }),
      }),
    });
