...
vi.mock('../../pi-config/storage.js', () => ({
  readPiAuthFile: () => ({ neuralwatt: { key: 'test-token' } }),
}));
...
describe('NeuralWatt quota provider', () => {
  it('builds subscription window from documented payload (keyed by plan name)', async () => {
    ...
    const window = requireWindow(result, 'standard');
    ...
    expect(requireWindow(result, 'credits_balance')).toBeDefined();
    expect(requireWindow(result, 'credits_balance').valueLabel).toBe('$32.68');
  });
  ...
  it('falls back to plan_limit title when plan is missing', async () => {
    ...
    expect(requireWindow(result, 'plan_limit')).toBeDefined();
    expect(requireWindow(result, 'plan_limit').usedPercent).toBeCloseTo((13.9023 / 20.0) * 100, 4);
  });
  ...
  it('surfaces subscription and allowance windows (allowance keyed by period, key name in valueLabel)', async () => {
    ...
    const subWindow = requireWindow(result, 'standard');
    ...
    const allowWindow = requireWindow(result, 'monthly');
  ...
  it('uses allowance effective limit = min(limit, credits_remaining + spent)', async () => {
    ...
    const window = requireWindow(result, 'monthly');
    expect(window).toBeDefined();
  ...
  it('uses weekly as the allowance key when period is weekly', async () => {
    ...
    const window = requireWindow(result, 'weekly');
    expect(window).toBeDefined();
  ...
  it('uses daily as the allowance key when period is daily', async () => {
    ...
    const window = requireWindow(result, 'daily');
    expect(window).toBeDefined();
  ...
  it('falls back to billing_cycle when allowance period is missing or unknown', async () => {
    ...
    const window = requireWindow(result, 'billing_cycle');
    expect(window).toBeDefined();
    expect(window.usedPercent).toBe(25);
  });
  ...
  it('falls back to credits_balance when neither subscription nor allowance exists', async () => {
    ...
    expect(requireWindow(result, 'credits_balance').valueLabel).toBe('$32.68');
    expect(requireWindow(result, 'credits_balance').usedPercent).toBeNull();
  });
