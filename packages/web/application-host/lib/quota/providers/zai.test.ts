...
vi.mock('../../pi-config/storage.js', () => ({
  readPiAuthFile: () => ({ 'zai-coding-plan': { key: 'test-token' } }),
}));
...
describe('Z.ai quota provider', () => {
  it('surfaces 5-hour, weekly, and MCP quota windows', async () => {
    ...
    if (!result.usage || typeof result.usage !== 'object' || Array.isArray(result.usage)
      || !('windows' in result.usage) || !result.usage.windows
      || typeof result.usage.windows !== 'object' || Array.isArray(result.usage.windows)) {
      throw new Error('Expected quota usage windows');
    }
    const windows = result.usage.windows as Record<string, unknown>;
