...
type TestRequestInit = RequestInit & { body: string; headers: Record<string, string> };
...
const lastCall = (mock: FetchMock) => {
  const call = mock.mock.calls.at(-1);
  if (!call) throw new Error('Expected a fetch call');
  const [url, init] = call;
  return { url: String(url), init: init as TestRequestInit };
};

const callAt = (mock: FetchMock, index: number): [unknown, TestRequestInit] => {
  const call = mock.mock.calls[index];
  if (!call) throw new Error(`Expected fetch call ${index}`);
  return [call[0], call[1] as TestRequestInit];
};
...
describe('callSmallModel — custom provider config', () => {
  ...
  describe('resolution order when auth.json is also present', () => {
    ...
    it('prefers the models.json apiKey over an auth.json credential', async () => {
      ...
      expect(JSON.stringify(callAt(fetchMock, 0)[1])).not.toContain('authjson-key');
    });
  });
...
describe('callSmallModel — Google thinking configuration', () => {
  ...
  const googleResponse = (text: string) => ({
    ok: true,
    status: 200,
...
describe('callSmallModel — GitHub Copilot endpoint routing', () => {
  ...
  const copilotAuth = (overrides: Record<string, unknown> = {}) => ({
    'github-copilot': {
      type: 'oauth',
      access: 'test-token',
      refresh: 'test-token',
  ...
  const callCopilot = (modelID: string, options: { auth?: Record<string, unknown> } = {}) => callSmallModel({
  ...
  it('routes a model advertising /responses through the Responses API', async () => {
    ...
    expect(String(callAt(fetchMock, 0)[0])).toBe('https://api.githubcopilot.com/models');
    expect(String(callAt(fetchMock, 1)[0])).toBe('https://api.githubcopilot.com/responses');
    const body = JSON.parse(callAt(fetchMock, 1)[1].body);
  ...
  it('prefers /v1/messages when a model advertises multiple endpoints', async () => {
    ...
    expect(String(callAt(fetchMock, 1)[0])).toBe('https://api.githubcopilot.com/v1/messages');
    const body = JSON.parse(callAt(fetchMock, 1)[1].body);
    expect(body).toMatchObject({
  ...
  it('uses /chat/completions when the model advertises the chat endpoint', async () => {
    ...
    expect(String(callAt(fetchMock, 1)[0])).toBe('https://api.githubcopilot.com/chat/completions');
  });
  ...
  it('uses the enterprise Copilot host for metadata and generation', async () => {
    ...
    expect(String(callAt(fetchMock, 0)[0])).toBe('https://copilot-api.ghe.example.com/models');
    expect(String(callAt(fetchMock, 1)[0])).toBe('https://copilot-api.ghe.example.com/responses');
  });
