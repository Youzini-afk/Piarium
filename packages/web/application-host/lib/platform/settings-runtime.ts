...
export const createSettingsRuntime = (deps) => {
    ...
    const validations = projects.map(async (project) => {
      ...
      try {
        ...
        if (!stats.isDirectory()) {
          console.warn(`[validateProjectEntries] Dropping project 鈥?path is not a directory: ${project.path}`);
          return null;
        }
        return project;
      } catch (error) {
        if (error && typeof error === 'object' && error.code === 'ENOENT') {
          console.warn(`[validateProjectEntries] Dropping project 鈥?directory no longer exists: ${project.path}`);
          return null;
        }
        // Permission or transient fs error 鈥?keep the project rather than
  ...
  const persistSettings = async (changes) => {
    const next = await updateSettingsOnDisk(async (current) => {
      // Log field names only 鈥?changes can carry credentials (UI password,
      ...
      // do it when the incoming update actually touches the projects list 鈥?      // not on every theme/window-state/etc. save.
