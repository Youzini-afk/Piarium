...
export interface PiWorktreeSessionRuntime {
  ...
  createWorktree(project: ProjectRef, preferredName: string): Promise<WorktreeMetadata>;
  generateBranchName(): string;
  getActiveProject(): ProjectEntry | null;
  removeWorktree(project: ProjectRef, worktree: WorktreeMetadata): Promise<void>;
  shouldWaitForBootstrap(project: ProjectRef): Promise<boolean>;
...
const defaultRuntime: PiWorktreeSessionRuntime = {
  ...
  removeWorktree: (project, worktree) => (
    removeProjectWorktree(project, worktree, { deleteLocalBranch: true })
  ),
...
export const createPiWorktreeSessionWithRuntime = async (
  ...
  if (!activeProject?.path) {
    throw new Error('Select a project before creating a worktree session');
  }

  const project: ProjectRef = { id: activeProject.id, path: activeProject.path };
  if (!await runtime.checkIsGitRepository(project.path)) {
    throw new Error('Worktree sessions require a Git repository');
  }

  const preferredName = runtime.generateBranchName().trim();
  if (!preferredName) {
    throw new Error('Could not generate a worktree branch name');
  }

  const worktree = await runtime.createWorktree(project, preferredName);
  if (await runtime.shouldWaitForBootstrap(project)) {
    await runtime.waitForBootstrap(worktree.path);
  }
  ...
  try {
    ...
    return {
      branch: worktree.branch || preferredName,
      path: worktree.path,
    ...
    try {
      ...
      throw new Error(
        `Failed to create the Pi session (${errorMessage(sessionError)}); `
        + `the worktree also could not be removed (${errorMessage(cleanupError)})`,
        { cause: sessionError },
      );
    }
    throw sessionError;
