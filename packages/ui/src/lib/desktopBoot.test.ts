...
describe('getInjectedBootOutcome', () => {
  ...
  test('returns null for malformed payload with unknown kind', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = { kind: 'unknown-kind' };
    try {
      expect(getInjectedBootOutcome()).toBeNull();
    } finally {
      restoreWindow();
  ...
  test('returns null for payload missing required hostId', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = { kind: 'main-remote', url: 'https://x.test' };
    try {
      expect(getInjectedBootOutcome()).toBeNull();
    } finally {
      restoreWindow();
  ...
  test('returns null for non-object payload', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = 'not-an-object';
    try {
      expect(getInjectedBootOutcome()).toBeNull();
    } finally {
      restoreWindow();
  ...
  test('returns valid outcome for well-formed main-local', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = { target: 'local', status: 'ok' };
    try {
  ...
  test('returns null for payload with numeric kind', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = { kind: 42 };
    try {
      expect(getInjectedBootOutcome()).toBeNull();
    } finally {
      restoreWindow();
...
describe('getBootInjectionStatus', () => {
  ...
  test('returns "not-injected" when global is explicitly null', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = null;
    try {
      expect(getBootInjectionStatus()).toBe('not-injected');
    } finally {
      restoreWindow();
  ...
  test('returns "malformed" when global is present but invalid', () => {
    const w = mockWindow();
    w.__VARIN_DESKTOP_BOOT_OUTCOME__ = { kind: 'bad' };
    try {
