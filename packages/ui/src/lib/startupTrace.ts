...
declare global {
  interface Window {
    __VARIN_STARTUP_TRACE__?: StartupTraceEvent[];
    __VARIN_STARTUP_TRACE_START__?: number;
  }
}
...
const enabled = () => {
  ...
  try {
    ...
    return params.get('startupTrace') === '1' || window.localStorage?.getItem('VARIN_STARTUP_TRACE') === '1';
  } catch {
    return false;
  }
};
...
export const markStartupTrace = (name: string, data?: Record<string, unknown>) => {
  ...
  window.__VARIN_STARTUP_TRACE_START__ ??= now;
  window.__VARIN_STARTUP_TRACE__ ??= [];
  window.__VARIN_STARTUP_TRACE__.push({
    t: Math.round(now - window.__VARIN_STARTUP_TRACE_START__),
    name,
    ...(data ? { data } : {}),
  });
  if (window.__VARIN_STARTUP_TRACE__.length > MAX_STARTUP_TRACE_EVENTS) {
    window.__VARIN_STARTUP_TRACE__.splice(
      0,
      window.__VARIN_STARTUP_TRACE__.length - MAX_STARTUP_TRACE_EVENTS,
    );
  }
};

const getStartupTraceSummary = () => {
  const trace = typeof window !== 'undefined' ? window.__VARIN_STARTUP_TRACE__ ?? [] : [];
...
if (typeof window !== 'undefined') {
  (window as typeof window & { __VARIN_STARTUP_TRACE_SUMMARY__?: typeof getStartupTraceSummary })
    .__VARIN_STARTUP_TRACE_SUMMARY__ = getStartupTraceSummary;
}
