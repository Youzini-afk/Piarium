...
const sessionUpdateMock = mock(async (...args: unknown[]) => {
  ...
  const parameters = args[0] as { sessionID?: string; time?: { archived?: number } };
  return {
    data: {
      id: parameters.sessionID ?? 'ses_1',
      title: 'Session',
      time: { created: 1, updated: 2, archived: parameters.time?.archived },
    },
  };
});
...
describe('opencodeClient session archive compatibility', () => {
  test('maps a null archive timestamp to the durable restored sentinel', async () => {
    await opencodeClient.updateSession('ses_restore', { time: { archived: null } }, '/repo');

    expect(sessionUpdateCalls[0]?.[0]).toEqual({
