...
class OpencodeService {
  ...
  async updateSession(
    ...
    const sdkPatch = {
      ...
      // OpenCode's current update schema accepts only numeric archive values.
      // Use 0 as the durable "restored" sentinel; OpenChamber classifies it as
      // active when rebuilding global session snapshots.
      ...(archived !== undefined ? { time: { archived: archived ?? 0 } } : {}),
    };
