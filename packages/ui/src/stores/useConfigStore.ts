...
const MODELS_DEV_PROXY_URL = "/api/piarium/models-metadata";
        ...
        if (runtimeSettings) {
            try {
                ...
                if (data) {
                    ...
                    const sttSilenceThresholdDb = typeof data?.sttSilenceThresholdDb === 'number' && Number.isFinite(data.sttSilenceThresholdDb)
                        ? data.sttSilenceThresholdDb
                        : undefined;
...
interface ConfigStore {
    ...
    sttLocalModel: string;
    sttLanguage: string;
    // Fork VAD settings (silence detection / auto-transcribe on stop)
    sttSilenceThresholdDb: number;
    sttSilenceHoldMs: number;
    sttTranscribeOnStop: boolean;
    ...
    loadProviders: (options?: { directory?: string | null; force?: boolean; source?: string }) => Promise<void>;
    loadAgents: (options?: { directory?: string | null; force?: boolean; source?: string }) => Promise<boolean>;
        ...
        persist(
            (set, get) => ({
                ...
                loadProviders: async (options) => {
                    ...
                    const source = options?.source ?? (force ? 'force' : 'unknown');
                    markStartupTrace('loadProviders:called', { directoryKey, source, requestedDirectory, effectiveDirectory, force });
                ...
                loadAgents: async (options) => {
                    ...
                    const force = options?.force === true;
                    const source = options?.source ?? (force ? 'force' : 'unknown');
                    markStartupTrace('loadAgents:called', { directoryKey, source, requestedDirectory, effectiveDirectory, force });
...
if (!unsubscribeConfigStoreChanges) {
    unsubscribeConfigStoreChanges = subscribeToConfigChanges(async (event) => {
        ...
        if (scopeMatches(event, "agents")) {
            ...
            tasks.push(loadAgents({ force: true, source: 'configChange:agents' }).then(() => {}));
        }
