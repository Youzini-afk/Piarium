...
mock.module('@/lib/walkthrough/api', () => ({
  ...
  cancelWalkthroughGeneration: async () => { cancelCalls += 1; },
...
describe('useWalkthroughStore — reattaching to a running generation', () => {
  ...
  test('cancels through the explicit server action and restores the prior result', async () => {
    readResult = finished;
    await useWalkthroughStore.getState().load('/repo', SOURCE);
    void useWalkthroughStore.getState().generate('/repo', SOURCE, { force: true });
    await flush();
