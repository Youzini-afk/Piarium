...
describe('useGlobalSessionsStore', () => {
  ...
  test('moves a restored session back to the active collection', () => {
    ...
    useGlobalSessionsStore.getState().unarchiveSessions([archived.id]);

    expect(useGlobalSessionsStore.getState().archivedSessions).toEqual([]);
    expect(useGlobalSessionsStore.getState().activeSessions[0]?.time.archived).toBe(0);
    expect(useGlobalSessionsStore.getState().sessionsByDirectory.get('/repo/app')?.[0]?.id).toBe(archived.id);
  });
