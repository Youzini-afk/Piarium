...
- Archived sessions are not shown in the web/desktop sidebar; the Archive page (`ArchiveView`, `useUIStore.isArchivePageOpen`) replaces the old toggle and exposes both restore and delete actions. VS Code keeps inline archived buckets behind `showArchivedSessions` (compact webview has no page surfaces). Restore uses the fork's `unarchiveSession` path: the client maps `null` to the durable numeric `time.archived: 0` sentinel accepted by OpenCode, publishes the optimistic active-session move, and rolls back if the update fails.
