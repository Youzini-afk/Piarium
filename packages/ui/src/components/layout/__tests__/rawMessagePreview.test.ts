...
describe('formatMessagePreviewTime', () => {
  ...
  test('12h mode includes the locale day-period marker', () => {
    const result = formatMessagePreviewTime(ts, '12h');
    const dayPeriod = new Intl.DateTimeFormat(undefined, {
      hour: 'numeric',
      hour12: true,
    }).formatToParts(new Date(ts)).find((part) => part.type === 'dayPeriod')?.value;

    expect(dayPeriod).toBeTruthy();
    expect(result).toContain(dayPeriod as string);
  });
