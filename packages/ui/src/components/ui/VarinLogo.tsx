...
export const VarinLogo: React.FC<VarinLogoProps> = ({
  ...
  return (
    ...
    >
      {isAnimated ? (
        <style>{`@keyframes varin-logo-glow{0%,100%{filter:drop-shadow(0 0 0 transparent)}50%{filter:drop-shadow(0 0 4px var(--varin-glow-color))}}.varin-logo-glow{animation:varin-logo-glow 1.8s ease-in-out infinite}@media (prefers-reduced-motion:reduce){.varin-logo-glow{animation:none}}`}</style>
      ) : null}
      ...
      <g
        ...
        style={isAnimated ? ({ '--varin-glow-color': strokeColor } as React.CSSProperties) : undefined}
      >
