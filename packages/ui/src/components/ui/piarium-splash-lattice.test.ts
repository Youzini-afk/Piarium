 ...
 * that the pre-paint hosts still carry the same generated bytes.
 */
...
describe('the camera is a perspective camera', () => {
  ...
  test('the floor transform the CSS applies is the one this arithmetic describes', () => {
    // The arithmetic defines floor coverage while the Canvas owner feeds the same authored camera to the
    // real cube. Any drift here would make the coverage proof describe a different view than the browser.
...
describe('the cube stands on the floor', () => {
  ...
  test('the cube and floor sample one Canvas-owned camera rather than separate clocks', () => {
    expect(css).toContain('.pi-splash-camera {');
    expect(css).toContain(`transform: ${CAMERA_DYNAMIC_FLOOR_TRANSFORM} scale(1);`);
    expect(css).toContain(`transform: translateZ(${CUBE_EDGE_PX / 2}px);`);
    expect(css).toContain(":not([data-piarium-camera-owner='canvas']) .pi-splash-camera");
    const renderer = mountSplashTileCanvas.toString();
    expect(renderer).toContain("setAttribute(\"data-piarium-camera-owner\", \"canvas\")");
    expect(renderer).toContain('Math.abs(appliedCameraTiltDeg - tiltDeg) > 1e-4');
    expect(renderer).toContain('style.setProperty(options.camera.tiltProperty, `${tiltDeg}deg`)');
    expect(renderer).toContain('renderer?.draw(frames, viewport, tiltDeg)');
...
describe('the floor covers the window it has to cover', () => {
  ...
  test('tile count grows with the viewport behind one drawing owner', () => {
    ...
    expect(renderer).toContain('crypto.getRandomValues(entropy)');
  });
...
describe('the floor is the cover', () => {
  ...
  test('the leaving-prop reveal is boot-only, and boot is what a host gets by saying nothing', () => {
    // Workbench transitions use explicit mirrored phases. The pre-module hosts set no
...
/**
 * The web shell, mini-chat window, and React component paint this splash. They differ in what they show
 * and where their colours come from, and those differences are
...
describe('every splash host carries the same generated bytes', () => {
  ...
  test('the generated floor script builds its tile field from the live viewport', () => {
    ...
    expect(script).toContain('var breathes=');
    expect(script).toContain('cycleIndex');
  ...
  test('the generated floor script is stable across calls', () => {
    // Runtime cycle selection is pseudo-random; generation itself remains byte-stable for the embedded hosts.
  ...
  test('the pre-paint hosts carry the generated camera rather than a handwritten projection', () => {
    for (const body of [readWebShell(), readMiniChat()]) {
      expect(body).toContain(CAMERA_DYNAMIC_FLOOR_TRANSFORM);
