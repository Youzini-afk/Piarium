...
export const KeyboardShortcutsSettings: React.FC = () => {
  ...
  const actionLabel = React.useCallback((id: string, fallbackLabel: string): string => {
    const key = `settings.varin.keyboardShortcuts.action.${id}.label`;
  ...
  const saveCombo = React.useCallback((actionId: string, combo: ShortcutCombo) => {
    ...
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(normalized) ? t('settings.varin.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
  ...
  const confirmOverwrite = React.useCallback(() => {
    ...
    setPendingOverwrite(null);
    setErrorText('');
    setWarningText(isRiskyBrowserShortcut(pendingOverwrite.combo) ? t('settings.varin.keyboardShortcuts.warning.riskyBrowserShortcut') : '');
    setDraftByAction((current) => {
      const rest = { ...current };
  ...
  return (
    <SettingsSection
      ...
      title={t('settings.varin.keyboardShortcuts.title')}
      divider={false}
      info={t('settings.varin.keyboardShortcuts.tooltip')}
      headerAction={(
        <Button
          type="button"
          variant="outline"
        ...
        >
          {t('settings.varin.keyboardShortcuts.actions.resetAll')}
        </Button>
      )}
    >
          ...
          {pendingOverwrite && (
            <div className="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] p-3 flex flex-col @xl:flex-row @xl:items-center justify-between gap-3">
              <span className="typography-meta text-foreground">
                {t('settings.varin.keyboardShortcuts.overwritePrompt')}
              </span>
              <div className="flex gap-2 shrink-0">
                <Button type="button" size="xs" className="!font-normal" onClick={confirmOverwrite}>{t('settings.varin.keyboardShortcuts.actions.overwrite')}</Button>
          ...
          return (
            <div key={action.id} className={cn("py-1.5", index > 0 && "border-t border-border/40")}>
              ...
              >
                <Input
                  readOnly
                  value={capturingActionId === action.id ? t('settings.varin.keyboardShortcuts.field.pressKeys') : formatShortcutForDisplay(displayCombo)}
                  onFocus={() => {
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!next) {
                      setErrorText(t('settings.varin.keyboardShortcuts.error.captureFirst'));
                      return;
                    }
