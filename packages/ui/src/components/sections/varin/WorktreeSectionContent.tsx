...
export const WorktreeSectionContent: React.FC<WorktreeSectionContentProps> = ({ projectRef: projectRefProp = null, sections = 'all' }) => {
  ...
  const persistSetupCommands = React.useCallback(async (commands: string[]): Promise<boolean> => {
    ...
    try {
      ...
      if (!ok) {
        toast.error(t('settings.varin.worktrees.setup.toast.saveFailed'));
        return false;
      }
  ...
  const setupTooltip = (
    <SettingsInfoHint>
      {t('settings.varin.worktrees.setup.tooltipPrefix')}
      {' '}
      <code className="font-mono text-xs bg-sidebar-accent/50 px-1 rounded">$ROOT_PROJECT_PATH</code>
      {' '}
      {t('settings.varin.worktrees.setup.tooltipSuffix')}
    </SettingsInfoHint>
  );

  const listTooltip = (
    <SettingsInfoHint>
      {t('settings.varin.worktrees.list.tooltip')}
    </SettingsInfoHint>
  );
    ...
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.varin.worktrees.state.selectProject')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  if (isGitRepoLocal === false) {
    return (
      ...
      >
        <p className="typography-meta text-muted-foreground">
          {t('settings.varin.worktrees.state.gitOnly')}
        </p>
      </ProjectSettingsSubsection>
    );
  }
  ...
  return (
    <>
      ...
      >
        {isLoadingCommands ? (
          <p className="typography-meta text-muted-foreground">{t('settings.varin.worktrees.setup.loading')}</p>
        ) : (
          <div className={cn('space-y-2', PROJECT_SETTINGS_CONTROL_WIDTH)}>
            {setupCommands.map((command, index) => (
              <div key={index} className="flex w-full gap-2">
                <Input
                  ...
                  placeholder={t('settings.varin.worktrees.setup.commandPlaceholder')}
                ...
                <Button
                  ...
                  aria-label={t('settings.varin.worktrees.setup.removeCommandAria')}
                >
            ...
            >
              ...
              {t('settings.varin.worktrees.setup.addCommand')}
            </Button>
            <label
            ...
            >
              <Checkbox
                ...
                ariaLabel={t('settings.varin.worktrees.setup.waitForCommandsAria')}
              />
              <span className={cn(
                'typography-ui-label font-normal',
                waitForSetupCommands ? 'text-foreground' : 'text-foreground/60'
              )}>
                {t('settings.varin.worktrees.setup.waitForCommands')}
              </span>
            </label>
          </div>
        )}
      ...
      <ProjectSettingsSubsection
        title={t('settings.varin.worktrees.list.title')}
        titleAccessory={listTooltip}
      >
        {isLoadingWorktrees ? (
          <p className="typography-meta text-muted-foreground">{t('settings.varin.worktrees.list.loading')}</p>
        ) : availableWorktrees.length === 0 ? (
          <p className="typography-meta text-muted-foreground/70">
            {t('settings.varin.worktrees.list.empty')}
          </p>
        ) : (
            ...
            {availableWorktrees.map((worktree) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 items-center gap-2">
                    <p className="typography-meta min-w-0 truncate text-foreground">
                      {worktree.label || worktree.branch || t('settings.varin.worktrees.list.detachedHead')}
                    </p>
                  </div>
                ...
                <button
                  ...
                  aria-label={t('settings.varin.worktrees.list.deleteWorktreeAria', { name: worktree.branch || worktree.label || worktree.path })}
                >
