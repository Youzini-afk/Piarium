...
export const GitSettings: React.FC = () => {
  ...
  const viewOptions = React.useMemo(
    () => [
      { id: 'flat' as const, label: t('settings.varin.git.option.flatList') },
      { id: 'tree' as const, label: t('settings.varin.git.option.treeView') },
    ],
    [t]
  );
  ...
  return (
    <SettingsSection title={t('settings.varin.git.title')}>
      <div className={SETTINGS_OPTION_STACK_CLASS}>
        <SettingsControlGroup
          settingsItem="git.changes-view"
          title={t('settings.varin.git.changesViewTitle')}
        >
          <SettingsRadioGroup aria-label={t('settings.varin.git.changesViewAria')}>
            {viewOptions.map((option) => (
              <SettingsRadioOption
                ...
                label={option.label}
                ariaLabel={t('settings.varin.git.optionAria', { option: option.label })}
              />
            ))}
          </SettingsRadioGroup>
        </SettingsControlGroup>
        ...
        <SettingsCheckboxRow
          ...
          label={t('settings.varin.git.enableGitmoji')}
          ariaLabel={t('settings.varin.git.enableGitmojiAria')}
        />
        ...
        <SettingsCheckboxRow
          ...
          label={t('settings.varin.git.showGitignored')}
          ariaLabel={t('settings.varin.git.showGitignoredAria')}
        />
      </div>
    </SettingsSection>
  );
