...
const TUNNEL_MODE_OPTIONS: Array<{ value: TunnelMode; labelKey: string; tooltipKey: string }> = [
  {
    value: 'quick',
    labelKey: 'settings.varin.tunnel.option.mode.quick.label',
    tooltipKey: 'settings.varin.tunnel.option.mode.quick.tooltip',
  },
  {
    value: 'managed-remote',
    labelKey: 'settings.varin.tunnel.option.mode.managedRemote.label',
    tooltipKey: 'settings.varin.tunnel.option.mode.managedRemote.tooltip',
  },
  {
    value: 'managed-local',
    labelKey: 'settings.varin.tunnel.option.mode.managedLocal.label',
    tooltipKey: 'settings.varin.tunnel.option.mode.managedLocal.tooltip',
  },
];

const MANAGED_LOCAL_CONFIG_ALLOWED_EXTENSIONS = ['.yml', '.yaml', '.json'];
const MANAGED_LOCAL_CONFIG_EXTENSION_ERROR_KEY = 'settings.varin.tunnel.error.invalidConfigExtension';
...
const getClientInstallPlatform = (): string => {
  if (typeof window !== 'undefined' && typeof window.__VARIN_PLATFORM__ === 'string') {
    const platform = window.__VARIN_PLATFORM__;
...
export const TunnelSettings: React.FC = () => {
  ...
  const renderedSessionRecords = React.useMemo(() => {
    return sessionRecords.map((record) => {
      ...
      const inactiveLabel = remainingTextForSession === 'expired'
        ? t('settings.varin.tunnel.state.expired')
        : (record.inactiveReason === 'tunnel-revoked'
          ? t('settings.varin.tunnel.state.revoked')
          : t('settings.varin.tunnel.state.inactive'));
  ...
  const refreshTunnelDependencyCheck = React.useCallback(async (provider: string, signal?: AbortSignal): Promise<boolean | null> => {
    try {
      const checkRes = await runtimeFetch('/api/varin/tunnel/check', {
        query: { provider },
        ...(signal ? { signal } : {}),
      });
      if (!checkRes.ok) {
  ...
  const checkAvailabilityAndStatus = React.useCallback(async (signal: AbortSignal) => {
    try {
      const [checkRes, statusRes, settingsRes, providersRes] = await Promise.all([
        runtimeFetch('/api/varin/tunnel/check', { signal }),
        runtimeFetch('/api/varin/tunnel/status', { signal }),
        runtimeFetch('/api/config/settings', { signal, headers: { Accept: 'application/json' } }),
        runtimeFetch('/api/varin/tunnel/providers', { signal }),
      ]);
      ...
      if (!signal.aborted) {
        setState('error');
        setErrorMessage(t('settings.varin.tunnel.toast.checkAvailabilityFailed'));
      }
    }
  ...
  React.useEffect(() => {
    if (!tunnelInfo?.bootstrapExpiresAt) {
      setRemainingText(t('settings.varin.tunnel.state.noExpiry'));
      return;
    }
    ...
    const updateRemaining = () => {
      ...
      if (remaining <= 0) {
        setRemainingText(t('settings.varin.tunnel.state.expired'));
      } else {
  ...
  React.useEffect(() => {
    ...
    const refreshSessions = async () => {
      try {
        const statusRes = await runtimeFetch('/api/varin/tunnel/status');
  ...
  const saveTunnelSettings = React.useCallback(async (payload: {
    ...
    try {
      ...
      toast.error(t('settings.varin.tunnel.toast.saveSettingsFailed'));
    } finally {
  ...
  const saveTtlSettings = React.useCallback(async (nextBootstrapTtlMs: number | null, nextSessionTtlMs: number) => {
    ...
    try {
      ...
      toast.error(t('settings.varin.tunnel.toast.saveTtlFailed'));
    } finally {
  ...
  const persistManagedRemoteTunnelToken = React.useCallback(async (payload: {
    ...
    try {
      ...
      toast.error(t('settings.varin.tunnel.toast.saveTokenFailed'));
    }
  ...
  const handleStart = React.useCallback(async () => {
    ...
    try {
      ...
      if (tunnelMode === 'managed-remote') {
        if (!selectedPreset) {
          setState('idle');
          setManagedRemoteValidationError(t('settings.varin.tunnel.toast.selectOrAddManagedRemoteFirst'));
          toast.error(t('settings.varin.tunnel.toast.selectOrAddManagedRemoteFirst'));
          return;
        }
      ...
      const res = await runtimeFetch('/api/varin/tunnel/start', {
        method: 'POST',
      ...
      if (!res.ok || !data.ok) {
        if (tunnelMode === 'managed-remote' && typeof data.error === 'string' && data.error.includes('Managed remote tunnel token is required')) {
          setState('idle');
          setManagedRemoteValidationError(t('settings.varin.tunnel.toast.managedRemoteTokenRequiredBeforeStarting'));
          toast.error(t('settings.varin.tunnel.toast.addManagedRemoteTokenBeforeStarting'));
          return;
        }
        setState('error');
        setErrorMessage(data.error || t('settings.varin.tunnel.toast.startFailed'));
        toast.error(data.error || t('settings.varin.tunnel.toast.startFailed'));
        return;
      }
      ...
      if (!startedUrl) {
        setState('error');
        setErrorMessage(t('settings.varin.tunnel.toast.startedButNoPublicUrl'));
        toast.error(t('settings.varin.tunnel.toast.startedButNoPublicUrl'));
        return;
      }
      ...
      if (data.replacedTunnel) {
        ...
        if (revokedBootstrapCount === 1 && invalidatedSessionCount === 1) {
          toast.warning(t('settings.varin.tunnel.toast.replacedTunnelSingleSingle'));
        } else if (revokedBootstrapCount === 1) {
          toast.warning(t('settings.varin.tunnel.toast.replacedTunnelSingleManySessions', { invalidatedSessionCount }));
        } else if (invalidatedSessionCount === 1) {
          toast.warning(t('settings.varin.tunnel.toast.replacedTunnelManyLinksSingleSession', { revokedBootstrapCount }));
        } else {
          toast.warning(t('settings.varin.tunnel.toast.replacedTunnelManyMany', { revokedBootstrapCount, invalidatedSessionCount }));
        }
      } else {
        toast.success(t('settings.varin.tunnel.toast.linkReady'));
      }
    } catch {
      setState('error');
      setErrorMessage(t('settings.varin.tunnel.toast.startFailed'));
      toast.error(t('settings.varin.tunnel.toast.startFailed'));
    }
  }, [
  ...
  const handleStop = React.useCallback(async () => {
    ...
    try {
      await runtimeFetch('/api/varin/tunnel/stop', { method: 'POST' });
      const statusRes = await runtimeFetch('/api/varin/tunnel/status');
      if (statusRes.ok) {
        const statusData = (await statusRes.json()) as TunnelStatusResponse;
        setSessionRecords(Array.isArray(statusData.activeSessions) ? statusData.activeSessions : []);
      ...
      setQrDataUrl(null);
      setState('idle');
      toast.success(t('settings.varin.tunnel.toast.stopped'));
    } catch {
      setState('error');
      setErrorMessage(t('settings.varin.tunnel.toast.stopFailed'));
      toast.error(t('settings.varin.tunnel.toast.stopFailed'));
    }
  }, [t]);
  ...
  const handleCopyUrl = React.useCallback(async () => {
    ...
    try {
      ...
      setCopied(true);
      toast.success(t('settings.varin.tunnel.toast.connectLinkCopied'));
      setTimeout(() => setCopied(false), 2000);
    } catch {
      toast.error(t('settings.varin.tunnel.toast.copyUrlFailed'));
    }
  ...
  const persistSelectedPreset = React.useCallback(async (preset: ManagedRemoteTunnelPreset, presets: ManagedRemoteTunnelPreset[]) => {
    try {
      ...
      toast.error(t('settings.varin.tunnel.toast.saveSelectedManagedRemoteFailed'));
    }
  }, [t]);
  ...
  const handleSaveNewPreset = React.useCallback(async () => {
    ...
    if (!name) {
      toast.error(t('settings.varin.tunnel.toast.tunnelNameRequired'));
      return;
    }
    if (!hostname) {
      toast.error(t('settings.varin.tunnel.toast.managedRemoteHostnameRequired'));
      return;
    }
    if (!token) {
      toast.error(t('settings.varin.tunnel.toast.managedRemoteTokenRequired'));
      return;
    }

    if (managedRemoteTunnelPresets.some((preset) => preset.hostname === hostname)) {
      toast.error(t('settings.varin.tunnel.toast.hostnameAlreadyExists'));
      return;
    }
    ...
    toast.success(t('settings.varin.tunnel.toast.managedRemoteSaved'));
  ...
  const handleRemovePreset = React.useCallback(async (presetId: string) => {
    ...
    toast.success(t('settings.varin.tunnel.toast.managedRemoteRemoved'));
  ...
  if (state === 'checking') {
    return (
      <div className="flex items-center justify-center py-12">
        <span className="h-1.5 w-1.5 rounded-full bg-current animate-busy-pulse" aria-label={t('settings.varin.tunnel.state.loading')} />
      </div>
    );
  }
  ...
  return (
    <SettingsSection
      title={t('settings.varin.tunnel.title')}
      info={(
        <div className="space-y-1">
          <p>{t('settings.varin.tunnel.description')}</p>
          <p>{t('settings.varin.tunnel.note.serverSideEnforced')}</p>
          <p>{t('settings.varin.tunnel.note.connectLinksOneTime')}</p>
        </div>
      )}
      divider={false}
    >
        ...
        <section className="space-y-2 px-2 pb-2 pt-0">
          <div className="rounded-lg border border-[var(--status-info-border)] bg-[var(--status-info-background)]/30 p-3">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className={SETTINGS_CALLOUT_TITLE_CLASS}>{t('settings.varin.tunnel.section.redeemedAccessLinks')}</p>
            </div>
            <div className="space-y-1">
              {renderedSessionRecords.map((record) => {
                ...
                const modeLabel = isQuick
                  ? t('settings.varin.tunnel.badge.quick')
                  : isManagedRemote
                    ? t('settings.varin.tunnel.badge.remote')
                    : t('settings.varin.tunnel.badge.local');
                ...
                return (
                  ...
                  >
                    ...
                    <span className="typography-meta text-muted-foreground/80">
                      {t('settings.varin.tunnel.session.redeemedAt', { time: formatAbsoluteTime(record.createdAt, timeFormatPreference) })}
                    </span>
                    <span className="typography-meta text-foreground">
                      {record.isActive
                        ? t('settings.varin.tunnel.session.expiresIn', { remaining: record.remainingTextForSession })
                        : (record.inactiveLabel === t('settings.varin.tunnel.state.inactive')
                          ? t('settings.varin.tunnel.state.inactive')
                          : t('settings.varin.tunnel.session.inactiveWithReason', { reason: record.inactiveLabel }))}
                    </span>
                  </div>
                );
              })}
          ...
          <div className="flex items-start gap-2 rounded-lg border border-[var(--status-warning)]/30 bg-[var(--status-warning)]/5 p-3">
            ...
            <div className="space-y-1">
              <p className={SETTINGS_CALLOUT_TITLE_CLASS}>
                {t('settings.varin.tunnel.notAvailable.dependencyNotFound', { dependency: displayedDependencyInstallInfo.dependency })}
              </p>
              <p className="typography-meta text-muted-foreground/70">{t('settings.varin.tunnel.notAvailable.installHint')}</p>
        ...
        <section className="space-y-4 px-2 pb-2 pt-0">
          <div className="space-y-3">
            <div data-settings-item="tunnel.provider" className="space-y-1.5">
              <p className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.varin.tunnel.field.provider')}</p>
              <Select
              ...
              >
                <SelectTrigger size={SETTINGS_SELECT_SIZE} className="max-w-[16rem]">
                  <SelectValue placeholder={t('settings.varin.tunnel.field.providerPlaceholder')}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="__more-soon" disabled>{t('settings.varin.tunnel.option.moreProvidersSoon')}</SelectItem>
                </SelectContent>
              </Select>
            </div>

            <div data-settings-item="tunnel.type" className="space-y-1.5">
              <p className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.varin.tunnel.field.tunnelType')}</p>
          ...
          <div data-settings-item="tunnel.ttl" className="mt-2 grid grid-cols-1 gap-2 py-1.5 md:grid-cols-[14rem_auto] md:gap-x-8 md:gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className={cn(SETTINGS_FIELD_LABEL_CLASS, 'shrink-0')}>{t('settings.varin.tunnel.field.connectLinkTtl')}</span>
              <Select
            ...
            <div className="flex min-w-0 items-center gap-2">
              <span className={cn(SETTINGS_FIELD_LABEL_CLASS, 'shrink-0')}>{t('settings.varin.tunnel.field.tunnelSessionTtl')}</span>
              <Select
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="typography-meta text-[var(--status-warning)]">
                    {t('settings.varin.tunnel.option.mode.quick.tooltip')}
                  </p>
                  {providerSupportsManagedModes && (
                    <p className="typography-meta mt-1 text-[var(--status-warning)]">
                      {t('settings.varin.tunnel.warning.quickModeReliability')}
                    </p>
                  )}
                </div>
              </div>
          ...
          {tunnelMode === 'managed-remote' && (
            <div data-settings-item="tunnel.managed-remote" className="space-y-2 rounded-lg border border-[var(--interactive-border)] bg-[var(--surface-elevated)] p-3">
              {typeof suggestedConnectorPort === 'number' && (
                <div className="rounded-md border border-[var(--status-info-border)] bg-[var(--status-info-background)]/35 px-2 py-1.5">
                  <p className="typography-meta text-[var(--status-info)]">
                    {t('settings.varin.tunnel.note.cloudflareConnectorTarget')} <code>http://localhost:{suggestedConnectorPort}</code>
                  </p>
                </div>
              )}

              <div className="mb-1 flex items-center justify-between gap-3">
                <SettingsGroupTitle>{t('settings.varin.tunnel.section.savedManagedRemoteTunnels')}</SettingsGroupTitle>
                <Button
                  variant="ghost"
                  size="xs"
                  className="!font-normal"
              ...
              {managedRemoteTunnelPresets.length > 0 ? (
                        ...
                        >
                          <div className="flex items-start gap-2 px-3">
                            ...
                            <Button
                              ...
                              aria-label={t('settings.varin.tunnel.actions.removePresetAria', { name: preset.name })}
                              onClick={() => {
                          ...
                          <CollapsibleContent className="pt-1.5">
                            <div className="space-y-1 px-3 pb-2">
                              <p className="typography-meta text-muted-foreground/70">{t('settings.varin.tunnel.field.hostnameLabel')} <code>{preset.hostname}</code></p>
                              <Input
                                type="password"
                                value={rowToken}
                                onChange={(event) => {
                                ...
                                placeholder={hasSavedToken ? t('settings.varin.tunnel.field.savedTokenAvailablePlaceholder') : t('settings.varin.tunnel.field.pasteTokenPlaceholder')}
                                className="h-7"
                              ...
                              <div className="flex items-center justify-end">
                                ...
                                >
                                  {t('settings.varin.tunnel.actions.saveToken')}
                                </Button>
                              </div>
                            </div>
                          </CollapsibleContent>
                ...
                <p className="typography-meta text-muted-foreground/70">{t('settings.varin.tunnel.empty.noManagedRemoteTunnels')}</p>
              )}
              ...
              {isAddingPreset && (
                <div className="space-y-2 rounded-md border border-[var(--surface-subtle)] p-2">
                  <Input
                    ...
                    placeholder={t('settings.varin.tunnel.field.newPresetNamePlaceholder')}
                    className="h-7"
                    disabled={isSavingMode || state === 'starting' || state === 'stopping'}
                  />
                  <Input
                    ...
                    placeholder={t('settings.varin.tunnel.field.newPresetHostnamePlaceholder')}
                    className="h-7"
                    disabled={isSavingMode || state === 'starting' || state === 'stopping'}
                  />
                  <Input
                    ...
                    placeholder={t('settings.varin.tunnel.field.newPresetTokenPlaceholder')}
                    className="h-7"
                    disabled={isSavingMode || state === 'starting' || state === 'stopping'}
                  />
                  {typeof suggestedConnectorPort === 'number' && (
                    <p className="typography-meta text-muted-foreground/70">
                      {t('settings.varin.tunnel.note.cloudflareConnectorTargetUse')} <code>http://localhost:{suggestedConnectorPort}</code>.
                    </p>
                  )}
              ...
              <div className="flex items-center gap-1.5">
                <p className="typography-meta text-muted-foreground/80">{t('settings.varin.tunnel.note.tokensSavedPerTunnel')}</p>
                <SettingsInfoHint>
                  {t('settings.varin.tunnel.tooltip.tokensSavedPath')}
                </SettingsInfoHint>
              </div>
          ...
          {tunnelMode === 'managed-local' && (
            <div data-settings-item="tunnel.managed-local-config" className="space-y-2 rounded-lg border border-[var(--interactive-border)] bg-[var(--surface-elevated)] p-3">
              <div className="space-y-1.5">
                <p className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.varin.tunnel.field.configurationFile')}</p>
                <input
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder={t('settings.varin.tunnel.field.configurationFilePlaceholder')}
                    className="h-7"
                    disabled={state === 'starting' || state === 'stopping' || isSavingMode}
                  />
                  <Button
                    ...
                    aria-label={t('settings.varin.tunnel.actions.browseConfigFileAria')}
                    onClick={() => {
                  ...
                  {managedLocalConfigPath && (
                    <Button
                      ...
                      size="xs"
                      className="h-7 w-7 p-0"
                      aria-label={t('settings.varin.tunnel.actions.clearConfigFileAria')}
                      onClick={() => {
                ...
                <p className="typography-meta text-muted-foreground/70">
                  {managedLocalConfigPath
                    ? t('settings.varin.tunnel.note.customConfigUsed')
                    : t('settings.varin.tunnel.note.defaultConfigUsed')}
                </p>
          ...
          {!isSelectedModeTunnelReady && (
            <div data-settings-item="tunnel.start" className="space-y-6">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="space-y-1">
                    {tunnelMode === 'managed-remote' && (
                      <>
                        <p className="typography-meta text-[var(--status-info)]">
                          {t('settings.varin.tunnel.note.managedRemoteRequiresDomain')}
                        </p>
                        <button
                          type="button"
                        ...
                        >
                          {t('settings.varin.tunnel.actions.openManagedRemoteDocs')}
                    ...
                    {tunnelMode === 'managed-local' && (
                      <>
                        <p className="typography-meta text-[var(--status-info)]">
                          {t('settings.varin.tunnel.note.managedLocalUsesConfig')}
                        </p>
                        <button
                          type="button"
                          className="typography-meta inline-flex items-center gap-1 text-[var(--status-info)] underline underline-offset-2 hover:opacity-90"
                        ...
                        >
                          {t('settings.varin.tunnel.actions.openManagedLocalDocs')}
                          <Icon name="external-link" className="size-3.5" />
                        </button>
                      </>
                    )}
                    <p className="typography-meta text-[var(--status-info)]">
                      {t('settings.varin.tunnel.note.startModeAndGenerateLink', {
                        mode: tUnsafe(TUNNEL_MODE_OPTIONS.find((option) => option.value === tunnelMode)?.labelKey ?? 'settings.varin.tunnel.option.mode.quick.label'),
                      })}
                    </p>
                  </div>
                </div>
              ...
              {tunnelMode === 'managed-remote' && (
                <div className="space-y-1.5">
                  <p className={SETTINGS_FIELD_LABEL_CLASS}>{t('settings.varin.tunnel.field.managedRemoteTunnelToConnect')}</p>
                  <Select
                  ...
                  >
                    <SelectTrigger size={SETTINGS_SELECT_SIZE}>
                      <SelectValue placeholder={t('settings.varin.tunnel.field.selectSavedTunnelPlaceholder')}>
                        {selectedPreset?.name}
                      </SelectValue>
                    </SelectTrigger>
              ...
              {willReplaceActiveTunnel && (
                <div className="rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] p-3">
                  <div className="flex items-start gap-2">
                    <Icon name="error-warning" className="mt-0.5 size-4 shrink-0 text-[var(--status-warning)]" />
                    <p className="typography-meta text-[var(--status-warning)]">
                      {t('settings.varin.tunnel.warning.replacesActiveTunnel')}
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                {state === 'starting'
                  ? <><Icon name="loader-4" className="size-3.5 animate-spin" /> {t('settings.varin.tunnel.actions.startingTunnel')}</>
                  : t('settings.varin.tunnel.actions.startTunnel')}
              </Button>
            </div>
          )}
        ...
        <section data-settings-item="tunnel.start" className="space-y-4 px-2 pb-2 pt-0">
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="typography-meta font-medium text-foreground">{t('settings.varin.tunnel.state.tunnelReady')}</p>
            </div>

            <div>
              <p className="typography-meta mb-1 text-muted-foreground/70">{t('settings.varin.tunnel.field.publicUrlHint')}</p>
            ...
            {isConnectLinkLive && tunnelInfo.connectUrl && (
              <>
                <div>
                  <p className="typography-meta mb-1 text-muted-foreground/70">{t('settings.varin.tunnel.field.connectLink')}</p>
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={handleCopyUrl} className="shrink-0 gap-1.5">
                      ...
                      {copied ? t('settings.varin.tunnel.actions.copied') : t('settings.common.actions.copyAll')}
                    </Button>
                  </div>
                  <p className="typography-meta mt-1 text-muted-foreground/70">
                    {t('settings.varin.tunnel.field.expires')}: {tunnelInfo.bootstrapExpiresAt ? remainingText : t('settings.varin.tunnel.state.never')}
                  </p>
                </div>

                <div className="flex flex-col items-center gap-2 rounded-lg border border-border/50 bg-[var(--surface-elevated)] p-4">
                  {qrDataUrl
                    ? <img src={qrDataUrl} alt={t('settings.varin.tunnel.field.connectQrAlt')} className="size-48" />
                    : <div className="size-48 rounded bg-muted/30" />}
                  <p className="typography-meta text-muted-foreground">{t('settings.varin.tunnel.note.scanQrToConnect')}</p>
                </div>
              </>
            )}
          </div>
          ...
          <div className="pt-1">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {t('settings.varin.tunnel.actions.newConnectLink')}
              </Button>
              ...
              >
                {state === 'stopping'
                  ? <><Icon name="loader-4" className="size-3.5 animate-spin" /> {t('settings.varin.tunnel.actions.stopping')}</>
                  : t('settings.varin.tunnel.actions.stopTunnel')}
              </Button>
            </div>
          </div>
        </section>
      ...
      {state === 'error' && errorMessage && (
        <section className="space-y-3 px-2 pb-2 pt-0">
          ...
          <Button size="sm" variant="ghost" onClick={handleStart}>{t('settings.varin.tunnel.actions.retry')}</Button>
        </section>
      )}
      </div>
    </SettingsSection>
