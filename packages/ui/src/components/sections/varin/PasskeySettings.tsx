...
export const PasskeySettings: React.FC = () => {
  ...
  const loadPasskeys = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.varin.passkeys.toast.loadFailed');
  ...
  const handleRegisterPasskey = React.useCallback(async () => {
    if (!status.enabled) {
      const message = t('settings.varin.passkeys.toast.enableUiPasswordFirst');
      setErrorMessage(message);
    ...
    try {
      ...
      await loadPasskeys();
      toast.success(t('settings.varin.passkeys.toast.added'));
    } catch (error) {
      if (isPasskeyCeremonyAbort(error)) {
        toast.message(t('settings.varin.passkeys.toast.setupCanceled'));
        return;
      }

      const message = error instanceof Error ? error.message : t('settings.varin.passkeys.toast.addFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setIsRegistering(false);
  ...
  const handleRevokePasskey = React.useCallback(async (id: string) => {
    ...
    try {
      ...
      setStatus(await fetchPasskeyStatus());
      await loadPasskeys();
      toast.success(t('settings.varin.passkeys.toast.removed'));
    } catch (error) {
      const message = error instanceof Error ? error.message : t('settings.varin.passkeys.toast.removeFailed');
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setRevokingId(null);
  ...
  const handleResetAllAuth = React.useCallback(async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : t('settings.varin.passkeys.toast.clearAuthFailed');
      setErrorMessage(message);
      toast.error(message);
  ...
  return (
    <SettingsSection title={t('settings.varin.passkeys.title')}>
      <div className="space-y-2">
        <SettingsFieldRow label={t('settings.varin.passkeys.field.currentDevice')}>
          <Button
            type="button"
          ...
          >
            {isRegistering ? t('settings.varin.passkeys.actions.cancelSetup') : t('settings.varin.passkeys.actions.add')}
          </Button>
          <Button
            type="button"
            variant="ghost"
          ...
          >
            {isResetting ? t('settings.varin.passkeys.actions.signingOut') : t('settings.varin.passkeys.actions.signOutEverywhere')}
          </Button>
        </SettingsFieldRow>

        {!status.enabled && (
          <p className="typography-meta text-muted-foreground">
            {t('settings.varin.passkeys.state.uiPasswordRequired')}
          </p>
        )}
        ...
        {isLoading ? (
          <p className="typography-meta text-muted-foreground">{t('settings.varin.passkeys.state.loading')}</p>
        ) : passkeys.length === 0 ? (
          <p className="typography-meta text-muted-foreground">{t('settings.varin.passkeys.state.noneSaved')}</p>
        ) : (
            ...
            {passkeys.map((passkey) => (
              ...
              >
                <span className="typography-meta text-muted-foreground truncate">
                  {passkey.lastUsedAt
                    ? t('settings.varin.passkeys.item.lastUsed', {
                        time: formatTimestamp(passkey.lastUsedAt, t('settings.varin.passkeys.time.neverUsed'), timeFormatPreference),
                      })
                    : t('settings.varin.passkeys.item.added', {
                        time: formatTimestamp(passkey.createdAt, t('settings.varin.passkeys.time.neverUsed'), timeFormatPreference),
                      })}
                </span>
                <Button
                  type="button"
                  ...
                  className="!font-normal text-muted-foreground hover:text-foreground"
                >
                  {revokingId === passkey.id ? t('settings.varin.passkeys.actions.removing') : t('settings.common.actions.delete')}
                </Button>
              </SettingsFieldRow>
            ))}
          </div>
