...
export const buildRevertedMessageDockState = (
    ...
    const userMessages = sortMessagesForRevert(
        (state.message[sessionId] ?? []).filter(isUserMessage),
    );
    const revertedUserMessages = partitionMessagesByRevert(userMessages, revertMessageID).removed;
    const records: RevertedMessageRecord[] = [];
    for (const message of revertedUserMessages) {
        records.push({
            message,
