...
describe('buildRevertedMessageDockState', () => {
    ...
    test('uses chronological revert order when message IDs are not time ordered', () => {
        const earlier = {
            ...message('z_earlier', 'user'),
            time: { created: 10 },
        } as Message;
        const target = {
            ...message('a_target', 'user'),
            time: { created: 20 },
        } as Message;
        const later = {
            ...message('m_later', 'user'),
            time: { created: 30 },
        } as Message;

        const result = buildRevertedMessageDockState(
            state({
                session: [{ id: 'ses_1', revert: { messageID: 'a_target' } } as State['session'][number]],
                message: { ses_1: [later, earlier, target] },
            }),
            'ses_1',
        );

        expect(result.records.map((record) => record.message.id)).toEqual(['a_target', 'm_later']);
    });
});
