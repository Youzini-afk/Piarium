// A tiny CSI cursor sequence can otherwise expand into gigabytes of spaces.
// This budget applies only to synthetic rows/columns; real command output is not truncated here.
...
/** Render the final visible terminal screen as plain text for persisted command output. */
export const renderTerminalOutput = (output: string): string => {
    ...
    for (let index = 0; index < output.length; index += 1) {
        ...
        if (nextCharacter === '[') {
            ...
            if (sequenceEnd === output.length) break;

            const command = output[sequenceEnd];
            const parameters = output.slice(sequenceStart, sequenceEnd)
                .split(';')
            ...
            if (command === 'A') {
                ...
                if (mode === 1) {
                    for (let cursor = 0; cursor <= column && cursor < line.length; cursor += 1) {
                        line[cursor] = ' ';
                    }
                } else if (mode === 2) {
                    lines[row] = [];
        ...
        if (nextCharacter === ']') {
            const bellTerminator = output.indexOf('\u0007', index + 2);
            const stringTerminator = output.indexOf('\u001B\\', index + 2);
            const end = bellTerminator === -1
                ? stringTerminator
                : stringTerminator === -1
                    ? bellTerminator
...
export const getToolOutput = (
    ...
    const shouldNormalize = tool === 'bash' && status !== 'running';
