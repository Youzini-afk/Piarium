/**
 * Varin deep-link vocabulary — the single source of truth for the `varin://`
...
export const DEEP_LINK_SCHEME = 'varin';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (varin://session/<id>),
  // but be tolerant of authority-less forms (varin:/session/<id>) where it lands in
...
/**
 * Parse a raw `varin://…` string into a typed intent, or `null` if it isn't a
 * recognised Varin deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `varin://…` URL for an intent. Used by anything that needs to hand
