/**
 * Piarium deep-link vocabulary — the single source of truth for the `piarium://`
...
export const DEEP_LINK_SCHEME = 'piarium';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (piarium://session/<id>),
  // but be tolerant of authority-less forms (piarium:/session/<id>) where it lands in
...
/**
 * Parse a raw `piarium://…` string into a typed intent, or `null` if it isn't a
 * recognised Piarium deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `piarium://…` URL for an intent. Used by anything that needs to hand
