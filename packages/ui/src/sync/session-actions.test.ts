...
const globalUnarchivedSessionIds: string[] = []
  ...
  useGlobalSessionsStore: {
    getState: () => ({
      ...
      unarchiveSessions: (ids: Iterable<string>) => {
        globalUnarchivedSessionIds.push(...ids)
      },
    }),
  },
}))
...
beforeEach(() => {
  setSyncRefs(
    ...
    createChildStores([]),
    "",
    (sessionID, directory) => registeredSessionDirectories.push({ sessionID, directory }),
  )
})
...
describe("unarchiveSession", () => {
  beforeEach(() => {
    replyCalls.length = 0
    globalUpsertedSessions.length = 0
    globalUnarchivedSessionIds.length = 0
    registeredSessionDirectories.length = 0
    sessionUpdateResult = {}
  })

  test("restores an archived session to the active global collection", async () => {
    ...
    sessionUpdateResult = { data: restoredSession }
    ...
    expect(globalUnarchivedSessionIds).toEqual(["session-a"])
