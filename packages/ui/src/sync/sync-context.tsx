...
import {
  ...
  getSessionMessageMaterializationReadiness,
...
const getUniqueActiveSessionCandidate = (
  ...
  for (const [directory, store] of childStores.children) {
    const candidates = getActiveSessionCandidateIds(directory, getDirectoryEventState(store, batch))
...
export const resolveDirectoryFromRoutingIndex = (
  ...
  if (messageID) {
    ...
    const unique = getUniqueActiveSessionCandidate(childStores, batch)
...
export const resolveMaterializationSessionId = (
  ...
  const storeSessionID = findSessionIdForMessageInStore(store, messageID, batch)
  if (storeSessionID) return storeSessionID
  const unique = getUniqueActiveSessionCandidate(childStores, batch)
  return unique?.directory === directory ? unique.sessionID : undefined
}
...
const resolveMaterializationSessionID = (
  ...
  if (messageID) {
    const indexedSessionID = routingIndex.messageSessionById.get(messageID)
    if (indexedSessionID) return indexedSessionID
    const store = childStores.getChild(resolvedDirectory)
    if (store) {
      const storeSessionID = findSessionIdForMessageInStore(store, messageID, batch)
      if (storeSessionID) return storeSessionID
    }
  ...
  const unique = getUniqueActiveSessionCandidate(childStores, batch)
  return unique?.directory === resolvedDirectory ? unique.sessionID : undefined
}
...
export function useEnsureSessionMessages(sessionID: string, directory?: string) {
  const { runtimeKey } = useSyncSystem()
  const syncDirectory = useSyncDirectory()
  const resolvedDirectory = directory ?? syncDirectory
  const store = useDirectoryStore(resolvedDirectory)
  const requestGenerationRef = React.useRef(0)
  const readiness = useDirectorySync(
    useCallback(
      (state: State) => getSessionMessageMaterializationReadiness(state, sessionID),
      [sessionID],
    ),
    resolvedDirectory,
  )
  ...
  React.useEffect(() => {
    if (!sessionID || readiness !== "needs-materialization") return

    const loadingKey = JSON.stringify([runtimeKey, resolvedDirectory, sessionID])
  ...
  }, [readiness, runtimeKey, sessionID, store, resolvedDirectory])
}
