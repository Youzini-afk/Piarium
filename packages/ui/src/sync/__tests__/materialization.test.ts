...
describe("getSessionMessageMaterializationReadiness", () => {
  test("becomes loadable when a delayed child session enters the directory store", () => {
    const missingState = { session: [], message: {}, part: {} }
    expect(getSessionMessageMaterializationReadiness(missingState, "ses_child")).toBe("missing-session")

    const discoveredState = {
      ...missingState,
      session: [{ id: "ses_child" }],
    }
    expect(getSessionMessageMaterializationReadiness(discoveredState, "ses_child")).toBe("needs-materialization")

    const materializedState = materializeSessionSnapshots(
      discoveredState,
      "ses_child",
      [{ info: message("msg_child", "ses_child"), parts: [part("prt_child", "msg_child")] }],
    )
    expect(getSessionMessageMaterializationReadiness(
      { ...materializedState, session: discoveredState.session },
      "ses_child",
    )).toBe("renderable")
  })
})
