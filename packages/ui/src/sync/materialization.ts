...
export type SessionMessageMaterializationReadiness = "missing-session" | "needs-materialization" | "renderable"

export function getSessionMessageMaterializationReadiness(
  state: MaterializedState & { session: Array<{ id: string }> },
  sessionID: string,
): SessionMessageMaterializationReadiness {
  if (getSessionMaterializationStatus(state, sessionID).renderable) return "renderable"
