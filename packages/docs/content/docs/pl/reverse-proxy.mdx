---
title: Reverse proxy
description: Skonfiguruj Piarium poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Piarium za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Piarium działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Piarium już kompresuje odpowiedzi.
...
- Piarium dostępny bezpośrednio w LAN
...
- Piarium kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Piarium, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
