---
title: Reverse proxy
description: Skonfiguruj Varin poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Varin za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Varin działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Varin już kompresuje odpowiedzi.
...
- Varin dostępny bezpośrednio w LAN
...
- Varin kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Varin, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
