...
Zaplanowane zadanie uruchamia za Ciebie prompt według harmonogramu — na przykład codzienne „podsumuj wczorajsze zmiany” albo cotygodniowe porządki. Gdy się uruchamia, Piarium rozpoczyna nową sesję i sam wysyła prompt. Otwórz harmonogram z przycisku u góry paska bocznego sesji.
...
Zaznacz **Uruchom jako cel**, aby uruchomienie doprowadziło prompt do końca zamiast zatrzymywać się po jednej odpowiedzi — zobacz [Cele sesji](/pl/session-goals/).
...
Zadania uruchamiają się tylko wtedy, gdy serwer Piarium działa. Jeśli go zamkniesz, zaplanowane uruchomienia są wstrzymane, dopóki nie wróci.
