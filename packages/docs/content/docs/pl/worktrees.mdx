...
3. Potwierdź folder worktree (Varin proponuje jeden na podstawie nazwy gałęzi).
4. Utwórz go.

Varin tworzy gałąź, konfiguruje folder i uruchamia w nim sesję. Możesz też wystartować jedną prosto z [zadania](/pl/notes-todos-plans/) albo z [issue lub PR na GitHub](/pl/github/).
...
Worktree może wymagać uwagi, jeśli jego folder zniknął, jego gałąź jest w stanie odłączonym (detached) albo scalanie lub rebase jest niedokończone. Varin to oznacza, byś mógł to naprawić — zobacz [Worktree i Git](/pl/troubleshooting/worktrees-git/).

## Powiązane

- [Multi-run](/pl/multi-run/) — uruchom wiele sesji worktree naraz
- [Przepływy Git i GitHub](/pl/git/) — commituj i integruj z wnętrza Varin
