---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do Varin z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do Varin z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.
...
- potwierdź, że serwer działa, za pomocą `varin status`
...
- przy parowaniu **Wszędzie** sprawdź **Settings → Remote Instances → Varin Relay** na serwerze: status powinien brzmieć **Połączono**; jeśli nie, wyłącz i włącz relay ponownie
...
- uruchom `varin tunnel status --all`
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, Varin wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — Varin nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś Varin za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse proxy](/pl/reverse-proxy/).
