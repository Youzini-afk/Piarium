---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do Piarium z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do Piarium z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.

## Najpierw sprawdź podstawy

- najpierw otwórz `http://localhost:3000` na tym samym komputerze — jeśli to zawiedzie, to nie jest problem zdalny; zobacz [Rozwiązywanie problemów](/pl/troubleshooting/)
- potwierdź, że serwer działa, za pomocą `piarium status`
...
- przy parowaniu **Wszędzie** sprawdź **Settings → Remote Instances → Piarium Relay** na serwerze: status powinien brzmieć **Połączono**; jeśli nie, wyłącz i włącz relay ponownie
...
- uruchom `piarium tunnel status --all`
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, Piarium wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — Piarium nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś Piarium za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse proxy](/pl/reverse-proxy/).
