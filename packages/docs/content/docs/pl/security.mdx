...
Varin daje dostęp do Twojej maszyny i Twojego kodu, więc zabezpiecz go, zanim ktokolwiek poza Tobą będzie mógł do niego dotrzeć. Ta strona omawia hasło UI, klucze dostępu (passkeys) oraz to, co warto wiedzieć przed udostępnieniem Varin w sieci.

## Ustaw hasło UI

Uruchom Varin z hasłem, a interfejs webowy poprosi o nie:

```bash
varin --ui-password be-creative-here
```

Możesz też ustawić je za pomocą zmiennej środowiskowej `VARIN_UI_PASSWORD`, zamiast podawać je w wierszu poleceń. Po zalogowaniu Varin zapamiętuje urządzenie na pewien czas, więc nie pyta za każdym razem.
...
Po ustawieniu hasła możesz dodać klucze dostępu (Face ID, Touch ID, klucz bezpieczeństwa) dla szybszego logowania. Dodaj je w **Settings → Varin → Passkeys**.
...
- Domyślnie Varin nasłuchuje tylko na Twojej własnej maszynie (`127.0.0.1`). Nasłuchiwanie szerzej wymaga celowej zmiany i najpierw powinieneś ustawić hasło.
...
- Jeśli umieszczasz Varin za własnym serwerem HTTPS, zobacz [Reverse proxy](/pl/reverse-proxy/).
...
- [Reverse proxy](/pl/reverse-proxy/) — uruchom Varin za własnym serwerem
