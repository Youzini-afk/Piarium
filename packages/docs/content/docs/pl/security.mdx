...
Piarium daje dostęp do Twojej maszyny i Twojego kodu, więc zabezpiecz go, zanim ktokolwiek poza Tobą będzie mógł do niego dotrzeć. Ta strona omawia hasło UI, klucze dostępu (passkeys) oraz to, co warto wiedzieć przed udostępnieniem Piarium w sieci.

## Ustaw hasło UI

Uruchom Piarium z hasłem, a interfejs webowy poprosi o nie:

```bash
piarium --ui-password be-creative-here
```

Możesz też ustawić je za pomocą zmiennej środowiskowej `PIARIUM_UI_PASSWORD`, zamiast podawać je w wierszu poleceń. Po zalogowaniu Piarium zapamiętuje urządzenie na pewien czas, więc nie pyta za każdym razem.
...
Po ustawieniu hasła możesz dodać klucze dostępu (Face ID, Touch ID, klucz bezpieczeństwa) dla szybszego logowania. Dodaj je w **Settings → Piarium → Passkeys**.
...
- Domyślnie Piarium nasłuchuje tylko na Twojej własnej maszynie (`127.0.0.1`). Nasłuchiwanie szerzej wymaga celowej zmiany i najpierw powinieneś ustawić hasło.
...
- Jeśli umieszczasz Piarium za własnym serwerem HTTPS, zobacz [Reverse proxy](/pl/reverse-proxy/).
...
- [Reverse proxy](/pl/reverse-proxy/) — uruchom Piarium za własnym serwerem
