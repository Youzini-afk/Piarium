---
title: Proxy reverso
description: Configure o Piarium corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o Piarium atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o Piarium funciona diretamente.
...
- Desative o gzip no proxy se o Piarium já comprime as respostas.
...
- Piarium acessível diretamente na LAN
...
- O Piarium comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo Piarium, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
