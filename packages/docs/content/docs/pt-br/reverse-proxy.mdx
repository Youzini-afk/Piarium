---
title: Proxy reverso
description: Configure o Varin corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o Varin atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o Varin funciona diretamente.
...
- Desative o gzip no proxy se o Varin já comprime as respostas.
...
- Varin acessível diretamente na LAN
...
- O Varin comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo Varin, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
