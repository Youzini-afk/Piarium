...
description: Conecte o app de desktop ao Varin em outra máquina via SSH.
---

# Instâncias Remotas

O app de desktop pode conectar ao Varin rodando em outra máquina via SSH — um servidor de trabalho, uma máquina na nuvem, um homelab — e trazer a UI dela para a sua tela como se fosse local. Configure isso em **Settings → Remote Instances**.
...
3. Escolha como o Varin roda ali:
   - **managed** — o Varin se instala e inicia na máquina remota
   - **external** — conecte a um que já está em execução
4. Conecte.

O Varin percorre as etapas — verificando a conexão, configurando o remoto, iniciando o servidor e encaminhando a porta — e mostra em que ponto está em cada estágio. Quando chega a **ready**, a UI remota carrega localmente.

## Credenciais

Você decide se quer salvar as senhas SSH e de UI ou inseri-las a cada vez. Se a conexão cair, o Varin informa qual etapa falhou para você corrigi-la — veja [Acesso remoto](/pt-br/troubleshooting/remote-access/).

## Links de conexão

Se uma máquina remota já executa Varin, a forma mais fácil de conectar o app de desktop é um link de pareamento. Na UI do servidor remoto, abra **Settings → Remote Instances → Conectar a este servidor → Adicionar um dispositivo**, crie um link e importe-o no seu desktop em **Settings → Remote Instances → Outros servidores Varin → Importar link**. Veja [Conectar um Dispositivo](/pt-br/connect-devices/) para o fluxo completo.
