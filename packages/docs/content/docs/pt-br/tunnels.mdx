---
title: Túneis
description: Exponha o Piarium com segurança para acesso remoto e móvel.
---

# Túneis

Um túnel é um link público para o seu Piarium, para que você possa acessá-lo de um navegador comum em outra rede. Use `piarium tunnel` para criar um para uma instância em execução.
...
O Piarium inicia o CLI do provedor de túnel na sua máquina. Instale primeiro o provedor que você quer usar:
...
Se você pular esta etapa, `piarium tunnel start` pode iniciar automaticamente um servidor CLI. Ao iniciar automaticamente, você pode passar opções do servidor como `--port`, `--host`, `--lan`, `--ui-password` e `--api-only`.
...
Quando o túnel estiver no ar, o `status` mostra uma URL pública. Abra-a, ou escaneie o QR code, para acessar o Piarium de qualquer lugar.

Por padrão, o Piarium imprime um QR code em sessões TTY interativas. Use `--qr` para forçar a saída do QR, ou `--no-qr` para desativá-la.
...
- um único túnel ativo por instância do Piarium (porta)
...
- [PWA e Acesso Móvel](/pt-br/mobile/) — acesse o Piarium pelo celular
