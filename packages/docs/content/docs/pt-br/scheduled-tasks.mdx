...
Uma tarefa agendada executa um prompt para você em uma agenda — por exemplo, um "resumir as mudanças de ontem" diário ou uma limpeza semanal. Quando ela é executada, o Piarium inicia uma nova sessão e envia o prompt sozinho. Abra o agendador pelo botão no topo da barra lateral de sessões.
...
Marque **Executar como objetivo** para que a execução persiga o prompt até concluir em vez de parar após uma resposta — veja [Objetivos de sessão](/pt-br/session-goals/).
...
As tarefas só disparam enquanto o servidor do Piarium estiver em execução. Se você o fechar, as execuções agendadas ficam pausadas até ele voltar a funcionar.
