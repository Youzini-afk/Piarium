...
description: Pair your phone, desktop, or another browser with your Piarium server using a one-time QR code.
---

# Connect a Device

Pair another device — the mobile app, the desktop app, or a browser on another machine — with your Piarium server by scanning a one-time QR code. This is the recommended way to connect devices; there are no ports to open and no addresses to type.

## Pair a device

1. On the machine running Piarium, open **Settings → Remote Instances → Connect to this server** and press **Add a device**.
...
3. Pick where you'll use the device:
   ...
   - **Anywhere** — works at home and away; away traffic goes through the [Private Relay](/en/private-relay/), an end-to-end encrypted tunnel with no setup needed
...
5. On the other device, scan the code:
   ...
   - **desktop app** — copy the connection link instead and paste it in **Settings → Remote Instances → Other Piarium servers → Import Link**
...
- **Away-from-home traffic is end-to-end encrypted.** With **Anywhere**, traffic outside your network rides the [Private Relay](/en/private-relay/), which cannot read what passes through it.
...
A `--relay` link carries both routes, just like the dialog: the device connects directly over your local network when it can reach the server, and falls back to the [Private Relay](/en/private-relay/) when away. The relay starts on its own: a running instance picks the link up within a minute, a stopped one on its next launch.

> The direct route only works if the server actually listens on your network. By default Piarium listens on the machine itself only — start it with `--lan` to make it reachable over Wi-Fi. The command warns you (`[LAN_UNREACHABLE]`) when the link's direct route won't be usable from other devices; a `--relay` link still works then, just always through the relay.

The printed link and QR code work exactly like the ones from the settings dialog — single-use, expiring, revocable.

## Related

- [Private Relay](/en/private-relay/) — how "Anywhere" connections work and what the relay can and cannot see
- [Mobile Apps](/en/mobile/) — install the iOS or Android app
- [Remote Instances](/en/remote-instances/) — connect the desktop app to servers over SSH or links
- [Remote access](/en/troubleshooting/remote-access/) — when a device won't connect
