...
For each connected provider, Piarium shows:
...
A provider only shows usage once you've signed in to it on the [Providers](/en/providers/) page. A few providers need an extra step — for example, Ollama Cloud reads a session file you set up separately. If a provider shows no data, it usually means that extra credential is missing.

## Related

- [Providers, Models & Agents](/en/providers/) — sign in before usage appears
