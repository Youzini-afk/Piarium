...
3. Confirm the worktree folder (Piarium suggests one from the branch name).
4. Create it.

Piarium makes the branch, sets up the folder, and starts a session in it. You can also kick one off straight from a [todo](/en/notes-todos-plans/) or a [GitHub issue or PR](/en/github/).
...
A worktree can need attention if its folder went missing, its branch is in a detached state, or a merge or rebase is half-finished. Piarium flags these so you can fix them — see [Worktrees & Git](/en/troubleshooting/worktrees-git/).

## Related

- [Multi-run](/en/multi-run/) — launch many worktree sessions at once
- [Git & GitHub Workflows](/en/git/) — commit and integrate from inside Piarium
