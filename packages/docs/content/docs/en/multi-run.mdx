...
Turn on **isolate runs** to give every run its own [worktree](/en/worktrees/) and branch, so they never touch the same files. This needs a git repo — it's turned off automatically for folders that aren't one. Pick the branch the runs start from.
...
- [Worktree Sessions](/en/worktrees/) — how isolation works under the hood
