...
A goal turns one prompt into a finish line. Instead of nudging the agent with "continue" after every reply, you set a goal once — and Piarium keeps the session working toward it automatically, checking progress with an independent auditor after every turn. It keeps running even while you are away.
...
- **On a schedule**: check **Run as goal** on a [scheduled task](/en/scheduled-tasks/) to make recurring runs pursue their prompt to completion.
...
After the agent stops and the session stays quiet for a moment, Piarium:
...
- The goal loop runs in the Piarium server, not in your browser tab. Close the tab, lock the phone — the agent keeps working, and you get a notification when the goal settles. The server (desktop app or `piarium` process) must stay running.
...
- [Scheduled Tasks](/en/scheduled-tasks/) — run a prompt on a schedule; enable "Run as goal" there to make a scheduled run pursue its prompt to completion
- [Notifications](/en/notifications/) — how you hear about a finished goal
