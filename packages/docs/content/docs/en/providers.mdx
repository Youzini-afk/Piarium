...
Before Piarium can do anything, it needs at least one AI provider connected. This page covers connecting a provider, choosing a model, and tailoring agents.
...
3. Sign in one of two ways, depending on the provider:
   ...
   - **Sign-in (device flow)** — Piarium shows a link and a short code. Open the link, enter the code, and approve. Piarium finishes connecting on its own.
...
Provider sign-ins are stored by Pi's `AuthStorage`, not in the renderer. API keys and OAuth tokens never appear in provider metadata sent to the UI. User, project, and operator `models.json` layers stay in their native files.

If you set the same provider in more than one place, the most specific setting wins: a trusted project layer overrides your personal one.

## Related

- [MCP Servers](/en/mcp/) — add extra tools for agents
- [Usage & Quotas](/en/usage/) — track how much you've used
