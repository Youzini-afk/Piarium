...
Piarium can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
Piarium creates a remote instance using that SSH command. When it reaches **ready**, the remote Piarium UI opens in the desktop app.
...
Piarium supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. Piarium opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/en/remote-instances/) — connect the desktop app to Piarium on another machine
- [Remote access](/en/troubleshooting/remote-access/) — when SSH or remote access does not connect
