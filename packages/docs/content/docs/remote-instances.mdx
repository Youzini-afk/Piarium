...
description: Connect the desktop app to Varin on another machine over SSH.
---

# Remote Instances

The desktop app can connect to Varin running on another machine over SSH — a work server, a cloud box, a homelab — and bring its UI to your screen as if it were local. Set this up at **Settings → Remote Instances**.

> Remote instances are a **desktop-only** feature. On the web, connect to a remote Varin server with a [tunnel](/tunnels/), a [reverse proxy](/reverse-proxy/), or [pairing](/connect-devices/).
...
3. Choose how Varin runs there:
   - **managed** — Varin installs and starts itself on the remote machine
   - **external** — connect to one that's already running
4. Connect.

Varin walks through the steps — checking the connection, setting up the remote, starting the server, and forwarding the port — and shows where it is at each stage. When it reaches **ready**, the remote UI loads locally.

## Credentials

You decide whether to save the SSH and UI passwords or enter them each time. If the connection drops, Varin reports which step failed so you can fix it — see [Remote access](/troubleshooting/remote-access/).

## Connection links

If a remote machine already runs Varin, the easiest way to connect the desktop app is a pairing link. On the remote server's UI, open **Settings → Remote Instances → Connect to this server → Add a device**, create a link, and import it on your desktop at **Settings → Remote Instances → Other Varin servers → Import Link**. See [Connect a Device](/connect-devices/) for the full flow.
...
- [Tunnels](/tunnels/) — connect to a remote Varin server on the web
