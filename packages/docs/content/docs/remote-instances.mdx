...
> Remote instances are a **desktop-only** feature. On the web or in VS Code, connect to a remote Piarium server with a [tunnel](/tunnels/), a [reverse proxy](/reverse-proxy/), or [pairing](/connect-devices/).
...
You decide whether to save the SSH and UI passwords or enter them each time. If the connection drops, Piarium reports which step failed so you can fix it — see [Remote access](/troubleshooting/remote-access/).
...
- [Tunnels](/tunnels/) — connect to a remote Piarium server on web or VS Code
