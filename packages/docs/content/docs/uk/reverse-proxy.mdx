...
description: Налаштуйте Piarium коректно за Nginx, Nginx Proxy Manager або іншим зворотним проксі.
---

# Зворотний проксі

Скористайтеся цією сторінкою, якщо запускаєте Piarium за Nginx, Nginx Proxy Manager, Caddy, Cloudflare або іншим зворотним проксі.

## Перш ніж проксувати

1. Спершу переконайтеся, що Piarium працює напряму.
...
- Вимкніть gzip на проксі, якщо Piarium уже стискає відповіді.
...
- Piarium доступний напряму в LAN
...
- Piarium стискає HTTP-відповіді через gzip (поріг 1 КБ).
...
SSE-маршрути потоків виключені зі стиснення в Piarium, але CDN усе одно може їх буферизувати. Перевірте документацію свого CDN щодо вимкнення буферизації на SSE-шляхах.
