...
description: Налаштуйте Varin коректно за Nginx, Nginx Proxy Manager або іншим зворотним проксі.
---

# Зворотний проксі

Скористайтеся цією сторінкою, якщо запускаєте Varin за Nginx, Nginx Proxy Manager, Caddy, Cloudflare або іншим зворотним проксі.

## Перш ніж проксувати

1. Спершу переконайтеся, що Varin працює напряму.
...
- Вимкніть gzip на проксі, якщо Varin уже стискає відповіді.
...
- Varin стискає HTTP-відповіді через gzip (поріг 1 КБ).
...
SSE-маршрути потоків виключені зі стиснення в Varin, але CDN усе одно може їх буферизувати. Перевірте документацію свого CDN щодо вимкнення буферизації на SSE-шляхах.
