---
title: Тунелі
description: Безпечно відкрийте Piarium для віддаленого та мобільного доступу.
---

# Тунелі

Тунель — це публічне посилання на ваш Piarium, щоб дістатися до нього зі звичайного браузера в іншій мережі. Скористайтеся `piarium tunnel`, щоб створити його для запущеного інстансу.
...
Piarium запускає CLI провайдера тунелю на вашій машині. Спочатку встановіть провайдера, який хочете використовувати:
...
Якщо пропустити цей крок, `piarium tunnel start` може автоматично запустити CLI server. Під час auto-start можна передати server options: `--port`, `--host`, `--lan`, `--ui-password` і `--api-only`.
...
Коли тунель піднято, `status` показує публічний URL. Відкрийте його або відскануйте QR-код, щоб дістатися до Piarium звідусіль.

Типово Piarium виводить QR-код в інтерактивних TTY-сесіях. Використовуйте `--qr`, щоб примусово вивести QR, або `--no-qr`, щоб вимкнути його.
...
- один активний тунель на інстанс Piarium (порт)
