...
description: Stage, commit, and manage branches without leaving Varin.
---

# Git & GitHub Workflows

Varin has a built-in git view so you can review changes, commit, and manage branches without switching to a terminal. Open it from the **Git** tab in the right sidebar.
...
Then write a commit message and commit. You can have Varin **generate a commit message** from your staged changes — it uses the current session's model, so you need a session open.
...
If a merge, rebase, or integrate hits a conflict, Varin shows what's stuck and lets you resolve it — including handing it to the agent.
