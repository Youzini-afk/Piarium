...
Varin can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
Varin creates a remote instance using that SSH command. When it reaches **ready**, the remote Varin UI opens in the desktop app.
...
Varin supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. Varin opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/remote-instances/) — connect the desktop app to Varin on another machine
