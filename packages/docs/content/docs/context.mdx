...
Every model can only hold so much of a conversation at once — its context. Varin shows how full that is, so you can tell when a session is getting close to the limit and a reply might start dropping older detail.
