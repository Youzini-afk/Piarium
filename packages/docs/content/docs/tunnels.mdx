---
title: Tunnels
description: Expose Varin safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your Varin, so you can reach it from an ordinary browser on another network. Use `varin tunnel` to create one for a running instance.
...
Varin starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
If you skip this step, `varin tunnel start` can auto-start a CLI server. When auto-starting, you can pass server options such as `--port`, `--host`, `--lan`, `--ui-password`, and `--api-only`.
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach Varin from anywhere.

By default, Varin prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per Varin instance (port)
...
- [PWA & Mobile](/mobile/) — reach Varin from your phone
