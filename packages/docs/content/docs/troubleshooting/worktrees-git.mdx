...
Varin flags a worktree when something's off:

- **folder missing** — the worktree's folder was deleted or moved outside Varin; remove the session and create a fresh worktree
...
- on Windows, git uses Unix-style paths (like `/c/Users/...`) — Varin handles this, but custom SSH key paths should follow the same form
