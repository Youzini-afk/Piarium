...
- open `http://localhost:3000` on the same computer first — if that fails, it's not a remote problem; see [troubleshooting](/troubleshooting/)
...
When a [remote instance](/remote-instances/) stalls, Piarium names the step that failed:
...
If you put Piarium behind a reverse proxy and it loads oddly or won't connect, see [Reverse Proxy](/reverse-proxy/).
