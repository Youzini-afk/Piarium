---
title: 터널
description: 원격 및 모바일 접근을 위해 Varin를 안전하게 공개하세요.
---

# 터널

터널은 Varin로 연결되는 공개 링크로, 다른 네트워크의 일반 브라우저에서 접근할 수 있게 해줍니다. 실행 중인 인스턴스에 터널을 만들려면 `varin tunnel`을 사용하세요.
...
Varin는 사용자의 컴퓨터에서 터널 공급자 CLI를 실행합니다. 먼저 사용할 공급자를 설치하세요:
...
이 단계를 건너뛰면 `varin tunnel start`가 CLI 서버를 자동으로 시작할 수 있습니다. 자동 시작 시 `--port`, `--host`, `--lan`, `--ui-password`, `--api-only` 같은 서버 옵션을 함께 전달할 수 있습니다.
...
터널이 올라오면 `status`에 공개 URL이 표시됩니다. 그 URL을 열거나 QR 코드를 스캔하면 어디서든 Varin에 접근할 수 있습니다.

기본적으로 Varin는 대화형 TTY 세션에서 QR 코드를 출력합니다. `--qr`로 QR 출력을 강제하거나 `--no-qr`로 비활성화하세요.
...
- [PWA 및 모바일 접속](/ko/mobile/) — 휴대폰에서 Varin에 접속하세요
