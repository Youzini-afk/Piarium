---
title: Reverse Proxy
description: Configure Piarium correctly behind Nginx, Nginx Proxy Manager, or another reverse proxy.
---

# Reverse Proxy

Use this page if you run Piarium behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Piarium works directly first.
...
- Disable gzip on the proxy if Piarium is already compressing responses.
...
- Piarium reachable directly on LAN
...
- Piarium compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Piarium, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
