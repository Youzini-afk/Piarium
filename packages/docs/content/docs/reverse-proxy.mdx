---
title: Reverse Proxy
description: Configure Varin correctly behind Nginx, Nginx Proxy Manager, or another reverse proxy.
---

# Reverse Proxy

Use this page if you run Varin behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm Varin works directly first.
...
- Disable gzip on the proxy if Varin is already compressing responses.
...
- Varin reachable directly on LAN
...
- Varin compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by Varin, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
