...
2. Add a server and choose its type:
   - **local** — Varin runs a command on your machine. You give it the command to run and, if needed, environment variables.
   - **remote** — Varin connects to a URL someone else hosts. You give it the URL and any headers it needs (for example an auth token).
