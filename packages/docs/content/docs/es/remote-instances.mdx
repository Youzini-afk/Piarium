...
description: Conecta la app de escritorio a Varin en otra máquina por SSH.
---

# Instancias remotas

La app de escritorio puede conectarse a Varin ejecutándose en otra máquina por SSH —un servidor de trabajo, una máquina en la nube, un homelab— y traer su UI a tu pantalla como si fuera local. Configúralo en **Settings → Remote Instances**.
...
3. Elige cómo se ejecuta Varin allí:
   - **managed** — Varin se instala y se inicia por sí mismo en la máquina remota
   - **external** — conéctate a una que ya está en funcionamiento
4. Conecta.

Varin recorre los pasos —comprobar la conexión, configurar el remoto, iniciar el servidor y reenviar el puerto— y muestra en qué punto está en cada etapa. Cuando llega a **ready**, la UI remota se carga localmente.

## Credenciales

Tú decides si guardar las contraseñas de SSH y de UI o introducirlas cada vez. Si la conexión se cae, Varin informa qué paso falló para que puedas arreglarlo; consulta [Acceso remoto](/es/troubleshooting/remote-access/).

## Enlaces de conexión

Si una máquina remota ya ejecuta Varin, la forma más fácil de conectar la app de escritorio es un enlace de vinculación. En la UI del servidor remoto, abre **Settings → Remote Instances → Conectarse a este servidor → Añadir un dispositivo**, crea un enlace e impórtalo en tu escritorio en **Settings → Remote Instances → Otros servidores de Varin → Importar enlace**. Consulta [Conectar un dispositivo](/es/connect-devices/) para el flujo completo.
