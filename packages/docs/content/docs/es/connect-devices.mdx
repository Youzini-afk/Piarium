...
description: Vincula tu teléfono, escritorio u otro navegador con tu servidor de Varin usando un código QR de un solo uso.
---

# Conectar un dispositivo

Vincula otro dispositivo —la app móvil, la app de escritorio o un navegador en otra máquina— con tu servidor de Varin escaneando un código QR de un solo uso. Es la forma recomendada de conectar dispositivos: no hay puertos que abrir ni direcciones que escribir.

## Vincula un dispositivo

1. En la máquina donde se ejecuta Varin, abre **Settings → Remote Instances → Conectarse a este servidor** y pulsa **Añadir un dispositivo**.
...
5. En el otro dispositivo, escanea el código:
   ...
   - **app de escritorio** — copia el enlace de conexión y pégalo en **Settings → Remote Instances → Otros servidores de Varin → Importar enlace**
