...
Piarium da acceso a tu máquina y a tu código, así que bloquéalo antes de que nadie más que tú pueda alcanzarlo. Esta página cubre la contraseña de UI, las passkeys y lo que debes saber antes de exponer Piarium a una red.

## Define una contraseña de UI

Inicia Piarium con una contraseña y la UI del navegador la pedirá:

```bash
piarium --ui-password be-creative-here
```

También puedes definirla con la variable de entorno `PIARIUM_UI_PASSWORD` en lugar de ponerla en la línea de comandos. Tras iniciar sesión, Piarium recuerda el dispositivo durante un tiempo para no pedírtela cada vez.
...
Una vez definida una contraseña, puedes añadir passkeys (Face ID, Touch ID, una llave de seguridad) para un inicio de sesión más rápido. Añádelas en **Settings → Piarium → Passkeys**.
...
- De forma predeterminada, Piarium solo escucha en tu propia máquina (`127.0.0.1`). Hace falta un cambio deliberado para escuchar más ampliamente, y deberías definir una contraseña primero.
...
- Si pones Piarium detrás de tu propio servidor HTTPS, consulta [Proxy inverso](/es/reverse-proxy/).
...
- [Proxy inverso](/es/reverse-proxy/) — ejecuta Piarium detrás de tu propio servidor
