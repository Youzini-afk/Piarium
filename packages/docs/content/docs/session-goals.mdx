...
A goal turns one prompt into a finish line. Instead of nudging the agent with "continue" after every reply, you set a goal once — and Varin keeps the session working toward it automatically, checking progress with an independent auditor after every turn. It keeps running even while you are away.
...
After the agent stops and the session stays quiet for a moment, Varin:
...
- The goal loop runs in the Varin server, not in your browser tab. Close the tab, lock the phone — the agent keeps working, and you get a notification when the goal settles. The server (desktop app or `varin` process) must stay running.
