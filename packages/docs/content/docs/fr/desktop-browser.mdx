---
title: Navigateur desktop
description: Parcourez n’importe quelle page dans l’application desktop, avec inspection et capture de console.
---

# Navigateur desktop

L’application desktop possède un navigateur intégré pour ouvrir n’importe quelle page juste à côté de votre chat, pointer des éléments pour poser des questions à leur sujet et capturer la console de la page. Ouvrez-le avec le bouton globe dans l’en-tête de l’application.

> Le navigateur desktop est une fonctionnalité **desktop uniquement**. Sur le web, le panneau [aperçu](/fr/preview/) offre les mêmes outils d’inspection et de console pour votre serveur de dev local.

## Inspecter et annoter

Activez **inspect** et cliquez sur n’importe quel élément de la page. Piarium capture une note à son sujet — ce que c’est, ses styles, sa position et une capture d’écran — puis l’attache à votre message de chat. C’est le moyen le plus rapide de dire à l’agent « cet élément, juste ici ».

## Capture de console

Le navigateur collecte la sortie console de la page — erreurs, avertissements et logs — pour que vous puissiez la filtrer et la lire sans ouvrir les outils développeur.

## Pages liées

- [Aperçu et serveurs de dev](/fr/preview/) — les mêmes outils pour votre serveur de dev local
