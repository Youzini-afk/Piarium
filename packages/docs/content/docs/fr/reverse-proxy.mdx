---
title: Reverse proxy
description: Configurez correctement Varin derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez Varin derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’Varin fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si Varin compresse déjà les réponses.
...
- Varin joignable directement sur le LAN
...
location ~ ^/api/(event|global/event|notifications/stream|varin/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- Varin compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par Varin, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
