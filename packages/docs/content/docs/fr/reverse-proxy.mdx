---
title: Reverse proxy
description: Configurez correctement Piarium derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez Piarium derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’Piarium fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si Piarium compresse déjà les réponses.
...
- Piarium joignable directement sur le LAN
...
location ~ ^/api/(event|global/event|notifications/stream|piarium/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- Piarium compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par Piarium, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
