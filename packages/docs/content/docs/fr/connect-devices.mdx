...
description: Associez votre téléphone, votre desktop ou un autre navigateur à votre serveur Piarium avec un QR code à usage unique.
---

# Connecter un appareil

Associez un autre appareil — l’application mobile, l’application desktop ou un navigateur sur une autre machine — à votre serveur Piarium en scannant un QR code à usage unique. C’est la méthode recommandée pour connecter des appareils : aucun port à ouvrir, aucune adresse à taper.

## Associer un appareil

1. Sur la machine qui exécute Piarium, ouvrez **Paramètres → Instances distantes → Se connecter à ce serveur** et pressez **Ajouter un appareil**.
...
3. Choisissez où vous utiliserez l’appareil :
   ...
   - **Partout** — fonctionne à la maison et en déplacement ; en déplacement, le trafic passe par le [Relais privé](/fr/private-relay/), un tunnel chiffré de bout en bout sans aucune configuration
...
5. Sur l’autre appareil, scannez le code :
   ...
   - **application desktop** — copiez plutôt le lien de connexion et collez-le dans **Paramètres → Instances distantes → Autres serveurs Piarium → Importer le lien**
...
- **Le trafic hors du domicile est chiffré de bout en bout.** Avec **Partout**, le trafic en dehors de votre réseau passe par le [Relais privé](/fr/private-relay/), qui ne peut pas lire ce qui le traverse.
...
- [Relais privé](/fr/private-relay/) — comment fonctionnent les connexions « Partout » et ce que le relais peut ou ne peut pas voir
- [Applications mobiles](/fr/mobile/) — installer l’application iOS ou Android
- [Instances distantes](/fr/remote-instances/) — connecter l’application desktop à des serveurs via SSH ou des liens
- [Accès distant](/fr/troubleshooting/remote-access/) — quand un appareil ne se connecte pas
