...
3. Confirmez le dossier du worktree (Piarium en propose un à partir du nom de branche).
4. Créez-le.

Piarium crée la branche, prépare le dossier et démarre une session dedans. Vous pouvez aussi en lancer une directement depuis une [todo](/fr/notes-todos-plans/) ou une [issue ou PR GitHub](/fr/github/).
...
Un worktree peut demander votre attention si son dossier a disparu, si sa branche est détachée, ou si un merge ou rebase est à moitié terminé. Piarium signale ces cas pour que vous puissiez les corriger — voir [Worktrees et Git](/fr/troubleshooting/worktrees-git/).

## Pages liées

- [Multi-run](/fr/multi-run/) — lancer plusieurs sessions worktree à la fois
- [Workflows Git et GitHub](/fr/git/) — commiter et intégrer depuis Piarium
