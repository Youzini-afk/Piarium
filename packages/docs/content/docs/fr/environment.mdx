...
Désactive la compression des réponses API lorsqu’il vaut `true` ou `1`. Cette variable a priorité sur `PIARIUM_COMPRESS_API`.
...
Autre exécutable Git de remplacement. Préférez `PIARIUM_GIT_BINARY` pour une configuration propre à OpenChamber.
