...
description: Pair your phone, desktop, or another browser with your Varin server using a one-time QR code.
---

# Connect a Device

Pair another device — the mobile app, the desktop app, or a browser on another machine — with your Varin server by scanning a one-time QR code. This is the recommended way to connect devices; there are no ports to open and no addresses to type.

## Pair a device

1. On the machine running Varin, open **Settings → Remote Instances → Connect to this server** and press **Add a device**.
...
5. On the other device, scan the code:
   ...
   - **desktop app** — copy the connection link instead and paste it in **Settings → Remote Instances → Other Varin servers → Import Link**
