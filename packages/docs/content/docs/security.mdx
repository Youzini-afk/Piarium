...
Varin gives access to your machine and your code, so lock it down before anyone but you can reach it. This page covers the UI password, passkeys, and what to know before exposing Varin to a network.

## Set a UI password

Start Varin with a password and the browser UI asks for it:

```bash
varin --ui-password be-creative-here
```

You can also set it with the `VARIN_UI_PASSWORD` environment variable instead of putting it on the command line. After signing in, Varin remembers the device for a while so you're not asked every time.
...
Once a password is set, you can add passkeys (Face ID, Touch ID, a security key) for quicker sign-in. Add them at **Settings → Varin → Passkeys**.
...
- By default Varin only listens on your own machine (`127.0.0.1`). It takes a deliberate change to listen more widely, and you should set a password first.
...
- If you put Varin behind your own HTTPS server, see [Reverse Proxy](/reverse-proxy/).
...
- [Reverse Proxy](/reverse-proxy/) — run Varin behind your own server
