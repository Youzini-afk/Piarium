...
**Write Simplified Chinese first; other locales are translations.**
...
1. Create the Chinese source file in `packages/docs/content/docs/`.
...
- every sidebar link must map to an existing Chinese MDX file
...
**Simplified Chinese is the source of truth and lives at the root of `content/docs/`.**
Every other language, including English, mirrors the Chinese files under a locale folder.

### Supported locales

| Language | Content folder | Sidebar `translations` key |
| --- | --- | --- |
| Chinese (Simplified) | _(root, no folder)_ | _(use `label`, do not add `zh-CN`)_ |
| English | `en/` | `en` |
| Ukrainian | `uk/` | `uk` |
| Spanish | `es/` | `es` |
...
Simplified Chinese as the root locale. If a language is added or removed, update both places.

### Translate a page

Mirror the Chinese file under each locale folder, keeping the **exact same
filename and path**. Starlight matches a translation to its Chinese page by path.

```
content/docs/
  install.mdx              # Chinese (source of truth)
  ...
  guides/tunnels.mdx       # nested Chinese page
  en/guides/tunnels.mdx    # its English translation
```
...
ships. Starlight can fall back to Chinese when a translation is missing, but do
...
A label with no translation for the active locale falls back to the Chinese
`label`.
...
frontmatter, or if a sidebar `link` does not resolve to a default (Chinese) page. Run it
